...
static int internal_redir(lsi_cb_param_t *param)
{
    int action = LSI_URL_REDIRECT_INTERNAL;
    g_api->set_uri_qs(
        param->_session, action, desturl, sizeof(desturl) - 1, "", 0);
    return LSI_HK_RET_OK;
}


static int deny_access(lsi_cb_param_t *param)
{
    g_api->set_status_code(param->_session, 406);
    return LSI_HK_RET_ERROR;
}


static int get_testtype(lsi_cb_param_t *param)
{
    int type = 0;
    int len;
    const char *qs = g_api->get_req_query_string(param->_session, &len);
    char buf[256];
    ...
    if ((type < 1) || (type > 10))
    {
        snprintf(buf, sizeof(buf),
                 ...
                 "Query String: [%.*s].\n", len, qs);
        g_api->append_resp_body(param->_session, buf, strlen(buf));
        g_api->end_resp(param->_session);
        return 0;
    }
...
static int session_hook_func(lsi_cb_param_t *param)
{
    int len;
    const char *qs = g_api->get_req_query_string(param->_session, &len);
...
{
    { LSI_HKPT_RECV_REQ_HEADER, get_testtype, LSI_HOOK_NORMAL, LSI_HOOK_FLAG_ENABLED },
