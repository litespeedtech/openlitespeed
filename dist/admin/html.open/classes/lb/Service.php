...
class Service
{
	...
	var $adminL = array();
	var $vhosts = array();
	var $awstats = array(); // not used, but need to leave here
	var $serv = array();
	

	function __construct() {
		$this->FNAME = '/tmp/lslbd/.admin';
		$this->FSTATUS = '/tmp/lslbd/.status';
		$this->FPID = '/tmp/lslbd/lslbd.pid';
	}
	...
	function refreshConf($data)
	{
		...
		foreach( $vhnames as $vhname )
		{
			if ( !isset($this->vhosts[$vhname]) )
				...
				$this->vhosts[$vhname] = $this->vhosts[$vhname]{0} . 'N';
		}
	...
	function issueCmd()
	{
		...
		if ( strncmp( $_SERVER["LSLB_ADMIN_SOCK"], 'uds://', 6 ) == 0 )
		{
		    $sock = socket_create( AF_UNIX, SOCK_STREAM, 0 );
		    $chrootOffset = 0;
		    if ( isset( $_SERVER['LS_CHROOT'] ) )
			$chrootOffset = strlen( $_SERVER['LS_CHROOT'] );
		    $addr = substr( $_SERVER["LSLB_ADMIN_SOCK"], 5 + $chrootOffset );
		...
		else
		{
		    ...
		    $addr = explode( ":", $_SERVER["LSLB_ADMIN_SOCK"] );
	...
	function getServerLog()
	{
		if ( $this->serverLog == null )
		{
			...
			$confpath = $_SERVER['LS_SERVER_ROOT'] . "conf/lslbd_config.xml" ; //fixed location
	...
	{
		//validate param
		if (!preg_match("/^\d+\.\d+(\.\d+)?(RC\d+)?$/", $release))
			return false;
		
		$product = PRODUCT::GetInstance();
		// e.g.: 'lslb-1.6-x86_64-linux.tar.gz'
		...
		$file = strtolower($product->type) . '-' . $release . '-'. $platform . '.tar.gz';
		$downloadurl = 'http://download.litespeedtech.com/packages/lslb/' . $file;
