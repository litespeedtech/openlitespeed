...
class DTblDefBase
{
	...
	protected function loadCommonAttrs()
	{
		...
		$attrs = [
			...
			'appserverEnv' => self::NewSelAttr('appserverEnv', DMsg::ALbl('l_runtimemode'), ['' => '', '0' => 'Development', '1' => 'Production', '2' => 'Staging']),
	...
	protected function add_A_EXT_FCGIAUTH($id)
	{
		...
		$this->_tblDef[$id]->Set(DTbl::FLD_DEFAULTEXTRACT, ['type' => 'fcgiauth']);
	}

	protected function add_A_EXT_LSAPI($id)
	{
		$this->_tblDef[$id] = $this->DupTblDef('A_EXT_FCGI', $id, DMsg::ALbl('l_extlsapi'));
		$this->_tblDef[$id]->Set(DTbl::FLD_DEFAULTEXTRACT, ['type' => 'lsapi']);
	}
	...
	{
		$attrs = [
			self::NewCheckBoxAttr('sslProtocol', DMsg::ALbl('l_protocolver'), ['1' => 'SSL v3.0', '2' => 'TLS v1.0', '4' => 'TLS v1.1', '8' => 'TLS v1.2', '16' => 'TLS v1.3']),
	...
	protected function add_L_SSL_FEATURE($id)
	{
		$attrs = [
			...
			self::NewCheckBoxAttr('enableSpdy', DMsg::ALbl('l_alpn'), ['1' => 'SPDY/2', '2' => 'SPDY/3', '4' => 'HTTP/2', '8' => 'HTTP/3', '0' => DMsg::ALbl('o_none')]),
			self::NewBoolAttr('enableQuic', DMsg::ALbl('l_openudpport'), true, 'allowQuic'),
		];
	...
	protected function get_ctx_attrs($type)
	{
		...
		if ($type == 'charset') {
			return [// todo: merge below
				self::NewSelAttr('addDefaultCharset', DMsg::ALbl('l_adddefaultcharset'), ['off' => DMsg::ALbl('o_off'), 'on' => DMsg::ALbl('o_on')]),
	...
	{
		$attrs = [
			self::NewSelAttr('useServer', DMsg::ALbl('l_logcontrol'), [0 => DMsg::ALbl('o_ownlogfile'), 1 => DMsg::ALbl('o_serverslogfile'), 2 => DMsg::ALbl('o_disabled')], false, 'aclogUseServer'),
			$this->_attrs['fileName3']->dup(null, null, 'accessLog_fileName'),
			$this->_attrs['logFormat'],
			$this->_attrs['logHeaders'],
			$this->_attrs['rollingSize'],
