...
namespace LSWebAdmin\Legacy\Crypto;
...
class Horde_Cipher_blowfish
{
	...
	private function _encryptN($N, $V, $p_idx)
	{
		$N = (int) $N;
		$V = (int) $V;
		$i1 = (int) ((int) ($N >> 24) & 0xFF);
		$i2 = (int) ((int) ($N >> 16) & 0x0FF);
		$i3 = (int) ((int) ($N >> 8) & 0x0FF);
		$i4 = (int) ($N & 0x0FF);
		$pi = (int) ($this->p[$p_idx]);

		$tmp = (int) ((int) $this->s1[$i1] + (int) $this->s2[$i2]);
		$tmp = (int) ($tmp ^ (int) $this->s3[$i3]);
		$tmp += (int) $this->s4[$i4];
		$tmp = (int) ((int) ($tmp) ^ $pi);
		$V ^= $tmp;
		return (int) $V;
	 ...
	 * @return String  The encrypted output.
	 */
	private function _encryptBlock($L, $R)
	{
		$L ^= (int)$this->p[0];
		$R = $this->_encryptN($L, $R, 1);
		$L = $this->_encryptN($R, $L, 2);
		$R = $this->_encryptN($L, $R, 3);
		$L = $this->_encryptN($R, $L, 4);
		$R = $this->_encryptN($L, $R, 5);
		$L = $this->_encryptN($R, $L, 6);
		$R = $this->_encryptN($L, $R, 7);
		$L = $this->_encryptN($R, $L, 8);
		$R = $this->_encryptN($L, $R, 9);
		$L = $this->_encryptN($R, $L, 10);
		$R = $this->_encryptN($L, $R, 11);
		$L = $this->_encryptN($R, $L, 12);
		$R = $this->_encryptN($L, $R, 13);
		$L = $this->_encryptN($R, $L, 14);
		$R = $this->_encryptN($L, $R, 15);
		$L = $this->_encryptN($R, $L, 16);
		$R ^= (int)$this->p[17];

		return ['L' => (int)$R, 'R' => (int)$L];
	}

	/* retired due to php8 implicit float conversion to int
	 * function _encryptBlock0($L, $R)
	  {
	  $L ^= $this->p[0];
...
// end change for phpMyAdmin
		...
		$R = $this->_encryptN($L, $R, 16);
		$L = $this->_encryptN($R, $L, 15);
		$R = $this->_encryptN($L, $R, 14);
		$L = $this->_encryptN($R, $L, 13);
		$R = $this->_encryptN($L, $R, 12);
		$L = $this->_encryptN($R, $L, 11);
		$R = $this->_encryptN($L, $R, 10);
		$L = $this->_encryptN($R, $L, 9);
		$R = $this->_encryptN($L, $R, 8);
		$L = $this->_encryptN($R, $L, 7);
		$R = $this->_encryptN($L, $R, 6);
		$L = $this->_encryptN($R, $L, 5);
		$R = $this->_encryptN($L, $R, 4);
		$L = $this->_encryptN($R, $L, 3);
		$R = $this->_encryptN($L, $R, 2);
		$L = $this->_encryptN($R, $L, 1);

		$decrypted = pack("NN", (int)($R ^ (int)$this->p[0]), $L);
		return $decrypted;
	}

	/*function decryptBlock0($block, $key = null)
	{
		if (!is_null($key)) {
			$this->setKey($key);
		}
...
function PMA_blowfish_encrypt($data, $secret)
{
	...
	for ($i = 0; $i < strlen($data); $i += 8) {
		...
		if (strlen($block) < 8) {
			$block = $pma_cipher->full_str_pad($block, 8, "\0", 1);
		}
