...
class DTblDef extends DTblDefBase
{
	...
	{
		$rules = self::NewTextAreaAttr('rules', null, 'cust', true, 5, null, 1, 1); // no label, will use table header
	...
	protected function add_S_TUNING_SSL($id)
	{
		$attrs = [
			...
			self::NewParseTextAttr('sslOcspProxy', DMsg::ALbl('l_ocspproxy'), '/^((http|https):\/\/)?([[:alnum:]._-]+|\[[[:xdigit:]:]+\])(:\d+)$/', DMsg::ALbl('parse_ocspproxy')),
