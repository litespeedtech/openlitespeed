...
$_tipsdb['accessLog_logFormat'] = new DAttrHelp("Log Format", ' アクセスログのログ形式を指定します。 ログフォーマットが設定されると、&quot;ログヘッダー&quot;の設定より優先されます。', '', '文字列。ログフォーマットの構文はApache 2.0のカスタムと互換性があります  <a href="http://httpd.apache.org/docs/2.0/mod/mod_log_config.html#formats" target="_blank" rel="noopener noreferrer">log format</a>。', '<br> <b>共通ログフォーマット（CLF）</b><br/>    &quot;%h %l %u %t \&quot;%r\&quot; %>s %b&quot;<br/><br/><b>バーチャルホストによる共通ログフォーマット</b><br/>    &quot;%v %h %l %u %t \&quot;%r\&quot; %>s %b&quot;<br/><br/><b>NCSA拡張/結合ログフォーマット</b><br/>    &quot;%h %l %u %t \&quot;%r\&quot; %>s %b \&quot;%{Referer}i\&quot; \&quot;%{User-agent}i\&quot; <br/><br/><b>FoobarのログCookie値</b><br/>    &quot;%{Foobar}C&quot;');
...
$_tipsdb['enableGzip'] = new DAttrHelp("GZIPを有効にする", 'このバーチャルホストに対してGZIP圧縮を有効にするかどうかを指定します。 この設定は、サーバーレベルでGZIP圧縮が有効になっている場合にのみ有効です。 圧縮設定はサーバーレベルで設定されます（チューニング> GZIP）。', '', 'ラジオボックスから選択', '');
...
$_tipsdb['enableIpGeo'] = new DAttrHelp("IPジオロケーションを有効にする", ' IPジオロケーション検索を有効/無効にするかどうかを指定します。 サーバ、バーチャルホスト、コンテキストレベルで設定できます。 値 「設定しない」を使用すると、IPジオロケーションはデフォルトで無効になります。', '', 'ラジオボックスから選択', '');
...
$_tipsdb['enableSpdy'] = new DAttrHelp("SPDY/HTTP2を有効にする", 'HTTP/2とSPDYは、ページロード時間を短縮する目的で、HTTPネットワークプロトコルの新バージョンです。 より多くの情報は<a href=&quot;http://en.wikipedia.org/wiki/HTTP/2&quot;>http://en.wikipedia.org/wiki/HTTP/2</a>で見つけることができます。', '', '有効にするプロトコルを確認します。 すべてのボックスをチェックしないと、SPDYとHTTP/2のサポート（デフォルト）が有効になります。 SPDYとHTTP/2を無効にする場合は、「なし」のみをチェックし、その他のチェックボックスはすべてオフにします。', '');
...
$_tipsdb['modParams'] = new DAttrHelp("モジュールのパラメータ", 'モジュールのパラメータを設定します。 モジュールパラメータは、モジュール開発者によって定義されます。<br/><br/>デフォルト値をグローバルに割り当てるようにサーバー構成内の値を設定します。 ユーザーは、この設定をリスナー、バーチャルホストまたはコンテキスト・レベルでオーバーライドできます。 「設定しない」ラジオボタンが選択されている場合、それは上位レベルから継承されます。', '', 'モジュールインタフェースで指定されます。', '');
...
$_tipsdb['moduleEnabled'] = new DAttrHelp("フックを有効にする", 'モジュールフックをグローバルに有効または無効にします。 <br/>「設定しない」ラジオボタンが選択され、モジュールにフック機能が含まれている場合、デフォルトが有効になります。 ユーザーは、各レベルでこのグローバル設定を上書きできます。', '', 'ラジオボックスから選択', '');

$_tipsdb['moduleEnabled_lst'] = new DAttrHelp("フックを有効にする", 'Listenerレベルでモジュールフックを有効または無効にします。 モジュールにTCP/IPレベルのフック（L4_BEGSESSION、L4_ENDSESSION、L4_RECVING、L4_SENDING）がある場合のみ、この設定が有効になります。<br/><br/>「設定しない」ラジオボタンが選択されている場合、デフォルトはServer設定から継承されます。 ユーザーはデフォルト設定を上書きするためにここで設定するだけです。', '', 'ラジオボックスから選択', '');

$_tipsdb['moduleEnabled_vh'] = new DAttrHelp("フックを有効にする", 'バーチャルホストまたはコンテキスト・レベルでモジュール・フックを使用可能または使用不可にします。 モジュールにHTTPレベルのフックがある場合のみ、この設定が有効になります。<br/><br/>「設定しない」ラジオボタンを選択すると、バーチャルホストレベルのデフォルト設定がサーバー設定から継承されます。 コンテキストレベルの設定は、バーチャルホストレベルから継承されます。 ユーザーはデフォルト設定を上書きするためにここで設定するだけです。', '', 'ラジオボックスから選択', '');
...
$_tipsdb['renegProtection'] = new DAttrHelp("SSL再交渉保護", 'SSL再交渉保護を有効にするかどうかを指定します。 SSLハンドシェイクベースの攻撃に対して防御します。 デフォルト値は &quot;Yes&quot;です。', '', 'ラジオボックスから選択', '');
...
$_tipsdb['spdyAdHeader'] = new DAttrHelp("SPDY 広告", 'SPDYプロトコルが使用可能であることをクライアントに通知するかどうかを指定します。 設定されている場合、クライアントがHTTP接続を介してSPDY対応のWebサイトにアクセスすると、LSWSは代替プロトコルの応答ヘッダーを送信します。 これを行うには、WebサイトにHTTPとHTTPSの両方の接続を設定し、SPDYサポートを有効にする必要があります。', '', 'ポート:プロトコル 文字列', 'ポート443でSPDY/3が有効になっている場合、文字列を &quot;443：npn-spdy/3&quot;に設定できます。');
