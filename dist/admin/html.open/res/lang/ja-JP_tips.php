...
$_tipsdb['UDBpass1'] = new DAttrHelp("パスワードを再入力", 'パスワードは任意の長さで、任意の文字を含めることができます。', '', '', '');
...
$_tipsdb['accessAllowed'] = new DAttrHelp("アクセス許可リスト", 'このコンテキストでリソースにアクセスできるIPまたはサブネットワークを指定します。 &quot;アクセス拒否リスト&quot;とサーバー/バーチャルホスト・レベルのアクセス制御とともに、アクセシビリティは、クライアントのIPアドレスが入る最小の範囲によって決まります。', '', 'IP/サブネットワークのカンマ区切りリスト。', 'サブネットワークは以下のように書くことができます <span class=&quot;lst-inline-token lst-inline-token--value&quot;>192.168.1.0/255.255.255.0</span>, <span class=&quot;lst-inline-token lst-inline-token--value&quot;>192.168.1</span>, 又は <span class=&quot;lst-inline-token lst-inline-token--value&quot;>192.168.1.*</span>.');
...
$_tipsdb['accessLog_fileName'] = new DAttrHelp("ファイル名", 'アクセスログファイル名。', ' アクセスログファイルは別のディスクに配置してください。', 'ファイル名への絶対パス又は$SERVER_ROOTからの相対パス', '');
...
$_tipsdb['acme'] = new DAttrHelp("ACMEによるAutoCert", 'Automatic Certificate Management Environment(ACME)認証プロトコルを使用して、SSL証明書を自動的に生成および更新します（設定済みの場合）。詳細はこちらを参照してください：<br/><a href=" https://docs.openlitespeed.org/config/advanced/acme/ " target="_blank" rel="noopener noreferrer"> 自動SSL証明書（ACME） </a><br/><br/>受け付ける値：<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>オン</span> - 証明書の自動生成と更新を有効にします。バーチャルホストはこの設定を個別に上書きできます。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>オフ</span> - 無効にしますが、バーチャルホストはこの設定を個別に上書きできます。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>無効</span> - （サーバーレベルのみ）サーバー全体でACMEを強制無効化します；バーチャルホストはこの設定を上書きできません。<br/><br/>デフォルト値：<br/><b>サーバーレベル：</b>Off<br/><b>VHレベル：</b>サーバーレベル設定を継承', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['addDefaultCharset'] = new DAttrHelp("デフォルトの文字セットを追加", 'コンテンツタイプが &quot;text/html&quot;または &quot;text/plain&quot;のいずれかのパラメータがない場合、文字セットタグを &quot;Content-Type&quot;レスポンスヘッダーに追加するかどうかを指定します。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>オフ</span>に設定すると、この機能は無効になります。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>オン</span>に設定すると、&quot;カスタマイズされたデフォルトの文字セット&quot;で指定された文字セットまたはデフォルトの &quot;iso-8859-1&quot;のいずれかが追加されます。', '', 'ラジオボタンから選択', '');
...
$_tipsdb['adminRetypePass'] = new DAttrHelp("パスワードを再入力", '新しいパスワードをもう一度入力します。新しいパスワードと一致する必要があります。', '', '', '');
...
$_tipsdb['allowQuic'] = new DAttrHelp("HTTP3/QUIC（UDP）ポートを開く", 'このリスナーにマップされたバーチャルホストでHTTP3/QUICネットワークプロトコルの使用を許可します。 この設定を有効にするには、サーバーレベルでも&quot;HTTP3/QUICを有効にする&quot;を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定する必要があります。 デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>です。', 'この設定を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定しても、バーチャルホストレベルの&quot;HTTP3/QUICを有効にする&quot;設定でHTTP3/QUICを無効にできます。', '', '');

$_tipsdb['allowSetUID'] = new DAttrHelp("UIDの設定を許可する", '設定されたUIDビットがCGIスクリプトに許可されるかどうかを指定します。 設定されたUIDビットが許可され、CGIスクリプトに対して設定されたUIDビットが有効になっている場合、CGIスクリプトがどのユーザーに代わって起動されたかにかかわらず、CGIプロセスのユーザーIDは、CGIスクリプトの所有者のユーザーIDに切り替わります。<br/>デフォルトは「オフ」です。', '可能であれば、UID CGIスクリプトを設定しないでください。 本質的にセキュリティリスクです。', 'ラジオボタンから選択', '');

$_tipsdb['allowSymbolLink'] = new DAttrHelp("シンボリックリンクをたどる", 'このバーチャルホスト内のシンボリックリンクをたどるかどうかを指定します。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>IF OWNER MATCH</span>オプションは、同じ所有権を持つ場合にのみシンボリックリンクに従います。 この設定は、デフォルトのサーバーレベルの設定よりも優先されます。', 'セキュリティを強化するために、この機能を無効にしてください。 パフォーマンスを向上させるには、有効にします。', 'ドロップダウンリストから選択', '');
...
$_tipsdb['appType'] = new DAttrHelp("アプリケーション種別", 'このコンテキストで使用するアプリケーションのタイプです。Rack/Rails、WSGI、またはNode.jsがサポートされています。', '', '', '');

$_tipsdb['appserverEnv'] = new DAttrHelp("ランタイムモード", 'アプリケーションを実行するモードを指定します。&quot;Development&quot;、&quot;Production&quot;、または&quot;Staging&quot;を選択できます。 デフォルトは&quot;Production&quot;です。', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['as_startupfile'] = new DAttrHelp("スタートアップファイル", 'アプリケーションの起動に使用するファイルの場所です。アプリケーションルートディレクトリからの相対パスで指定します。<br/><br/>デフォルトの起動ファイル名には、Rack/Railsの&#039;config.ru&#039;、WSGIの&#039;wsgi.py&#039;と&#039;passenger_wsgi.py&#039;、NodeJSの&#039;app.js&#039;が含まれます。', '', 'パス', '');

$_tipsdb['authName'] = new DAttrHelp("認証名", '現在のコンテキストの認証レルムの代替名を指定します。 指定しない場合、元のレルム名が使用されます。 認証名は、ブラウザのログインポップアップに表示されます。', '', 'テキスト', '');

$_tipsdb['autoFix503'] = new DAttrHelp("503エラーの自動修復", 'サーバーをグレースフルリスタートすることで&quot;503 Service Unavailable&quot;エラーの修復を試みるかどうかを指定します。 &quot;503&quot;エラーは通常、外部アプリケーションの誤動作によって発生し、Webサーバーの再起動で一時的に修復できることがあります。 有効にすると、30秒以内に&quot;503&quot;エラーが30件を超えた場合、サーバーは自動的に再起動します。<br/><br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>', '', 'ラジオボックスから選択', '');

$_tipsdb['autoIndex'] = new DAttrHelp("自動インデックス", '&quot;インデックスファイル&quot;にリストされているインデックスファイルがディレクトリで使用できないときに、その場でディレクトリインデックスを生成するかどうかを指定します。 このオプションは、バーチャルホストおよびコンテキストレベルでカスタマイズ可能で、明示的に上書きされるまでディレクトリツリーに沿って継承されます。 生成された索引ページをカスタマイズできます。 オンラインWikiのHow-toを確認してください。', ' 可能であれば、自動インデックスをオフにして、機密データの漏えいを防ぐことをお勧めします。', 'ラジオボックスから選択', '');
...
$_tipsdb['autoStart'] = new DAttrHelp("自動起動", 'Webサーバーでアプリケーションを自動的に開始するかどうかを指定します。 同じマシン上で実行されているFastCGIおよびLSAPIアプリケーションのみを自動的に起動することができます。 &quot;アドレス&quot;のIPはローカルIPでなければなりません。 メインサーバプロセスではなくLiteSpeed CGIデーモンを起動すると、システムのオーバーヘッドを軽減できます。', '', 'ドロップダウンリストから選択', '');

$_tipsdb['backlog'] = new DAttrHelp("バックログ", 'リスニングソケットのバックログを指定します。 &quot;自動起動&quot;が有効な場合は必須です。', '', '整数', '');
...
$_tipsdb['blockBadReq'] = new DAttrHelp("不良リクエストブロック", '不正な形式のHTTPリクエストを&quot;禁止期間（秒）&quot;の間送信し続けるIPをブロックします。デフォルトは<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>です。 これは、不要なリクエストを繰り返し送信するボットネット攻撃のブロックに役立ちます。', '', 'ラジオボックスから選択', '');

$_tipsdb['brStaticCompressLevel'] = new DAttrHelp("Brotli圧縮レベル（静的ファイル）", '静的ファイルに適用するBrotli圧縮レベルを指定します。範囲は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>（無効）から<span class=&quot;lst-inline-token lst-inline-token--value&quot;>11</span>（最高）です。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>に設定すると、Brotli圧縮はグローバルに無効になります。<br/>デフォルト値：5', ' ネットワーク帯域幅を節約します。html、css、javascriptファイルなどのテキストベースのレスポンスが最も効果を受け、平均で元のサイズの半分まで圧縮できます。', '0から11までの数値。', '');

$_tipsdb['bubbleWrap'] = new DAttrHelp("Bubblewrapコンテナ", 'CGIプロセス（PHPプログラムを含む）をbubblewrapサンドボックス内で起動する場合は、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>有効</span>に設定します。bubblewrapの使用方法の詳細は次を参照してください。 <a href="   https://wiki.archlinux.org/title/Bubblewrap " target="_blank" rel="noopener noreferrer">   https://wiki.archlinux.org/title/Bubblewrap </a> この設定を使用する前に、システムにBubblewrapがインストールされている必要があります。<br/><br/>サーバーレベルで&quot;Disabled&quot;に設定されている場合、この設定をバーチャルホストレベルで有効にすることはできません。<br/><br/>デフォルト値：<br/><b>サーバーレベル：</b>Disabled<br/><b>VHレベル：</b>サーバーレベル設定を継承', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['certChain'] = new DAttrHelp("証明書チェーン", '証明書がチェーン証明書であるかどうかを指定します。 チェーン証明を格納するファイルは、PEM形式でなければならず、証明書は最下位レベル（実際のクライアントまたはサーバー証明書）から最上位（ルート）CAまでの連鎖の順序でなければなりません。', '', 'ラジオボックスから選択', '');

$_tipsdb['certFile'] = new DAttrHelp("証明書ファイル", 'SSL証明書ファイルのファイル名。', '証明書ファイルは、サーバーが実行されるユーザーへの読み取り専用アクセスを可能にする安全なディレクトリに配置する必要があります。', 'ファイル名への絶対パス又は$SERVER_ROOTからの相対パス', '');
...
$_tipsdb['cgroups'] = new DAttrHelp("cgroups", 'プロセス群のリソース使用量（CPU、メモリ、ディスクI/O、ネットワークなど）を制限、集計、分離するLinuxカーネル機能です。ファイル<span class=&quot;lst-inline-token lst-inline-token--value&quot;>/sys/fs/cgroup/cgroup.controllers</span>の存在で判定されるcgroups v2を実行している必要があります。<br/><br/>サーバーレベルで<span class=&quot;lst-inline-token lst-inline-token--value&quot;>無効</span>に設定すると、この設定はサーバー全体で無効になります。それ以外の場合、サーバーレベルの設定はバーチャルホストレベルで上書きできます。<br/><br/>デフォルト値：<br/><b>サーバーレベル：</b>Off<br/><b>VHレベル：</b>サーバーレベル設定を継承', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['configureparams'] = new DAttrHelp("パラメータの設定", 'PHPビルド用のconfigureパラメータを設定します。次のステップをクリックすると、Apache固有のパラメータと&quot;--prefix&quot;値は自動的に削除され、&quot;--with-litespeed&quot;が自動的に追加されます。 （プレフィックスは上のフィールドで設定できます。）これにより、既存の稼働中のPHPビルドのphpinfo()出力からconfigureパラメータをそのままコピーして貼り付けることができます。', '', 'スペース区切りの一連のオプション（二重引用符を使用する場合と使用しない場合）', '');

$_tipsdb['connTimeout'] = new DAttrHelp("コネクションタイムアウト（秒）", '1つの要求の処理中に許容される最大接続アイドル時間を指定します。 この時間の間、接続がアイドル状態の場合、つまりI/Oアクティビティがない場合は、接続が閉じられます。', '潜在的なDoS攻撃の間に不在接続を回復するのに役立つように、この値を低く設定します。', '整数', '');

$_tipsdb['consoleSessionTimeout'] = new DAttrHelp("セッションタイムアウト（秒）", 'WebAdminコンソールのセッションタイムアウト時間をカスタマイズします。 デフォルトは60秒です。', ' 本番環境では適切な値を設定してください。通常は300秒未満です。', '整数', '');
...
$_tipsdb['crlFile'] = new DAttrHelp("クライアント失効ファイル", ' 取り消されたクライアント証明書を列挙するPEMエンコードCA CRLファイルを含むファイルを指定します。これは&quot;クライアント失効パス&quot;の代替として、または追加で使用できます。', '', 'ファイル名への絶対パス又は$SERVER_ROOTからの相対パス', '');

$_tipsdb['crlPath'] = new DAttrHelp("クライアント失効パス", ' 取り消されたクライアント証明書のPEMエンコードされたCA CRLファイルを含むディレクトリを指定します。 このディレクトリのファイルはPEMでエンコードする必要があります。 これらのファイルは、ハッシュファイル名、hash-value.rNによってアクセスされます。 ハッシュファイル名の作成については、openSSLまたはApache mod_sslのドキュメントを参照してください。', '', 'パス', '');
...
$_tipsdb['defaultCharsetCustomized'] = new DAttrHelp("カスタマイズされたデフォルトの文字セット", '&quot;デフォルトの文字セットを追加&quot;が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>オン</span>のときに使用される文字セットを指定します。        これはオプションです。 デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;> iso-8859-1 </span>です。        &quot;デフォルトの文字セットを追加&quot;が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>オフ</span>の場合、このエントリは無効です。', '', '文字セットの名前。', 'utf-8');
...
$_tipsdb['domainName'] = new DAttrHelp("ドメイン", 'マッピングドメイン名を指定します。 ドメイン名は大文字と小文字を区別せず、先頭の&quot;www.&quot;は削除されます。 ワイルドカード文字&quot;*&quot;と&quot;?&quot;を使用できます。 &quot;?&quot;は1文字だけを表します。 &quot;*&quot;は任意の数の文字を表します。 重複したドメイン名は許可されません。', ' リスナーが1つのバーチャルホスト専用の場合は、不要なチェックを避けるために、ドメイン名に常に<span class=&quot;lst-inline-token lst-inline-token--value&quot;>*</span>を使用します。 可能であれば、catchallドメイン以外でワイルドカード文字を使用するドメイン名は避けてください。', 'カンマ区切りリスト。', 'www?.example.com<br/>&quot;*.mydomain.com&quot;はmydomain.comのすべてのサブドメインと一致します。<br/>&quot;*&quot;は単独でcatchallドメインとなり、一致しなかった任意のドメイン名に一致します。');

$_tipsdb['dynReqPerSec'] = new DAttrHelp("動的リクエスト/秒", '確立された接続の数に関係なく、1秒ごとに処理できる単一のIPアドレスからの動的に生成されるコンテンツへの要求の最大数を指定します。 この制限に達すると、今後のすべての動的コンテンツへのリクエストは、次の秒までタールピットされます。<br/><br/>静的コンテンツの要求制限は、この制限とは関係ありません。 このクライアントごとの要求制限は、サーバーまたはバーチャルホストレベルで設定できます。 バーチャルホストレベルの設定は、サーバーレベルの設定よりも優先されます。', 'この制限によって、信頼できるIPまたはサブネットワークは制限されません。', '整数', '');

$_tipsdb['enableCoreDump'] = new DAttrHelp("コアダンプを有効にする", 'サーバーが&quot;root&quot;ユーザーによって起動されたときにコアダンプを有効にするかどうかを指定します。 最新のUnixシステムでは、セキュリティ上の理由から、ユーザーIDまたはグループIDを変更するプロセスはコアファイルをダンプできません。 ただし、コアダンプがあると問題の根本原因を特定しやすくなります。 このオプションは、Linuxカーネル2.4以上でのみ動作します。 Solarisユーザーは<span class=&quot;lst-inline-token lst-inline-token--command&quot;>coreadm</span>コマンドを使用してこの機能を制御してください。', ' サーバーログファイルに<span class=&quot;lst-inline-token lst-inline-token--value&quot;>no core file created</span>と表示されている場合にのみ有効にします。 コアファイルを生成した直後に無効にしてください。 コアダンプが作成されたらバグレポートを提出してください。', 'ラジオボックスから選択', '');

$_tipsdb['enableDHE'] = new DAttrHelp("DHキー交換を有効にする", 'さらにSSL暗号化のためにDiffie-Hellman鍵交換を使用できます。', ' DHキーの交換は、RSAキーを使用するよりも安全です。 ECDHとDHキーの交換は同等に安全です。<br/><br/> DHキー交換を有効にするとCPU負荷が増加し、ECDHキー交換とRSAよりも遅くなります。 ECDH鍵交換が利用可能な場合は、そちらを優先することをお勧めします。', 'ラジオボックスから選択', '');

$_tipsdb['enableDynGzipCompress'] = new DAttrHelp("動的GZIP圧縮を有効化", '動的に生成されるレスポンスのGZIP圧縮を制御します。<br/>この設定を有効にするには、GZIP compressionを<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定する必要があります。<br/>デフォルト値：Yes', ' 動的レスポンスを圧縮するとCPUとメモリの使用量は増えますが、ネットワーク帯域幅を節約できます。', 'ラジオボックスから選択', '');

$_tipsdb['enableECDHE'] = new DAttrHelp("ECDH鍵交換を有効にする", 'さらにSSL暗号化のために楕円曲線 Diffie-Hellman鍵交換を使用できます。', ' ECDH鍵交換は、RSA鍵だけを使用するより安全です。 ECDHとDHキーの交換は同等に安全です。<br/><br/> ECDH鍵交換を有効にするとCPU負荷が増加し、RSA鍵だけを使用する場合よりも遅くなります。', 'ラジオボックスから選択', '');
...
$_tipsdb['enableGzipCompress'] = new DAttrHelp("圧縮を有効にする", '静的レスポンスと動的レスポンスの両方に対するGZIP/Brotli圧縮を有効にします。<br/>デフォルト値：Yes', ' ネットワーク帯域幅を節約するために有効にします。html、css、javascriptファイルなどのテキストベースのレスポンスが最も効果を受け、平均で元のサイズの半分まで圧縮できます。', 'ラジオボックスから選択', '');

$_tipsdb['enableIpGeo'] = new DAttrHelp("IPジオロケーションを有効化", ' IPジオロケーション検索を有効または無効にするかどうかを指定します。 サーバー、バーチャルホスト、またはコンテキストレベルで設定できます。&quot;Not Set&quot;値を使用すると、IPジオロケーションはデフォルトで無効になります。', '', 'ラジオボックスから選択', '');
...
$_tipsdb['enableRecaptcha'] = new DAttrHelp("CAPTCHAを有効にする", '現在のレベルでCAPTCHA保護機能を有効にします。この機能を使用するには、サーバーレベルでこの設定を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定する必要があります。<br/><br/>デフォルト値：<br/><b>サーバーレベル：</b><span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span><br/><b>VHレベル：</b>サーバーレベル設定を継承', '', 'ラジオボックスから選択', '');
...
$_tipsdb['enableSpdy'] = new DAttrHelp("ALPN", 'Application-Layer Protocol Negotiation(ALPN)は、HTTP/3とHTTP/2ネットワークプロトコルを選択的に有効にするために使用されます。<br/><br/>HTTP/2とHTTP3を無効にする場合は、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>なし</span>を選択し、他のすべてのチェックボックスを未選択のままにしてください。<br/><br/>デフォルト値: すべて有効', 'これはリスナーレベルとバーチャルホストレベルで設定できます。', 'チェックボックスから選択', '');

$_tipsdb['enableStapling'] = new DAttrHelp("OCSP Staplingを有効化", 'OCSPステープルを有効にするかどうかを決定します。これは、公開鍵証明書を検証するより効率的な方法です。', '', 'ラジオボックスから選択', '');

$_tipsdb['enableh2c'] = new DAttrHelp("平文TCP上のHTTP/2を有効化", '暗号化されていないTCP接続に対してHTTP/2を有効にするかどうかを指定します。 デフォルトは無効です。', '', 'ラジオボタンから選択', '');
...
$_tipsdb['errPage'] = new DAttrHelp("カスタマイズエラーページ", 'サーバーが要求を処理する際に問題が発生すると、サーバーはエラーコードとhtmlページをエラーメッセージとしてWebクライアントに返します。 エラーコードはHTTPプロトコルで定義されています（RFC 2616参照）。 LiteSpeed Webサーバーには、エラーコードごとにデフォルトのエラーページが組み込まれていますが、各エラーコードに対してカスタマイズされたページを設定することもできます。 これらのエラーページは、各バーチャルホストごとに一意になるようにさらにカスタマイズすることができます。', '', '', '');
...
$_tipsdb['extAppType'] = new DAttrHelp("タイプ", '外部アプリケーションのタイプを指定します。 アプリケーションタイプは、提供するサービスまたはサーバーとの通信に使用するプロトコルによって区別されます。 以下から選んでください。 <ul> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>FastCGI</span>: <span class=&quot;lst-inline-token lst-inline-token--value&quot;>Responder</span>ロールを持つFastCGIアプリケーションです。</li> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>FastCGI オーソライザ</span>: <span class=&quot;lst-inline-token lst-inline-token--value&quot;>オーソライザ</span>ロールを持つFastCGIアプリケーション</li> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>サーブレットエンジン</span>: tomcatなどのAJPv13コネクタを持つサーブレットエンジンです。</li> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Web サーバー</span>: HTTPプロトコルをサポートするWebサーバーまたはアプリケーションサーバー。</li> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>LiteSpeed SAPI アプリ</span>: LSAPIプロトコルを使用してWebサーバーと通信するアプリケーションです。</li> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>ロードバランサー</span>:ワーカーアプリケーション間の負荷を分散できるバーチャルアプリケーションです。</li> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>パイプログロガー</span>: STDINストリームで受け取ったアクセスログエントリを処理できるアプリケーションです。</li> </ul>', 'ほとんどのアプリケーションは、LSAPIまたはFastCGIプロトコルを使用します。 LSAPIはPHP、Ruby、Pythonをサポートしています。 PerlはFastCGIで使用できます。 （PHP、Ruby、およびPythonはFastCGIを使用して実行するように設定することもできますが、LSAPIを使用する方が高速です）。Javaはサーブレットエンジンを使用します。', 'ドロップダウンリストから選択', '');

$_tipsdb['extAuthorizer'] = new DAttrHelp("オーソライザ", '権限のある/権限のないかの決定を生成するために使用できる外部アプリケーションを指定します。 現在、FastCGI Authorizerのみが使用可能です。 FastCGI Authorizerの役割の詳細については、<a href="   https://fastcgi-archives.github.io/ " target="_blank" rel="noopener noreferrer">   https://fastcgi-archives.github.io/ </a>を参照してください。 。', '', 'ドロップダウンリストから選択', '');

$_tipsdb['extGroup'] = new DAttrHelp("suEXECグループ", '外部アプリケーションを実行するグループ名を指定します。 未設定の場合は、バーチャルホストレベルの設定を継承します。<br/><br/>デフォルト値：未設定', '', '有効なグループ名。', '');

$_tipsdb['extMaxIdleTime'] = new DAttrHelp("最大アイドル時間", '外部アプリケーションがサーバーによって停止され、アイドルリソースが解放されるまでの最大アイドル時間を指定します。 &quot;-1&quot;に設定すると、ProcessGroupモードで実行されている場合を除き、外部アプリケーションはサーバーによって停止されません。 ProcessGroupモードでは、アイドル状態の外部アプリケーションは30秒後に停止されます。<br/><br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>-1</span>', ' この機能は大量ホスティング環境で特に便利です。 あるバーチャルホストが所有するファイルに、別のバーチャルホストの外部アプリケーションスクリプトからアクセスされるのを防ぐため、 SetUIDモードで多数の異なるアプリケーションを同時に実行する必要があります。 これらの外部アプリケーションが不要にアイドル状態のままにならないよう、この値を低く設定してください。', '整数', '');
...
$_tipsdb['extUser'] = new DAttrHelp("suEXECユーザー", '外部アプリケーションを実行するユーザー名を指定します。 未設定の場合は、バーチャルホストレベルの設定を継承します。<br/><br/>デフォルト値：未設定', '', '有効なユーザー名。', '');
...
$_tipsdb['fileUpload'] = new DAttrHelp("ファイルアップロード", 'Request Body Parserを使用してファイルをアップロードしてサーバーのローカルディレクトリにファイルを解析し、第三者のモジュールによる悪意のある行為を簡単にスキャンできるようにする、追加のセキュリティ機能を提供します。 Request Body Parserは、&quot;ファイルパスでアップロードデータを渡す&quot;が有効になっているか、モジュールがLSI_HKPT_HTTP_BEGINレベルでLSIAPIのset_parse_req_bodyを呼び出すときに使用されます。 ソースパッケージで提供されるAPIの例', '', '', '');

$_tipsdb['followSymbolLink'] = new DAttrHelp("シンボリックリンクに従う", '静的ファイルを提供する際のシンボリックリンクのサーバーレベルのデフォルト設定を指定します。<br/><br/>選択肢は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>オーナーと一致する場合</span> <span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>です。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>は、シンボリックリンクに常に従うようにサーバーを設定します。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>所有者の一致がの場合</span>は、リンクの所有者とターゲットの所有者が同じ場合にのみシンボリックリンクに従うようにサーバーを設定します。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>は、サーバーがシンボリックリンクを決して辿らないことを意味します。 この設定はバーチャルホスト設定で上書きできますが、.htaccessファイルで上書きすることはできません。', '最高のセキュリティを実現するには、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>所有者と一致する場合</span>を選択します。 最高のパフォーマンスを得るには、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>を選択します。', 'ドロップダウンリストから選択', '');

$_tipsdb['forceGID'] = new DAttrHelp("強制GID", 'suEXECモードで起動したすべての外部アプリケーションに使用するグループIDを指定します。 ゼロ以外の値に設定すると、すべてのsuEXEC外部アプリケーション（CGI/FastCGI/LSAPI）がこのグループIDを使用します。 これにより、外部アプリケーションが他のユーザーが所有するファイルにアクセスするのを防ぐことができます。<br/><br/>たとえば、共有ホスティング環境では、LiteSpeedはユーザー &quot;www-data&quot;として実行され、グループ &quot;www-data&quot;として実行されます。 各docrootは、 &quot;www-data&quot;のグループと許可モード0750を持つユーザーアカウントによって所有されています。 強制GIDが &quot;nogroup&quot;（または &#039;www-data&#039;以外のグループ）に設定されている場合、すべてのsuEXEC外部アプリケーションは特定のユーザーとして実行されますが、グループ &quot;nogroup&quot;に実行されます。 これらの外部アプリケーション・プロセスは（ユーザーIDのために）特定のユーザーが所有するファイルには引き続きアクセスできますが、他のユーザーのファイルにアクセスするためのグループ権限は持っていません。 一方、サーバは、（グループIDのために）どんなユーザのdocrootディレクトリ下でもファイルを提供することができます。', 'システムユーザーが使用するすべてのグループを除外するのに十分な高さに設定します。', '整数', '');

$_tipsdb['forceStrictOwnership'] = new DAttrHelp("厳格な所有権チェックを強制する", '厳密なファイル所有権チェックを実施するかどうかを指定します。 有効になっている場合、Webサーバーは、提供されるファイルの所有者がバーチャルホストの所有者と同じかどうかをチェックします。 異なる場合は、403アクセス拒否エラーが返されます。 これはデフォルトではオフになっています。', '共有ホスティングの場合、このチェックを有効にしてセキュリティを強化します。', 'ラジオボタンから選択', '');
...
$_tipsdb['geoipDBFile'] = new DAttrHelp("DBファイルのパス", 'MaxMind GeoIPデータベースへのパスを指定します。', '', '絶対パス', '/usr/local/share/GeoIP/GeoLite2-Country.mmdb');

$_tipsdb['geoipDBName'] = new DAttrHelp("データベース名", 'MaxMind GeoIPデータベース名です。GeoIP2以降、この設定は必須です。<br/><br/>GeoIPからGeoIP2へアップグレードする場合、この設定に&quot;COUNTRY_DB&quot;、&quot;CITY_DB&quot;、または&quot;ASN_DB&quot;を使用すると、 移行を容易にするため、PHPの$_SERVER変数にGeoIP互換エントリ（以下のDB名値別一覧）が自動的に設定されます。<br/><br/><b>CITY_DB:</b> &#039;GEOIP_COUNTRY_CODE&#039;, &#039;GEOIP_COUNTRY_NAME&#039;, &#039;GEOIP_CONTINENT_CODE&#039;, &#039;GEOIP_COUNTRY_CONTINENT&#039;, &#039;GEOIP_DMA_CODE&#039;, &#039;GEOIP_METRO_CODE&#039;, &#039;GEOIP_LATITUDE&#039;, &#039;GEOIP_LONGITUDE&#039;, &#039;GEOIP_POSTAL_CODE&#039;, and &#039;GEOIP_CITY&#039;.<br/><b>COUNTRY_DB:</b> &#039;GEOIP_COUNTRY_CODE&#039;, &#039;GEOIP_COUNTRY_NAME&#039;, &#039;GEOIP_CONTINENT_CODE&#039;, and &#039;GEOIP_COUNTRY_CONTINENT&#039;.<br/><b>ASN_DB:</b> &#039;GEOIP_ORGANIZATION&#039; and &#039;GEOIP_ISP&#039;.', '', '', 'COUNTRY_DB');

$_tipsdb['geolocationDB'] = new DAttrHelp("MaxMind GeoIP データベース", '複数のMaxMindジオロケーションデータベースをここで指定できます。MaxMindには、 Country、Region、City、Organization、ISP、NetspeedタイプのDBがあります。 &quot;Country&quot;、&quot;Region&quot;、&quot;City&quot;タイプの複数のデータベースが設定されている場合、最後の設定が有効になります。', '', '', '');

$_tipsdb['gracePeriod'] = new DAttrHelp("猶予期間（秒）", '1つのIPから確立された接続の数が&quot;接続ソフトリミット&quot;を超えた後に新しい接続が受け入れられる期間を指定します。 この期間内に、総接続数が&quot;接続ハードリミット&quot;未満の場合は、新しい接続が受け入れられます。 この期間が経過した後、まだ接続数が&quot;接続ソフトリミット&quot;よりも高い場合、問題のIPは&quot;禁止期間（秒）&quot;でブロックされます。', 'ダウンロードに十分な大きさに設定してください。 完全なページですが、故意の攻撃を防ぐのに十分な低さです。', '整数', '');

$_tipsdb['gracefulRestartTimeout'] = new DAttrHelp("グレースフル再起動タイムアウト(秒)", 'グレースフルリスタート時には、新しいサーバーインスタンスが起動した後でも、古いインスタンスは既存の要求を引き続き処理します。 このタイムアウトは、前のインスタンスが終了するまでの待機時間を定義します。 デフォルト値は300秒です。 -1は永遠に待つことを意味します。 0は待機しないことを意味し、直ちに中止します。', '', '整数', '');
...
$_tipsdb['groupDBMaxCacheSize'] = new DAttrHelp("グループDB最大キャッシュサイズ", 'グループデータベースの最大キャッシュサイズを指定します。', 'キャッシュが大きくなるとメモリが消費されるため、値が高くなるほどパフォーマンスが向上する場合があります。 ユーザーのデータベースサイズとサイトの使用状況に応じて適切なサイズに設定します。', '整数', '');

$_tipsdb['gzipAutoUpdateStatic'] = new DAttrHelp("静的ファイルの自動更新", '圧縮可能な静的ファイルについて、サーバーがGZIP圧縮版を自動的に作成/更新するかどうかを指定します。<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定されている場合、&quot;圧縮可能なタイプ&quot;にリストされたMIMEタイプのファイルが要求されると、サーバーは圧縮ファイルのタイムスタンプに応じて、そのファイルに対応する圧縮版を作成または更新できます。この圧縮ファイルは&quot;静的GZIPキャッシュディレクトリ&quot;の下に作成されます。ファイル名は元のファイルパスのMD5ハッシュに基づきます。<br/>デフォルト値：Yes', '', 'ラジオボックスから選択', '');

$_tipsdb['gzipCacheDir'] = new DAttrHelp("静的GZIPキャッシュディレクトリ", '静的コンテンツ用の圧縮ファイルを保存するために使用するディレクトリのパスを指定します。<br/>デフォルト値：&quot;スワップディレクトリ&quot;。', '', 'ディレクトリパス', '');

$_tipsdb['gzipCompressLevel'] = new DAttrHelp("GZIP圧縮レベル(動的コンテンツ)", '動的コンテンツに適用するGZIP圧縮レベルを指定します。範囲は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span>（最低）から<span class=&quot;lst-inline-token lst-inline-token--value&quot;>9</span>（最高）です。<br/>この設定は、GZIP圧縮と&quot;動的GZIP圧縮を有効化&quot;が有効な場合にのみ有効です。<br/>デフォルト値：6', ' 圧縮レベルを高くすると、より多くのメモリとCPUサイクルを使用します。マシンに余力がある場合は、より高いレベルに設定できます。6と9の間に大きな差はありませんが、9ではCPUサイクルをかなり多く使用します。', '1から9までの数値。', '');

$_tipsdb['gzipMaxFileSize'] = new DAttrHelp("最大静的ファイルサイズ(バイト)", 'サーバーが自動的に圧縮ファイルを作成する静的ファイルの最大サイズを指定します。<br/>デフォルト値：10M', ' 大きなファイルに対してサーバーが圧縮ファイルを作成/更新することは推奨されません。圧縮はサーバープロセス全体をブロックし、圧縮が完了するまで以降のリクエストを処理できません。', '1K以上のバイト数。', '');
...
$_tipsdb['gzipStaticCompressLevel'] = new DAttrHelp("GZIP圧縮レベル（静的ファイル）", '静的ファイルに適用するGZIP圧縮レベルを指定します。範囲は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span>（最低）から<span class=&quot;lst-inline-token lst-inline-token--value&quot;>9</span>（最高）です。<br/>この設定は、GZIP compressionと&quot;静的ファイルの自動更新&quot;が有効な場合にのみ有効です。<br/>デフォルト値：6', '', '1から9までの数値。', '');

$_tipsdb['hardLimit'] = new DAttrHelp("接続ハードリミット", '単一のIPアドレスから同時に許可される接続の最大数を指定します。 この制限は常に強制され、クライアントはこの制限を超えることはできません。 HTTP/1.0クライアントは通常、埋め込みコンテンツを同時にダウンロードするのに必要な数の接続を設定しようとします。 この制限は、HTTP/1.0クライアントが引き続きサイトにアクセスできるように十分に高く設定する必要があります。 &quot;接続ソフトリミット&quot;を使用して、目的の接続制限を設定します。', '数字が小さいほど、より明確なクライアントに対応できます。<br/>信頼できるIPまたはサブネットワークは影響を受けません。<br/>多数の同時クライアントマシンでベンチマークテストを実行する場合は、高い値に設定します。', '整数', '');

$_tipsdb['httpdWorkers'] = new DAttrHelp("ワーカー数", 'httpdワーカーの数を指定します。', 'ニーズに合わせて適切な番号を設定します。 より多くのワーカーを追加することは、必ずしもより良いパフォーマンスを意味するとは限りません。', '整数値は1〜16です。', '');

$_tipsdb['inBandwidth'] = new DAttrHelp("受信帯域幅（バイト/秒）", '確立された接続の数に関係なく、単一のIPアドレスからの最大許容着信スループット。 実際の帯域幅は効率上の理由からこの設定よりわずかに高くなることがあります。 帯域幅は1KB単位で割り当てられます。 スロットルを無効にするには、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>に設定します。 クライアント単位の帯域幅制限（バイト/秒）は、バーチャルホストレベルの設定がサーバーレベルの設定を上回るサーバーまたはバーチャルホストレベルで設定できます。', '信頼できるIPまたはサブネットワークは影響を受けません。', '整数', '');

$_tipsdb['inMemBufSize'] = new DAttrHelp("メモリI/Oバッファサイズ", '要求本体およびその動的生成応答を格納するために使用される最大バッファー・サイズを指定します。 この制限に達すると、サーバーは&quot;スワップディレクトリ&quot;の下に一時的なスワップファイルを作成し始めます。', 'メモリとディスクのスワップを避けるために、すべての同時要求/応答を収容できる大きさのバッファサイズを設定します。 スワップディレクトリに頻繁にI/Oアクティビティがある場合（デフォルトでは/tmp/lshttpd/swap/）、このバッファサイズは小さすぎるため、LiteSpeedはディスクにスワップします。', '整数', '');

$_tipsdb['indexFiles'] = new DAttrHelp("インデックスファイル", 'URLがディレクトリにマップされたときに順番に検索されるインデックスファイルの名前を指定します。 サーバー、バーチャルホスト、コンテキストレベルでカスタマイズできます。', '必要なインデックスファイルのみを設定します。', 'インデックスファイル名のカンマ区切りリスト。', '');

$_tipsdb['indexUseServer'] = new DAttrHelp("サーバーインデックスファイルを使用する", 'サーバーのインデックスファイル設定を使用するかどうかを指定します。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定すると、サーバーの設定だけが使用されます。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>に設定すると、サーバーの設定は使用されません。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>Addition</span>に設定すると、このバーチャルホストのサーバーインデックスファイルリストに追加のインデックスファイルを追加できます。 このバーチャルホストのインデックスファイルを無効にする場合は、値を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>に設定して、インデックスファイル欄を空のままにします。', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['instances'] = new DAttrHelp("インスタンス", 'サーバーが作成する外部アプリケーションの最大インスタンスを指定します。 &quot;自動起動&quot;が有効な場合は必須です。 ほとんどのFastCGI/LSAPIアプリケーションは、プロセスインスタンスごとに1つの要求しか処理できません。これらのタイプのアプリケーションの場合、インスタンスは&quot;最大接続数&quot;の値に一致するように設定する必要があります。 一部のFastCGI / LSAPIアプリケーションでは、複数の子プロセスを生成して複数の要求を同時に処理できます。 これらのタイプのアプリケーションでは、インスタンスを &quot;1&quot;に設定し、環境変数を使用してアプリケーションが生成できる子プロセスの数を制御する必要があります。', '', '整数', '');

$_tipsdb['internalmodule'] = new DAttrHelp("内部", 'モジュールが外部の.soライブラリではなく、静的にリンクされている内部モジュールであるかどうかを指定します。', '', 'ラジオボックスから選択', '');

$_tipsdb['ip2locDBCache'] = new DAttrHelp("データベースキャッシュ種別", '使用するキャッシュ方法です。デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Memory</span>です。', '', 'ドロップダウンリストから選択', '');

$_tipsdb['ip2locDBFile'] = new DAttrHelp("IP2Location DBファイルのパス", '有効なデータベースファイルの場所です。', '', 'ファイル名への絶対パス又は$SERVER_ROOTからの相対パス。', '');
...
$_tipsdb['javaWebApp_location'] = new DAttrHelp("場所", 'このWebアプリケーションのファイルを含むディレクトリを指定します。 これは &quot;WEB-INF/web.xml&quot;を含むディレクトリです。', '', 'パス', '');

$_tipsdb['jsonReports'] = new DAttrHelp("JSONレポートを出力", '.json拡張子を持つ追加のJSON形式レポートファイルを/tmp/lshttpdディレクトリへ出力します。<br/><br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>', ' これは、ほとんどのプログラミング言語に組み込まれている標準JSON処理ツールを使用して、 LiteSpeedステータスとリアルタイムレポートを自分のアプリケーションへ統合したいアプリケーション開発者に便利です。', 'ラジオボックスから選択', '<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定すると、通常の.status、.rtreport、.rtreport.2などのレポートファイルに加えて、 .status.json、.rtreport.json、.rtreport.2.jsonなどのレポートファイルが作成されます。');

$_tipsdb['keepAliveTimeout'] = new DAttrHelp("キープアライブタイムアウト（秒）", 'キープアライブ接続からの要求間の最大アイドル時間を指定します。 この期間中に新しい要求が受信されない場合、接続は閉じられます。 この設定は、HTTP/1.1接続にのみ適用されます。 HTTP/2接続は、設計によって長いキープアライブタイムアウトを持ち、この設定の影響を受けません。', 'ロードする必要がある単一のページで参照されるアセットが多くある場合、クライアントからの後続のリクエストを待機するのに十分な時間だけこの値を設定することをお勧めします。 キープアライブ接続で次のページが配信されることを期待して、これを長く設定しないでください。 多くのアイドル状態のキープアライブ接続を維持することはサーバーリソースの浪費であり、（D）DoS攻撃によって活用される可能性があります。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>2-5</span>秒はほとんどのアプリケーションにとって妥当な範囲です。 LiteSpeedは非キープアライブ環境で非常に効率的です。', '整数', '');

$_tipsdb['keyFile'] = new DAttrHelp("秘密鍵ファイル", 'SSL秘密鍵ファイルのファイル名。キーファイルは暗号化しないでください。', '秘密鍵ファイルは、サーバーが実行されるユーザーへの読み取り専用アクセスを可能にするセキュリティ保護されたディレクトリに配置する必要があります。', 'ファイル名への絶対パス又は$SERVER_ROOTからの相対パス', '');
...
$_tipsdb['logUseServer'] = new DAttrHelp("サーバーログを使用する", '独自のログファイルを作成するのではなく、このバーチャルホストからのログメッセージをサーバーログファイルに入れるかどうかを指定します。', '', 'ラジオボックスから選択', '');

$_tipsdb['log_compressArchive'] = new DAttrHelp("圧縮アーカイブ", 'ディスク領域を節約するためにローテーションしたログファイルを圧縮するかどうかを指定します。', 'ログファイルは圧縮率が高いため、古いログのディスク使用量を減らす目的で推奨されます。', 'ラジオボックスから選択', '');

$_tipsdb['log_debugLevel'] = new DAttrHelp("デバッグレベル", 'デバッグログのレベルを指定します。 この機能を使用するには、&quot;ログレベル&quot;を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>に設定する必要があります。 &quot;ログレベル&quot;が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>に設定されていても、「デバッグレベル」が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>に設定されていると、デバッグログは無効になります。 &quot;デバッグログ切替&quot;を使用すると、稼働中のサーバーで再起動せずにデバッグレベルを制御できます。', ' 重要！詳細なデバッグログが必要ない場合は、必ず<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>に設定してください。 有効なデバッグログはサービス性能を著しく低下させ、非常に短時間でディスク容量を使い切る可能性があります。 デバッグログには、各リクエストとレスポンスの詳細情報が含まれます。<br/>ログレベルを<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>に設定し、デバッグレベルを<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>に設定することをお勧めします。 この設定ではデバッグログでディスクを埋めることなく、&quot;デバッグログ切替&quot;アクションでデバッグ出力を制御できます。 このアクションはデバッグログを即座にオン/オフできるため、多忙な本番サーバーのデバッグに役立ちます。', 'ドロップダウンリストから選択', '');
...
$_tipsdb['log_fileName'] = new DAttrHelp("ファイル名", 'ログファイルのパスを指定します。', ' ログファイルは別のディスクに配置してください。', 'ファイル名への絶対パス又は$SERVER_ROOTからの相対パス。', '');
...
$_tipsdb['loginHistoryRetention'] = new DAttrHelp("ログイン履歴保持日数", '古いエントリが削除されるまでログイン履歴レコードを保持する日数を指定します。未設定時のデフォルト値:90日。', '監査とトラブルシューティングに十分な履歴を保持しつつ、実際に必要な量を超えるデータ保持は避けてください。', '整数', '');
...
$_tipsdb['lsrecaptcha'] = new DAttrHelp("LS reCAPTCHA", 'CAPTCHA保護は、高いサーバー負荷を緩和する手段として提供されるサービスです。CAPTCHA保護は、以下のいずれかの状況に達すると有効化されます。有効化されると、信頼されていない（設定に基づく）クライアントからのすべてのリクエストはCAPTCHA検証ページにリダイレクトされます。検証後、クライアントは目的のページにリダイレクトされます。<br/><br/>次の状況でCAPTCHA保護が有効化されます：<br/>1. サーバーまたはvhostの同時リクエスト数が、設定された接続数上限を超えた場合。<br/>2. Anti-DDoSが有効で、クライアントが疑わしい方法でURLにアクセスしている場合。トリガーされると、拒否される前にクライアントはまずCAPTCHAにリダイレクトされます。<br/>3. CAPTCHAをRewriteRulesで有効化するための新しいrewrite rule環境が提供されています。&#039;verifycaptcha&#039;を設定すると、クライアントをCAPTCHAにリダイレクトできます。特殊な値&#039;: deny&#039;を設定すると、失敗回数が多すぎるクライアントを拒否できます。たとえば、[E=verifycaptcha]は検証されるまで常にCAPTCHAにリダイレクトします。[E=verifycaptcha: deny]は最大試行回数に達するまでCAPTCHAにリダイレクトし、その後クライアントを拒否します。', '', '', '');

$_tipsdb['lstatus'] = new DAttrHelp("ステータス - リスナー", 'このリスナーの現在のステータス。 ステータスは<span class=&quot;lst-inline-token lst-inline-token--value&quot;>稼働中</span>か<span class=&quot;lst-inline-token lst-inline-token--value&quot;>エラー</span>のいずれかです。', 'リスナーが<span class=&quot;lst-inline-token lst-inline-token--value&quot;>エラー</span>状態にある場合は、サーバーログを表示して理由を調べることができます。', '', '');

$_tipsdb['mappedListeners'] = new DAttrHelp("マップされたリスナー", 'このテンプレートがマップするすべてのリスナー名を指定します。 このテンプレートのメンバーバーチャルホスト用のリスナーからバーチャルホストへのマッピングが、このフィールドで指定されたリスナーに追加されます。 このマッピングは、各メンバーバーチャルホストの個別設定にあるドメイン名とエイリアスに基づいて、リスナーをバーチャルホストにマップします。', '', 'カンマ区切りリスト', '');

$_tipsdb['maxCGIInstances'] = new DAttrHelp("最大CGIインスタンス", 'サーバーが開始できる同時CGIプロセスの最大数を指定します。 CGIスクリプトに対する各要求に対して、サーバーはスタンドアロンCGIプロセスを開始する必要があります。 Unixシステムでは、並行プロセスの数が制限されています。 過度の並行処理は、システム全体のパフォーマンスを低下させ、DoS攻撃を実行する1つの方法です。 LiteSpeedサーバーはCGIスクリプトへの要求をパイプライン処理し、同時のCGIプロセスを制限して最適なパフォーマンスと信頼性を確保します。 上限は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>2000</span>です。', '上限が高いと必ずしもパフォーマンスが向上するとは限りません。 ほとんどの場合、下限値を指定するとパフォーマンスとセキュリティが向上します。 上限は、CGI処理中にI / O待ち時間が過大になる場合にのみ役立ちます。', '整数', '');

$_tipsdb['maxCachedFileSize'] = new DAttrHelp("最大キャッシュ小ファイルサイズ(バイト)", '事前に割り当てられたメモリバッファにキャッシュされる最大の静的ファイルを指定します。 静的ファイルは、メモリバッファキャッシュ、メモリマップキャッシュ、プレーンリード/ライト、およびsendfile（）の4つの方法で提供できます。 サイズがこの設定より小さいファイルはメモリバッファキャッシュから提供されます。 サイズがこの設定より大きく、&quot;最大MMAPファイルサイズ（バイト）&quot;より小さいファイルは、メモリマップドキャッシュから処理されます。 &quot;最大MMAPファイルサイズ（バイト）&quot;より大きいサイズのファイルは、プレーン・リード/ライトまたはsendfile（）を介して処理されます。 メモリバッファキャッシュから4Kより小さい静的ファイルを提供することが最適です。', '', '整数', '');
...
$_tipsdb['maxConns'] = new DAttrHelp("最大接続数", 'サーバーと外部アプリケーションの間で確立できる同時接続の最大数を指定します。 この設定は、外部アプリケーションによって同時に処理できる要求の数を制御しますが、実際の制限は外部アプリケーション自体によっても異なります。 この値を高く設定すると、外部アプリケーションの速度が不十分であるか、多数の同時要求に対応できない場合に役立ちません。', '高い値を設定しても、高いパフォーマンスに直接変換されるわけではありません。 制限を外部アプリケーションに負荷をかけない値に設定すると、最高のパフォーマンス/スループットが得られます。', '整数', '');

$_tipsdb['maxDynRespHeaderSize'] = new DAttrHelp("最大動的レスポンスヘッダーサイズ(バイト)", '動的に生成されるレスポンスの最大ヘッダーサイズを指定します。ハードリミットは<span class=&quot;lst-inline-token lst-inline-token--value&quot;>131072</span>バイト、つまり128Kです。<br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32768</span>または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32K</span>', ' 外部アプリケーションによって動的に生成される不正なレスポンスを認識しやすくするため、適度に低く設定してください。', '整数', '');

$_tipsdb['maxDynRespSize'] = new DAttrHelp("最大動的レスポンスボディサイズ(バイト)", '動的に生成されるレスポンスの最大ボディサイズを指定します。', ' 不正なレスポンスを識別しやすくするため、制限を適度に低く設定してください。不正なスクリプトに無限ループが含まれ、無限大のレスポンスにつながることは珍しくありません。', '整数', '');

$_tipsdb['maxKeepAliveReq'] = new DAttrHelp("最大Keep-Aliveリクエスト数", 'キープアライブ（永続的）セッションを介して処理できる要求の最大数を指定します。 この制限に達すると接続は終了します。 バーチャルホストごとにこの制限を設定することもできます。', '適度に高い値に設定します。 &quot;1&quot;または &quot;0&quot;の値はキープアライブを無効にします。', '整数', '');

$_tipsdb['maxMMapFileSize'] = new DAttrHelp("最大MMAPファイルサイズ（バイト）", 'メモリマップされる最大の静的ファイル（MMAP）を指定します。   静的ファイルは、メモリバッファキャッシュ、メモリマップキャッシュ、プレーンリード/ライト、およびsendfile（）の4つの方法で提供できます。 サイズが&quot;最大キャッシュ小ファイルサイズ(バイト)&quot;より小さいファイルはメモリバッファキャッシュから提供されます。 サイズが&quot;最大キャッシュ小ファイルサイズ(バイト)&quot;よりも大きいが最大MAPファイルサイズより小さいファイルは、メモリマップドキャッシュから提供されます。 最大MMAPファイルサイズよりも大きいファイルは、プレーンな読み取り/書き込みまたはsendfile（）を介して提供されます。 サーバは32ビットのアドレス空間（2GB）を持っているので、非常に大きなファイルをメモリに格納することは推奨されません。', '', '整数', '');
...
$_tipsdb['maxReqBodySize'] = new DAttrHelp("最大リクエストボディサイズ（バイト）", 'HTTPリクエスト本文の最大サイズを指定します。 32ビットOSの場合、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>2GB</span>はハード制限です。 64ビットOSの場合、実質的に無制限です。', ' DoS攻撃を防ぐには、この制限を実際に必要なものだけに制限してください。 スワッピング空間には、この制限に対応するための十分な空き領域が必要です。', '整数', '');

$_tipsdb['maxReqHeaderSize'] = new DAttrHelp("最大リクエストヘッダーサイズ(バイト)", 'リクエストURLを含むHTTPリクエストヘッダーの最大サイズを指定します。ハードリミットは<span class=&quot;lst-inline-token lst-inline-token--value&quot;>131072</span>バイト、つまり128Kです。<br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32768</span>または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32K</span>', ' メモリ使用量を減らし、不正なリクエストやDoS攻撃を識別しやすくするため、適度に低く設定してください。<br/>通常の状況では、ほとんどのWebサイトで4-8Kで十分です。', '整数', '');

$_tipsdb['maxReqURLLen'] = new DAttrHelp("最大リクエストURLの長さ(バイト)", 'リクエストURLの最大サイズを指定します。URLは、クエリ文字列を含む、サーバーリソースへのアクセスに使用される完全なテキストアドレスです。ハードリミットは<span class=&quot;lst-inline-token lst-inline-token--value&quot;>65530</span>バイトです。<span class=&quot;lst-inline-token lst-inline-token--value&quot;>64K</span>（6バイト超過）など、これより大きい値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>65530</span>が使用されたものとして扱われます。<br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>8192</span>または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>8K</span>。', ' メモリ使用量を減らし、不正なリクエストやDoS攻撃を識別しやすくするため、適度に低く設定してください。<br/>POSTの代わりにHTTP GETメソッドで大きなクエリ文字列を使用しない限り、ほとんどのWebサイトでは2-3Kで十分です。', '整数', '');
...
$_tipsdb['memHardLimit'] = new DAttrHelp("メモリハードリミット(バイト)", 'ソフトリミットをユーザープロセス内のハードリミットまで上げることができることを除けば、&quot;メモリソフトリミット（バイト）&quot;と同じくらい同じです。 ハード・リミットは、サーバー・レベルまたは個々の外部アプリケーション・レベルで設定できます。 サーバーレベルの制限は、個々のアプリケーションレベルで設定されていない場合に使用されます。<br/><br/>値が両方のレベルにないか、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>に設定されている場合、オペレーティングシステムのデフォルトが使用されます。', 'この制限を過度に調整しないでください。アプリケーションがより多くのメモリを必要とする場合、503エラーが発生する可能性があります。', '整数', '');

$_tipsdb['memSoftLimit'] = new DAttrHelp("メモリソフトリミット（バイト）", '外部アプリケーション・プロセスまたはサーバーによって開始された外部アプリケーションのメモリー消費制限をバイト単位で指定します。<br/><br/>この制限の主な目的は、ソフトウェアのバグや意図的な攻撃のために過度のメモリ使用を防止し、通常の使用に制限を設けないことです。 十分なヘッドスペースを確保してください。そうしないと、アプリケーションが失敗し、503エラーが返される可能性があります。 サーバーレベルまたは個々の外部アプリケーションレベルで設定できます。 サーバーレベルの制限は、個々のアプリケーションレベルで設定されていない場合に使用されます。<br/><br/>オペレーティングシステムのデフォルト設定は、値が両方のレベルにないか、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>に設定されている場合に使用されます。', 'この制限を過度に調整しないでください。 アプリケーションでより多くのメモリが必要な場合は、503のエラーが発生する可能性があります。', '整数', '');
...
$_tipsdb['minGID'] = new DAttrHelp("最小GID", '外部アプリケーションの最小グループIDを指定します。 ここで指定した値よりも小さいグループIDを持つ外部の実行は拒否されます。 LiteSpeed Webサーバーが &quot;root&quot;ユーザーによって起動された場合、Apacheで見つかった &quot;suEXEC&quot;モードで外部アプリケーションを実行できます（Webサーバー以外のユーザー/グループIDに変更する）。', 'システムユーザーが使用するすべてのグループを除外するのに十分な高さに設定します。', '整数', '');

$_tipsdb['minUID'] = new DAttrHelp("最小UID", '外部アプリケーションの最小ユーザーIDを指定します。 ここで指定した値よりも小さいユーザーIDを持つ外部スクリプトの実行は拒否されます。 LiteSpeed Webサーバーが「root」ユーザーによって起動された場合、Apacheなどの「suEXEC」モードで外部アプリケーションを実行できます（Webサーバー以外のユーザー/グループIDに変更する）。', 'すべてのシステム/特権ユーザーを除外するのに十分な高さに設定します。', '整数', '');
...
$_tipsdb['moduleEnabled'] = new DAttrHelp("モジュールを有効化", 'モジュールフックをグローバルに有効または無効にします。<br/>「未設定」ラジオボタンが選択され、モジュールにフック機能が含まれている場合、デフォルトが有効になります。 ユーザーは、各レベルでこのグローバル設定を上書きできます。', '', 'ラジオボックスから選択', '');

$_tipsdb['moduleEnabled_lst'] = new DAttrHelp("モジュールを有効化", 'リスナーレベルでモジュールフックを有効または無効にします。 モジュールにTCP/IPレベルのフック（L4_BEGSESSION、L4_ENDSESSION、L4_RECVING、L4_SENDING）がある場合のみ、この設定が有効になります。<br/><br/>「未設定」ラジオボタンが選択されている場合、デフォルトはサーバー設定から継承されます。 ユーザーはデフォルト設定を上書きするためにここで設定するだけです。', '', 'ラジオボックスから選択', '');

$_tipsdb['moduleEnabled_vh'] = new DAttrHelp("モジュールを有効化", 'バーチャルホストまたはコンテキスト・レベルでモジュール・フックを使用可能または使用不可にします。 モジュールにHTTPレベルのフックがある場合のみ、この設定が有効になります。<br/><br/>「未設定」ラジオボタンを選択すると、バーチャルホストレベルのデフォルト設定がサーバー設定から継承されます。 コンテキストレベルの設定は、バーチャルホストレベルから継承されます。 ユーザーはデフォルト設定を上書きするためにここで設定するだけです。', '', 'ラジオボックスから選択', '');

$_tipsdb['moduleNameSel'] = new DAttrHelp("モジュール", 'モジュールの名前。 モジュールはサーバーモジュールの設定タブに登録する必要があります。 登録されると、モジュール名がリスナーおよびバーチャルホスト構成のドロップダウンボックスで使用可能になります。', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['namespace'] = new DAttrHelp("Namespaceコンテナ", 'CGIプロセス（PHPプログラムを含む）をNamespaceコンテナサンドボックス内で起動する場合は、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>有効</span>に設定します。&quot;Bubblewrapコンテナ&quot;が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>無効</span>に設定されている場合にのみ使用されます。<br/><br/>サーバーレベルで<span class=&quot;lst-inline-token lst-inline-token--value&quot;>無効</span>以外に設定されている場合、この設定値はバーチャルホストレベルで上書きできます。<br/><br/>デフォルト値：<br/><b>サーバーレベル：</b><span class=&quot;lst-inline-token lst-inline-token--value&quot;>無効</span><br/><b>バーチャルホストレベル：</b>サーバーレベル設定を継承', '', 'ドロップダウンリストから選択', '');

$_tipsdb['namespaceConf'] = new DAttrHelp("Namespaceテンプレートファイル", 'マウントするディレクトリのリストと、それらをマウントする方法を含む既存の設定ファイルへのパスです。&quot;Namespaceコンテナ&quot;が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>有効</span>に設定され、この値が未設定の場合、次の安全なデフォルト設定が使用されます。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;> $HOMEDIR/.lsns/tmp /tmp,tmp<br/>/usr,ro-bind<br/>/lib,ro-bind<br/>/lib64,ro-bind-try<br/>/bin,ro-bind<br/>/sbin,ro-bind<br/>/var,dir<br/>/var/www,ro-bind-try<br/>/proc,proc<br/>../tmp var/tmp,symlink<br/>/dev,dev<br/>/etc/localtime,ro-bind-try<br/>/etc/ld.so.cache,ro-bind-try<br/>/etc/resolv.conf,ro-bind-try<br/>/etc/ssl,ro-bind-try<br/>/etc/pki,ro-bind-try<br/>/etc/man_db.conf,ro-bind-try<br/>/usr/local/bin/msmtp /etc/alternatives/mta,ro-bind-try<br/>/usr/local/bin/msmtp /usr/sbin/exim,ro-bind-try<br/>$HOMEDIR,bind-try<br/>/var/lib/mysql/mysql.sock,bind-try<br/>/home/mysql/mysql.sock,bind-try<br/>/tmp/mysql.sock,bind-try<br/>/run/mysqld/mysqld.sock,bind-try<br/>/var/run/mysqld.sock,bind-try<br/>/run/user/$UID,bind-try<br/>$PASSWD<br/>$GROUP<br/>/etc/exim.jail/$USER.conf $HOMEDIR/.msmtprc,copy-try<br/>/etc/php.ini,ro-bind-try<br/>/etc/php-fpm.conf,ro-bind-try<br/>/etc/php-fpm.d,ro-bind-try<br/>/var/run,ro-bind-try<br/>/var/lib,ro-bind-try<br/>/etc/imunify360/user_config/,ro-bind-try<br/>/etc/sysconfig/imunify360,ro-bind-try<br/>/opt/plesk/php,ro-bind-try<br/>/opt/alt,bind-try<br/>/opt/cpanel,bind-try<br/>/opt/psa,bind-try<br/>/var/lib/php/sessions,bind-try </span>', '', '絶対パス又は$SERVER_ROOTからの相対パス。', '');
...
$_tipsdb['nodeBin'] = new DAttrHelp("ノードパス", 'Node.js実行ファイルへのパスです。', '', '絶対パス', '');

$_tipsdb['nodeDefaults'] = new DAttrHelp("Node.jsアプリのデフォルト設定", 'Node.jsアプリケーションのデフォルト設定です。これらの設定はコンテキストレベルで上書きできます。', '', '', '');

$_tipsdb['note'] = new DAttrHelp("ノート", '自分用のメモを追加します。', '', '', '');
...
$_tipsdb['ocspRespMaxAge'] = new DAttrHelp("OCSPレスポンス最大有効期間(秒)", 'このオプションは、OCSP応答の許容可能な最大経過時間を設定します。 OCSP応答がこの最大年齢より古い場合、サーバーはOCSP応答者に新しい応答を要求します。 デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>86400</span>です。 この値を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>-1</span>に設定すると、最大年齢を無効にすることができます。', '', '秒数', '');
...
$_tipsdb['opsAuditRetainFiles'] = new DAttrHelp("アクティビティログ保持ファイル数", 'WebAdminコンソールで保持する操作監査ファイルの最大数を指定します。アクティビティログファイルは5 MBに達したとき、または1日1回ローテーションされます。未設定時のデフォルト値:30ファイル。', 'より長い監査証跡が必要な場合はこの値を増やしてください。この保持設定はファイル数ベースのため、アクティビティ量が多い場合は保持される日数が短くなる可能性があります。ファイル数が増えるほどディスク容量を使用します。', '整数', '');

$_tipsdb['outBandwidth'] = new DAttrHelp("送信帯域幅（バイト/秒）", '確立された接続の数に関係なく、単一のIPアドレスへの最大の送信スループット。 実際の帯域幅は効率上の理由からこの設定よりわずかに高くなることがあります。 帯域幅は4KB単位で割り当てられます。 スロットルを無効にするには、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>に設定します。 クライアント単位の帯域幅制限（バイト/秒）は、バーチャルホストレベルの設定がサーバーレベルの設定を上回るサーバーまたはバーチャルホストレベルで設定できます。', 'パフォーマンスを向上させるため、帯域幅を8KB単位で設定します。.<br/><br/>信頼できるIPまたはサブネットワークは影響を受けません。', '整数', '');

$_tipsdb['pcKeepAliveTimeout'] = new DAttrHelp("接続のKeep-Aliveタイムアウト", 'アイドル状態の永続接続を開いたままにする最大秒数を指定します。<br/><br/>&quot;-1&quot;に設定すると、接続はタイムアウトしません。0以上に設定すると、この秒数が経過した後に接続が閉じられます。', '', '整数', '');

$_tipsdb['perClientConnLimit'] = new DAttrHelp("クライアントごとの帯域制限", 'これらは、クライアントIPに基づいた接続制御設定です。 これらの設定は、DoS（サービス拒否）攻撃とDDoS（分散サービス拒否）攻撃を緩和するのに役立ちます。', '', '', '');

$_tipsdb['persistConn'] = new DAttrHelp("永続的な接続", 'リクエスト処理後に接続を開いたままにするかどうかを指定します。 永続接続はパフォーマンスを向上させる可能性がありますが、一部のFastCGI外部アプリケーションは永続接続を完全にはサポートしません。 デフォルトは&quot;On&quot;です。', '', 'ラジオボックスから選択', '');
...
$_tipsdb['quicEnable'] = new DAttrHelp("HTTP3/QUICを有効にする", 'サーバー全体でHTTP3/QUICネットワークプロトコルを有効にします。デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>です。', 'この設定が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定されている場合でも、リスナーレベルの&quot;HTTP3/QUIC（UDP）ポートを開く&quot;設定、またはバーチャルホストレベルの&quot;HTTP3/QUICを有効にする&quot;設定でHTTP3/QUICを無効にできます。', 'ラジオボックスから選択', '');
...
$_tipsdb['quicMaxCfcw'] = new DAttrHelp("接続フロー制御ウィンドウ最大値", 'ウィンドウ自動調整によって接続フロー制御ウィンドウバッファが到達できる最大サイズを指定します。<br/>デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>です。これは&quot;接続フロー制御ウィンドウ&quot;の値が使用され、自動調整が行われないことを意味します。', 'ウィンドウサイズを大きくすると、より多くのメモリを使用します。', '0、または64Kから512Mまでの数値', '');
...
$_tipsdb['quicMaxSfcw'] = new DAttrHelp("ストリームフロー制御ウィンドウ最大値", 'ウィンドウ自動調整によってストリームフロー制御ウィンドウが到達できる最大サイズを指定します。<br/>デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>です。これは&quot;ストリームフロー制御ウィンドウ&quot;の値が使用され、自動調整が行われないことを意味します。', 'ウィンドウサイズを大きくすると、より多くのメモリを使用します。', '0、または64Kから128Mまでの数値', '');

$_tipsdb['quicMaxStreams'] = new DAttrHelp("接続ごとの最大同時ストリーム数", 'QUIC接続ごとに許可される同時ストリームの最大数です。デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>100</span>です。', '', '10から1000までの整数', '');
...
$_tipsdb['rcvBufSize'] = new DAttrHelp("受信バッファサイズ（バイト）", '各TCPソケットの受信バッファーサイズ。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>512K</span>は許容されるバッファの最大サイズです。', 'オペレーティングシステムのデフォルトのバッファサイズを使用するには、この値を &quot;未設定&quot;のままにするか、0に設定することをお勧めします。<br/>大きい受信バッファは、大きなペイロード、すなわちファイルアップロードで着信要求を処理するときのパフォーマンスを向上させます。<br/>これを低い値に設定すると、ソケットあたりのスループットとメモリ使用量が減少し、メモリがボトルネックになった場合にサーバーがより多くの同時ソケットを持つことが可能になります。', '整数', '');
...
$_tipsdb['recaptchaAllowedRobotHits'] = new DAttrHelp("許可されるボットアクセス数", '「good bots」の通過を許可する10秒あたりのヒット数です。サーバーが高負荷状態の場合、ボットは引き続きスロットルされます。<br/><br/>デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>3</span>です。', '', '整数', '');
...
$_tipsdb['recaptchaType'] = new DAttrHelp("CAPTCHAの種類", 'キーペアで使用するCAPTCHAの種類を指定します。<br/>キーペアが指定されておらず、この設定が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>未設定</span>に設定されている場合、CAPTCHA種類<span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Invisible</span>のデフォルトキーペアが使用されます。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Checkbox</span>は、訪問者が検証するためのチェックボックスCAPTCHAを表示します。 （&quot;サイトキー&quot;、&quot;シークレットキー&quot;が必要）<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Invisible</span>はCAPTCHAを自動的に検証しようとし、成功した場合は目的のページにリダイレクトします。 （&quot;サイトキー&quot;、&quot;シークレットキー&quot;が必要）<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>hCaptcha</span>は、CAPTCHAプロバイダー <a href="https://www.hcaptcha.com" target="_blank" rel="noopener noreferrer">hCaptcha</a>をサポートするために使用できます。 （&quot;サイトキー&quot;、&quot;シークレットキー&quot;が必要）<br/><br/>デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Invisible</span>です。', '', 'ドロップダウンリストから選択', '');

$_tipsdb['recaptchaVhReqLimit'] = new DAttrHelp("同時リクエスト数上限", 'CAPTCHAを有効化するために必要な同時リクエスト数です。同時リクエスト数がこの数値を下回るまで、CAPTCHAは引き続き使用されます。<br/><br/>デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>15000</span>です。', '', '整数', '');
...
$_tipsdb['respBuffer'] = new DAttrHelp("応答バッファリング", '外部アプリケーションから受信したレスポンスをバッファリングするかどうかを指定します。 &quot;nph-&quot;（Non-Parsed-Header）スクリプトが検出された場合、完全なHTTPヘッダーを持つレスポンスではバッファリングがオフになります。', '', 'ドロップダウンリストから選択', '');

$_tipsdb['restart'] = new DAttrHelp("変更を適用する/緩やかなリスタート", '<span class=&quot;lst-inline-token lst-inline-token--value&quot;>緩やかな再起動</span>をクリックすると、新しいサーバープロセスが開始されます。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>緩やかな再起動</span>の場合、古いサーバープロセスは、すべてのリクエストが完了した後（または&quot;グレースフル再起動タイムアウト(秒)&quot;に達すると）終了します。<br/><br/>設定の変更は、次回の再起動時に適用されます。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>緩やかな再起動</span>は、サーバーのダウンタイムなしにこれらの変更を適用します。', '緩やかな再起動では、通常2秒未満で新しいサーバープロセスが生成されます。', '', '');

$_tipsdb['restrained'] = new DAttrHelp("制限付き", 'このバーチャルホストのルート（$VH_ROOT）を超えるファイルにこのWebサイトからアクセスできるかどうかを指定します。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定すると、$VH_ROOT以下のファイルにしかアクセスできず、$VH_ROOTを超えるファイルやディレクトリを指すシンボリックリンクまたはContextへのアクセスは拒否されます。 しかし、これはCGIスクリプトのアクセシビリティを制限しません。 これは共有ホスティング環境で便利です。 &quot;シンボリックリンクをたどる&quot;は、ユーザーが$VH_ROOT内でシンボリックリンクを使用できるように<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定できます。 $VH_ROOTを超えるものはありません。', '共有ホスティング環境で有効にします。', 'ラジオボックスから選択', '');
...
$_tipsdb['rewriteBase'] = new DAttrHelp("Rewrite ベース", '書き換えルールのベースURLを指定します。', '', 'URL', '');

$_tipsdb['rewriteInherit'] = new DAttrHelp("Rewrite 継承", '親コンテキストから書き換えルールを継承するかどうかを指定します。 書き換えが有効で継承されていない場合は、このコンテキストで定義されている書き換えのベースルールと書き換えルールが使用されます。', '', 'ラジオボタンから選択', '');
...
$_tipsdb['rubyBin'] = new DAttrHelp("Rubyのパス", 'Ruby実行可能ファイルへのパスです。 通常はRubyのインストール先に応じて、/usr/bin/rubyまたは/usr/local/bin/rubyになります。', '', '絶対パス。', '');

$_tipsdb['runOnStartUp'] = new DAttrHelp("起動時に実行", 'サーバー起動時に外部アプリケーションを起動するかどうかを指定します。 自身の子プロセスを管理でき、&quot;インスタンス&quot;の値が&quot;1&quot;に設定されている外部アプリケーションにのみ適用されます。<br/><br/>有効にすると、外部プロセスは実行時ではなくサーバー起動時に作成されます。<br/><br/>&quot;Yes (Detached mode)&quot;を選択した場合、デタッチされたすべてのプロセスは、サーバーレベルでは$SERVER_ROOT/admin/tmp/配下、 バーチャルホストレベルでは$VH_ROOT/配下の&#039;.lsphp_restart.txt&#039;ファイルにtouchすることで再起動できます。<br/><br/>デフォルト値：Yes (Detached mode)', ' Railsアプリのように、設定された外部プロセスの起動オーバーヘッドが大きい場合は、 このオプションを有効にすると最初のページ応答時間を短縮できます。', 'ラジオボックスから選択', '');

$_tipsdb['runningAs'] = new DAttrHelp("実行ユーザ・グループ", 'サーバープロセスが実行されるユーザー/グループを指定します。 これは、インストールの前にconfigureコマンドを実行するときに、 &quot;--with-user&quot;と &quot;--with-group&quot;というパラメータを使用して設定されます。これらの値をリセットするには、configureコマンドを再実行して再インストールする必要があります。', 'サーバーは、&quot;root&quot;のような特権ユーザーとして実行しないでください。 サーバーが、ログイン/シェルアクセスを持たない特権のないユーザー/グループの組み合わせで実行するように構成されていることが重要です。 一般的に<span class=&quot;lst-inline-token lst-inline-token--value&quot;>nobody</span>のユーザー/グループが良い選択です。', '', '');
...
$_tipsdb['servAction'] = new DAttrHelp("操作", 'このメニューからは、次の6つのアクションを使用できます：  <span class=&quot;lst-inline-token lst-inline-token--value&quot;>緩やかな再起動</span>, <span class=&quot;lst-inline-token lst-inline-token--value&quot;>デバッグログの切り替え</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>サーバーログビューア</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>リアルタイム統計</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>バージョンマネージャ</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>PHPをコンパイル</span>です。 <ul><li>&quot;変更を適用する/緩やかなリスタート&quot;は、処理中のリクエストを中断することなく、サーバープロセスを平滑に再起動します。</li> 	<li>&quot;デバッグログ切替&quot;は、デバッグログのオン/オフを切り替えます。</li> 	<li>&quot;サーバーログビューア&quot;は、ログビューアを介してサーバーログを表示できます。</li> 	<li>&quot;リアルタイム統計&quot;は、リアルタイムのサーバステータスを表示できます。</li> 	<li>&quot;バージョン管理&quot;は、LSWSの新しいバージョンをダウンロードし、異なるバージョン間で切り替えることができます。</li> 	<li>PHPをコンパイルすると、PHPをLiteSpeed Web Server用にコンパイルできます。</li> </ul>', 'シェルユーティリティ<span class=&quot;lst-inline-token lst-inline-token--command&quot;>$SERVER_ROOT/bin/lswsctrl</span>を使用してサーバープロセスを制御することもできますが、ログインシェルが必要です。', '', '');
...
$_tipsdb['serverPriority'] = new DAttrHelp("優先度", 'サーバープロセスの優先度を指定します。値の範囲は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>-20</span>〜<span class=&quot;lst-inline-token lst-inline-token--value&quot;>20</span>です。 数値が小さいほど優先度が高くなります。', '通常、ビジー状態のサーバーで優先度を高くすると、Webパフォーマンスが若干高くなります。 データベースプロセスの優先度よりも高い優先度を設定しないでください。', '整数', '');
...
$_tipsdb['setUidMode'] = new DAttrHelp("外部アプリのSet UIDモード", '外部アプリケーションプロセスにユーザーIDを設定する方法を指定します。3つの選択肢があります： <ul><li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Server UID</span>：外部アプリケーションプロセスのユーザー/グループIDをサーバーのユーザー/グループIDと同じに設定します。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI File UID</span>：実行ファイルのユーザー/グループIDに基づいて外部CGIプロセスのユーザー/グループIDを設定します。このオプションはCGIにのみ適用され、FastCGIやLSPHPには適用されません。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Doc Root UID</span>：現在のバーチャルホストのドキュメントルートのユーザー/グループIDに基づいて、外部アプリケーションプロセスのユーザー/グループIDを設定します。</li> </ul><br/><br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Server UID</span>', ' 共有ホスティング環境では、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI File UID</span>または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Doc Root UID</span>モードを使用し、あるバーチャルホストが所有するファイルに別のバーチャルホストの外部アプリケーションスクリプトからアクセスされるのを防ぐことを推奨します。', 'ドロップダウンリストから選択', '');
...
$_tipsdb['shType'] = new DAttrHelp("ハンドラタイプ", 'これらのスクリプトファイルを処理する外部アプリケーションのタイプを指定します。使用可能なタイプは次のとおりです: <span class=&quot;lst-inline-token lst-inline-token--value&quot;>LSAPI app</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Webサーバー</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Fast CGI</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>SCGI</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Load balancer</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>サーブレットエンジン</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>uWSGI</span>、または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>モジュールハンドラ</span>。<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI</span>および<span class=&quot;lst-inline-token lst-inline-token--value&quot;>モジュールハンドラ</span>ハンドラタイプを除き、&quot;ハンドラ名&quot;も &quot;外部アプリケーション&quot;セクションで事前に定義された外部アプリケーションに設定する必要があります。', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['sslSessionCacheTimeout'] = new DAttrHelp("セッションキャッシュのタイムアウト(秒)", 'この値は、再ネゴシエーションが必要な前にセッションIDがキャッシュ内で有効である期間を決定します。 デフォルトは3,600です。', '', '整数', '');
...
$_tipsdb['sslStrictSni'] = new DAttrHelp("厳格なSNI証明書", '専用のバーチャルホスト証明書設定を厳格に要求するかどうかを指定します。有効にすると、専用の証明書設定がないバーチャルホストへのSSL接続は、デフォルトの包括証明書を使用する代わりに失敗します。<br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>', '', 'ラジオボックスから選択', '');
...
$_tipsdb['staticReqPerSec'] = new DAttrHelp("静的リクエスト/秒", '確立された接続の数に関係なく、1秒間に処理できる単一のIPアドレスからの静的コンテンツへの要求の最大数を指定します。<br/><br/>この制限に達すると、将来のすべての要求は次の秒までタールピットされます。 動的に生成されるコンテンツのリクエスト制限は、この制限とは関係ありません。 クライアントごとの要求制限は、サーバーまたはバーチャルホストレベルで設定できます。 バーチャルホストレベルの設定は、サーバーレベルの設定よりも優先されます。', '信頼できるIPまたはサブネットワークは影響を受けません。', '整数', '');
...
$_tipsdb['swappingDir'] = new DAttrHelp("スワップディレクトリ", 'スワッピングファイルを配置するディレクトリを指定します。サーバーがchrootモードで起動されている場合、 このディレクトリは新しいルートディレクトリからの相対パスになります。それ以外の場合は、実際のルートディレクトリからの相対パスになります。<br/><br/>サーバーは独自のバーチャルメモリを使用してシステムメモリ使用量を削減します。 バーチャルメモリとディスクスワッピングは、大きなリクエストボディと動的に生成されたレスポンスを保存するために使用されます。 スワッピングディレクトリは十分な空き容量があるディスク上に配置してください。<br/><br/>デフォルト値：/tmp/lshttpd/swap', ' スワッピングをなくすには、スワッピングディレクトリを別のディスクに配置するか、最大I/Oバッファサイズを増やします。', '絶対パス', '');
...
$_tipsdb['templateVHRoot'] = new DAttrHelp("デフォルトバーチャルホストルート", 'このテンプレートを使用して、メンバーバーチャルホストのデフォルトルートディレクトリを指定します。 変数<span class=&quot;lst-inline-token lst-inline-token--value&quot;>$VH_NAME</span>はパスに指定する必要があります。 これにより、各メンバーテンプレートに名前に基づいて別々のルートディレクトリが自動的に割り当てられます。', '', 'パス', '');

$_tipsdb['throttleBlockWindow'] = new DAttrHelp("初回ブロック時間(秒)", '許可された最大ログイン失敗回数に達した後、クライアントをブロックする初期時間を秒単位で指定します。未設定時のデフォルト値:900秒。', '正当なユーザーに過度なロックアウト時間を発生させずに、繰り返し攻撃を抑止できる十分な長さを使用してください。', '整数', '');

$_tipsdb['throttleEnabled'] = new DAttrHelp("ログインスロットルを有効にする", 'WebAdminコンソールのログインスロットルを有効にします。有効にすると、失敗したログイン試行の繰り返しが追跡され、ブルートフォースパスワード攻撃を減らすために一時的にブロックされます。このオプションのみを有効にし、他のスロットル設定を未設定のままにした場合は、組み込みのデフォルト値が使用されます。', ' ログイン問題のトラブルシューティング中でない限り、本番環境では有効のままにしてください。カスタム値なしで有効にした場合、デフォルトは5回の失敗、900秒の初期ブロック、14400秒の最大ブロックです。', 'ラジオボックスから選択', '');

$_tipsdb['throttleMaxBackoff'] = new DAttrHelp("最大ブロック時間(秒)", 'ログイン失敗の繰り返しが続き、スロットルのバックオフが増加した場合の最大ブロック期間を秒単位で指定します。未設定時のデフォルト値:14400秒。', '自動化された攻撃を十分に遅らせつつ、妥当な時間内に復旧できる上限を設定してください。', '整数', '');

$_tipsdb['throttleMaxFailures'] = new DAttrHelp("最大ログイン失敗回数", 'クライアントがブロックされる前に許可される連続ログイン失敗回数を指定します。未設定時のデフォルト値:5。', ' 値を低くすると保護は強くなりますが、正当なユーザーがより早くブロックされる可能性があります。', '整数', '');

$_tipsdb['toggleDebugLog'] = new DAttrHelp("デバッグログ切替", 'デバッグログの切り替えは、&quot;デバッグレベル&quot;の値を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>と<span class=&quot;lst-inline-token lst-inline-token--value&quot;>HIGH</span>の間で切り替えます。 デバッグログはパフォーマンスに影響し、ハードドライブをすぐに埋める可能性があるため、通常は本番サーバーで&quot;デバッグレベル&quot;を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>に設定する必要があります。 この機能を代わりに使用して、本番サーバー上の問題をデバッグするために、デバッグログを素早くオン/オフすることができます。 このようにデバッグロギングをオンまたはオフにしても、サーバー構成に表示されるものは変更されません。', '&quot;デバッグログ切替&quot;は、&quot;ログレベル&quot;が<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>に設定されている場合にのみ機能します。<br/> 重要！デバッグログには、各リクエストとレスポンスの詳細情報が含まれます。 有効なデバッグログはサービス性能を著しく低下させ、非常に短時間でディスク容量を使い切る可能性があります。 この機能は、サーバー問題を診断する短時間だけ使用してください。', '', '');

$_tipsdb['totalInMemCacheSize'] = new DAttrHelp("小ファイルキャッシュサイズの合計(バイト)", '小さな静的ファイルをキャッシュ/提供するためにバッファーキャッシュに割り振ることができる合計メモリーを指定します。', '', '整数', '');

$_tipsdb['totalMMapCacheSize'] = new DAttrHelp("MMAPキャッシュサイズの合計(バイト)", '中規模の静的ファイルをキャッシュ/配信するためにメモリマップされたキャッシュに割り当てることができる合計メモリを指定します。', '', '整数', '');
...
$_tipsdb['uploadPassByPath'] = new DAttrHelp("ファイルパスでアップロードデータを渡す", 'ファイルデータをパスでアップロードするかどうかを指定します。 有効にすると、アップロード時にファイル自体ではなく、バックエンドハンドラにファイルパスと他の情報が送信されます。 これにより、CPUリソースとファイル転送時間が節約されますが、実装するためにバックエンドに若干の更新が必要です。 無効にすると、ファイルの内容はバックエンドハンドラに転送され、要求本文は引き続きファイルに解析されます。', '下位互換性が問題にならないようにするには、これを有効にしてファイルのアップロード処理を高速化します。', 'ラジオボタンから選択', '');

$_tipsdb['uploadTmpDir'] = new DAttrHelp("一時ファイルのパス", '要求本体パーサーが動作している間に、サーバーにアップロードされるファイルが格納される一時ディレクトリ。 デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>/tmp/lshttpd/</span>です。', '', '絶対パス、または$SERVER_ROOTで始まるパス（ServerおよびVHostレベル）、または$VH_ROOTで始まるパス（VHostレベル）。', '');
...
$_tipsdb['useAIO'] = new DAttrHelp("非同期I/O (AIO)を使用する", '静的ファイルを提供するために非同期I/Oを使用するかどうか、および使用するAIO実装を指定します。オプション<span class=&quot;lst-inline-token lst-inline-token--value&quot;>LINUX AIO</span>と<span class=&quot;lst-inline-token lst-inline-token--value&quot;>io_uring</span>はLinuxマシンでのみ使用できます。<br/>デフォルト値：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>', ' I/O待ち時間が長いサーバーでは、AIOがパフォーマンス向上に役立つ場合があります。<br/> <span class=&quot;lst-inline-token lst-inline-token--value&quot;>io_uring</span>が選択されていても現在のマシンでサポートされていない場合、代わりに<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Linux AIO</span>が使用されます。', 'ドロップダウンリストから選択', '');

$_tipsdb['useIpInProxyHeader'] = new DAttrHelp("ヘッダーにクライアントIPを使用する", '&quot;X-Forwarded-For&quot;HTTPリクエストヘッダーに記載された最初の有効なIPアドレスを、 接続/帯域幅スロットリング、アクセス制御、IPジオロケーションなど、すべてのIPアドレス関連機能に使用するかどうかを指定します。<br/><br/>この機能は、Webサーバーがロードバランサまたはプロキシサーバーの背後にある場合に便利です。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>信頼できるIPのみ</span>を選択すると、サーバーレベルの&quot;許可リスト&quot;で定義された信頼済みIPからリクエストが来た場合にのみ、 X-Forwarded-For IPが使用されます。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>信頼できる IP のヘッダーを保持</span>は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>信頼できるIPのみ</span>と同じですが、バックエンドで使用されるX-Forwarded-Forヘッダーは、 接続元ピアアドレスを含めるようには変更されません。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>最後の IP を使用（AWS ELB 向け）</span>は、&quot;X-Forwarded-For&quot;リスト内の最後のIPアドレスを使用します。 AWS Elastic Load Balancerを使用している場合、または実IPが&quot;X-Forwarded-For&quot;リストの末尾に追加されることを想定している場合は、このオプションを選択してください。', '', 'ドロップダウンリストから選択', '');

$_tipsdb['useSendfile'] = new DAttrHelp("sendfile（）を使用する", '静的ファイルを提供するためにsendfile（）システムコールを使用するかどうかを指定します。 静的ファイルは、メモリバッファキャッシュ、メモリマップキャッシュ、プレーンリード/ライト、およびsendfile（）の4つの方法で提供できます。 &quot;最大キャッシュ小ファイルサイズ(バイト)&quot;より小さいファイルはメモリバッファキャッシュから提供されます。 &quot;最大キャッシュ小ファイルサイズ(バイト)&quot;より大きいが、&quot;最大MMAPファイルサイズ（バイト）&quot;より小さいファイルは、メモリマップドキャッシュから提供されます。 &quot;最大MMAPファイルサイズ（バイト）&quot;より大きいファイルは、プレーンな読み取り/書き込みまたはsendfile（）で処理されます。 Sendfile（）は、非常に大きな静的ファイルを処理するときにCPU使用率を大幅に下げることができる「ゼロコピー」システムコールです。 Sendfile（）は最適化されたネットワークカードカーネルドライバを必要とするため、一部の小規模ベンダーのネットワークアダプタには適していない可能性があります。', '', 'ラジオボタンから選択', '');

$_tipsdb['userDBCacheTimeout'] = new DAttrHelp("ユーザーDBキャッシュタイムアウト（秒）", 'バックエンドユーザーデータベースの変更の確認頻度を指定します。 キャッシュ内のすべてのエントリにタイムスタンプがあります。 キャッシュされたデータが指定されたタイムアウトよりも古い場合、バックエンドデータベースの変更がチェックされます。 変更がなければ、タイムスタンプは現在の時刻にリセットされ、そうでない場合は新しいデータがロードされます。 サーバーのリロードとgraceful restartにより、キャッシュは直ちにクリアされます。', 'バックエンドデータベースが頻繁に変更されない場合は、パフォーマンスを向上させるために、より長いタイムアウトを設定します。', '整数', '');
...
$_tipsdb['userDBMaxCacheSize'] = new DAttrHelp("ユーザーDB最大キャッシュサイズ", 'ユーザーデータベースの最大キャッシュサイズを指定します。 最近アクセスされたユーザー認証データは、最大のパフォーマンスを提供するためにメモリにキャッシュされます。', 'キャッシュが大きくなるとメモリが消費されるため、値が高くなるほどパフォーマンスが向上する場合があります。 ユーザーのデータベースサイズとサイトの使用状況に応じて適切なサイズに設定します。', '整数', '');
...
$_tipsdb['venable'] = new DAttrHelp("有効化", '<span class=&quot;lst-inline-token lst-inline-token--value&quot;>有効</span>アクションは、停止したバーチャルホストを起動します。 これにより、新しい要求を受け入れることができます。', '', '', '');

$_tipsdb['verifyDepth'] = new DAttrHelp("検証深度", ' クライアントに有効な証明書がないと判断する前に、証明書の検証の深さを指定します。 デフォルトは &quot;1&quot;です。', '', 'ドロップダウンリストから選択', '');
...
$_tipsdb['vhEnableGzip'] = new DAttrHelp("圧縮を有効にする", 'このバーチャルホストでGZIP圧縮を有効にするかどうかを指定します。 この設定は、サーバーレベルでGZIP compressionが<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定されている場合にのみ有効です。', '', 'ラジオボックスから選択', '');

$_tipsdb['vhEnableQuic'] = new DAttrHelp("HTTP3/QUICを有効にする", 'このバーチャルホストでHTTP3/QUICネットワークプロトコルを有効にします。 この設定を有効にするには、サーバーレベルの&quot;HTTP3/QUICを有効にする&quot;とリスナーレベルの&quot;HTTP3/QUIC（UDP）ポートを開く&quot;もそれぞれ<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>に設定する必要があります。 デフォルト値は<span class=&quot;lst-inline-token lst-inline-token--value&quot;>はい</span>です。', 'この設定を<span class=&quot;lst-inline-token lst-inline-token--value&quot;>いいえ</span>にすると、HTTP3/QUIC広告は送信されなくなります。ブラウザにキャッシュされたHTTP3/QUIC情報が残っており、サーバーレベルとリスナーレベルでHTTP3/QUICが引き続き有効な場合、その情報のキャッシュが消えるかHTTP3/QUICプロトコルエラーが発生するまで、HTTP3/QUIC接続は引き続き使用されます。', 'ラジオボックスから選択', '');

$_tipsdb['vhMaxKeepAliveReq'] = new DAttrHelp("最大Keep-Aliveリクエスト数", 'Keep-Alive（永続）接続で処理できる最大リクエスト数を指定します。 この制限に達すると接続が閉じられます。 バーチャルホストごとに異なる制限を指定できます。 この数値は、サーバーレベルの&quot;最大Keep-Aliveリクエスト数&quot;の制限を超えることはできません。', '合理的に高い値に設定します。 「1」以下の値を指定すると、キープアライブ接続が無効になります。', '整数', '');
...
$_tipsdb['vhadminEmails'] = new DAttrHelp("管理者Eメール", 'このバーチャルホストの管理者の電子メールアドレスを指定します。', '', '電子メールアドレスのカンマ区切りリスト', '');
...
$_tipsdb['vstatus'] = new DAttrHelp("ステータス - バーチャルホスト", 'バーチャルホストの現在のステータス。   ステータスは<span class=&quot;lst-inline-token lst-inline-token--value&quot;>稼働中</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Stopped</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Restart Required</span>、または<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Running - Removed from Configuration</span>です。  <ul>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>稼働中</span>は、バーチャルホストがロードされ、サービス中であることを意味します。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Stopped</span>は、バーチャルホストがロードされていてもサービスされていない（無効になっている）ことを意味します。 </li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Restart Required</span>は、これが新しく追加されたバーチャルホストであり、サーバがまだ設定をロードしていないことを意味します。 </li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Running - Removed from Configuration</span>は、バーチャルホストがサーバの設定から削除されたが、まだ稼働中であることを意味します。 </li> </ul>', '', '', '');
...
$_tipsdb['wsgiBin'] = new DAttrHelp("WSGIのパス", 'LiteSpeed Python Web Server Gateway Interface実行ファイル（lswsgi）へのパスです。<br/><br/>この実行ファイルは、LiteSpeedのWSGI LSAPIモジュールを使用してPythonをコンパイルすることで作成されます。', '', '絶対パス', '');

$_tipsdb['wsgiDefaults'] = new DAttrHelp("Python WSGIのデフォルト設定", 'Python WSGIアプリケーションのデフォルト設定です。これらの設定はコンテキストレベルで上書きできます。', '', '', '');
