...
$_tipsdb['listenerIP'] = new DAttrHelp("IP地址", '指定此侦听器的IP。 列出了所有可用的IP地址。 IPv6地址包含在“ []”中。 要监听所有IPv4 IP地址，请使用 ANY. 要侦听所有IPv4和IPv6 IP地址,请使用[ANY].  clients,  为了同时为IPv4和IPv6客户端提供服务，应使用IPv4映射的IPv6地址代替纯IPv4地址。 IPv4映射的IPv6地址写为[::FFFF:x.x.x.x]。', ' If your machine has multiple IPs on different sub-networks, you can select a specific IP to only allow traffic from the corresponding sub-network.', '从列表中选择', '');
