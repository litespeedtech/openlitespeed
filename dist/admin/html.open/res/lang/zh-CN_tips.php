...
$_tipsdb['instances'] = new DAttrHelp("Instances", 'Specifies the maximum instances of the external application the server will create. It is required if &quot;Start By Server&quot; is enabled. Most FastCGI/LSAPI applications can only process one request per process instance and for those types of applications, instances should be set to match the value of &quot;最大连接数&quot;. Some FastCGI/LSAPI applications can spawn multiple child processes to handle multiple requests concurrently. For these types of applications, instances should be set to &quot;1&quot; and environment variables used to control how many child processes the application can spawn.', '', '整数', '');
...
$_tipsdb['vhEnableBr'] = new DAttrHelp("Enable Brotli Compression", 'Specifies whether to enable Brotli compression for this virtual host. This setting is only effective when &quot;Brotli 压缩级别（静态内容）&quot; is set to a non-zero value at the server level.', '', '从单选框选择', '');
