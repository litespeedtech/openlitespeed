...
$_tipsdb['adminUser'] = new DAttrHelp("Web Admin User", 'Change the username and password for WebAdmin Console.  The old password has to be entered and verified in order to save changes.', '', '', '');
...
$_tipsdb['autoFix503'] = new DAttrHelp("自动修复503错误", '指定是否尝试通过平滑重启{ws_lb}修复“503 服务不可用”错误。“503”错误通常是由 发生故障的外部应用程序引起的，Web服务器重新启动往往可以临时修复 错误。如果启用，当30秒内出现超过30次“503”错误时，服务器将自动 重新启动。此功能是默认启用的。', '', '布尔值', '');
...
$_tipsdb['ctxType'] = new DAttrHelp("Context Type", 'The type of context created determines it&#039;s usage.<br><br><b>Static</b> context can be used to map a URI to a directory either outside document root or within it.<br> <b>Java Web App</b> context is used to automatically import a predefined Java Application in an AJPv13 compilant Java servlet engine.<br> <b>Servlet</b> context is used to import a specific servlet under a web application.<br> <b>Fast CGI</b> context is a mount point of Fast CGI application.<br> <b>LiteSpeed SAPI</b> context can be used to associate a URI with an LSAPI application.<br> <b>Proxy</b> context enables this virtual host to serve as a transparant reverse proxy server to an external web server or application server.<br> <b>CGI</b> context can be used to specify a directory only contains CGI scripts.<br> <b>Load Balancer</b> context can set up an internal or external redirect URI.<br> <b>Redirect</b> context can set up an internal or external redirect URI.<br> <b>Rack/Rails</b> context is specifically used for Rack/Rails applications.<br> <b>Module handler</b> context is a mount point of hander type modules.<br>', '', '', '');
...
$_tipsdb['enableDHE'] = new DAttrHelp("Enable DH Key Exchange", 'Allows use of Diffie-Hellman key exchange for further SSL encryption.', '[Security] DH key exchange is more secure than using just an RSA key. ECDH and DH key exchange are equally secure.<br/>[Performance] Enabling DH key exchange will increase CPU load and is slower than ECDH key exchange and RSA. ECDH key exchange is preferred when available.', 'Select from radio box', '');
...
$_tipsdb['enableECDHE'] = new DAttrHelp("Enable ECDH Key Exchange", 'Allows use of Elliptic Curve Diffie-Hellman key exchange for further SSL encryption.', '[Security] ECDH key exchange is more secure than using just an RSA key. ECDH and DH key exchange are equally secure.<br/>[Performance] Enabling ECDH key exchange will increase CPU load and is slower than using just an RSA key.', 'Select from radio box', '');
...
$_tipsdb['eventDispatcher'] = new DAttrHelp("I/O事件调度", '指定要使用的I/O事件调度器。不同操作系统支持不同类型的事件调度器:  <ul>   <li>Linux 内核 2.4.x 支持:      <ul><li>poll</li></ul>   </li>   <li>Linux 内核 2.6.x 支持:      <ul><li>poll</li><li>epoll</li></ul>   </li>   <li>FreeBSD 支持:      <ul><li>poll</li><li>kqueue</li></ul>   </li>   <li>Solaris 支持:      <ul><li>poll</li><li>devpoll</li></ul>   </li>   <li>Mac OS X 10.3 及以上支持:      <ul><li>poll</li><li>kqueue</li></ul>   </li> </ul>   poll被所有的平台支持，并且是默认的选择。   对于高流量的网站，可以使用其他事件调度器来提高{ws_lb}的可扩展性。', '', '选项', '');
...
$_tipsdb['gzipAutoUpdateStatic'] = new DAttrHelp("自动更新静态文件", '指定是否由{ws_lb}自动创建/更新可压缩静态文件的GZIP压缩版本。 如果设置为Yes，当请求文件MIME属于&quot;压缩类型&quot;时， {ws_lb}会根据压缩的文件时间戳来创建/更新文件的压缩版本。 压缩的文档会创建在&quot;静态GZIP缓存目录&quot;目录下。 文件名称根据原文件的MD5散列创建。', '', '布尔值', '');
...
$_tipsdb['gzipMaxFileSize'] = new DAttrHelp("静态文件最大尺寸", '指定{ws_lb}可以自动创建压缩文件的静态文件最大尺寸。', '[性能建议] 不建议使用{ws_lb}创建/更新较大文件的压缩文件。 压缩操作会占用整个服务器进程并且在压缩结束前新请求都无法被处理。', '不小于1K的字节数。', '');

$_tipsdb['gzipMinFileSize'] = new DAttrHelp("静态文件最小尺寸", '指定{ws_lb}创建相应压缩文件的静态文件最小尺寸。', '因为流量节省可以忽略不计，所以压缩非常小的文件是没有必要的。', '不小于200的字节数。', '');
...
$_tipsdb['mime'] = new DAttrHelp("MIME设置", '为此服务器指定包含MIME设置的文件。 在chroot模式中提供了绝对路径时，该文件路径总是相对于真正的根。 点击文件名可查看/编辑详细的MIME项。', 'Click the file name to edit the MIME settings.', '文件2', '');
...
$_tipsdb['requiredPermissionMask'] = new DAttrHelp("Required Permission Mask", '为{ws_lb}服务的静态文件指定必需的权限掩码。 例如，如果只允许所有人都可读的文件可以被输出，将该值设置为0004。 用man 2 stat命令了解所有可选值。', '', '八进制数', '');
...
$_tipsdb['restrictedDirPermissionMask'] = new DAttrHelp("脚本目录限制权限掩码", '为{ws_lb}不能服务的脚本文件父目录指定限制权限掩码。 例如，要禁止服务属组可写和全局可写的文件夹内的PHP脚本， 设置掩码为022。默认值是000。 此选项可用于防止执行文件上传目录内的脚本。<br/><br/>用man 2 stat命令了解所有可选值。', '', '八进制数', '');

$_tipsdb['restrictedPermissionMask'] = new DAttrHelp("限制权限掩码", '为{ws_lb}不能输出的静态文件指定限制权限掩码。 例如，要禁止服务可执行文件，将掩码设置为0111。<br/><br/>用man 2 stat命令了解所有可选值。', '', '八进制数', '');

$_tipsdb['restrictedScriptPermissionMask'] = new DAttrHelp("脚本限制权限掩码", '为{ws_lb}不能服务的脚本文件指定限制权限掩码。 例如，要禁止服务属组可写和全局可写的PHP脚本， 设置掩码为022。默认值是000。<br/><br/>用man 2 stat命令了解所有可选值。', '', '八进制数', '');
...
$_tipsdb['sslSessionCacheTimeout'] = new DAttrHelp("Session Cache Timeout (secs)", 'This value determines how long a session ID will be valid within the cache before a renegotiation is required. Default is 3,600.', '', 'Integer number', '');

$_tipsdb['sslSessionTBL'] = new DAttrHelp("SSL Session", 'Session caching allows a client to resume a session within a set amount of time without having to re-perform an SSL handshake. You can do this by assigning clients a  session ID using  <b>Enable Session Cache</b>, or by creating and using session tickets. <br><br> Session tickets will be rotated automatically if the tickets are being generated by the server. If using the <b>SSL Session Ticket Key File</b> option to create and manage your own session tickets, you must be rotate the tickets yourself using a cron job.', '', '', '');
...
$_tipsdb['swappingDir'] = new DAttrHelp("交换目录", '指定交换文件的存放目录。 服务器在chroot模式启动时，该路径相对于新的根目录， 否则，它相对于真正的根目录。<br/><br/>Litespeed {ws_lb}使用自己的虚拟内存 以降低系统的内存使用量。虚拟内存和磁盘交换会用来存储大的请求内容和 动态响应。交换目录应设置在有足够剩余空间的磁盘上。', '[性能建议] 将交换目录设置在一个单独的磁盘上，或者增加最大读写缓冲区大小以避免交换。', '路径1', '');
...
$_tipsdb['EDTP:accessLog_fileName'] = array('Log file path can be either be an absolute path or relative to $SERVER_ROOT.'); 

$_tipsdb['EDTP:aclogUseServer'] = array('When required, you can disable access logging for this virtual host save disk i/o.'); 
...
$_tipsdb['EDTP:ctxType'] = array('<b>Static</b> context can be used to map a URI to a directory either outside document root or within it.','<b>Java Web App</b> context is used to automatically import a predefined Java Application in an AJPv13 compilant Java servlet engine.','<b>Servlet</b> context is used to import a specific servlet under a web application.','<b>Fast CGI</b> context is a mount point of Fast CGI application.','<b>LiteSpeed SAPI</b> context can be used to associate a URI with an LSAPI application.','<b>Proxy</b> context enables this virtual host to serve as a transparant reverse proxy server to an external web server or application server.','<b>CGI</b> context can be used to specify a directory only contains CGI scripts.','<b>Load Balancer</b> context can set up an internal or external redirect URI.','<b>Redirect</b> context can set up an internal or external redirect URI.','<b>Rack/Rails</b> context is specifically used for Rack/Rails applications.','<b>Module handler</b> context is a mount point of hander type modules.'); 
...
$_tipsdb['EDTP:log_fileName'] = array('Log file path can be either an absolute path or relative to $SERVER_ROOT.'); 
...
$_tipsdb['EDTP:sndBufSize'] = array('Numbers can be represented as 10240, 10K or 1M.','If send/receive buffer size is 0, default TCP buffer size will be used.'); 
...
$_tipsdb['EDTP:vhlog_fileName'] = array('Log file path can be either absolute path or relative to $SERVER_ROOT, $VH_ROOT.','If you want to set Log Level to DEBUG, you must set server log level to DEBUG as well. The level of  debugging is solely controlled by Server DEBUG Level. Use DEBUG only if you have to since it has great impact on server performance and may fill up disk space quickly.'); 
