...
$_tipsdb['UDBpass1'] = new DAttrHelp("确认新密码", '密码可以是任意长度，可以包含任何字符。', '', '', '');
...
$_tipsdb['accessDenyDir'] = new DAttrHelp("拒绝访问的目录", '指定应该拒绝访问的目录。 将包含敏感数据的目录加入到这个列表，以防止向客户端意外泄露敏感文件。 在路径后加一个“*”，可包含所有子目录。 如果&quot;跟随符号链接&quot;和&quot;检查符号链接&quot;都被启用， 符号链接也将被检查是否在被拒绝访问目录中。', ' 至关重要: 此设置只能防止服务这些目录中的静态文件。 这不能防止外部脚本如PHP、Ruby、CGI造成的泄露。', '以逗号分隔的目录列表。', '');

$_tipsdb['accessLog_bytesLog'] = new DAttrHelp("字节日志", '指定带宽字节日志文件的路径。设置后，将创建一份兼容cPanel面板的带宽日志。这将记录 一个请求传输的总字节数，包括请求内容和响应内容。', ' 将日志文件放在单独的磁盘上。', '文件名可以是绝对路径，也可以是相对于$SERVER_ROOT的相对路径。', '');

$_tipsdb['accessLog_fileName'] = new DAttrHelp("文件名", '指定访问日志文件的文件名。', ' 将访问日志文件放置在单独的磁盘上。', '文件路径', '');
...
$_tipsdb['accessLog_logHeader'] = new DAttrHelp("日志头部", '指定是否记录HTTP请求头：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Referer</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>UserAgent</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Host</span>。', ' 如果不需要在访问日志中记录这些头部信息，请关闭此功能。', '从复选框选择', '');
...
$_tipsdb['acme'] = new DAttrHelp("ACME自动证书", '使用Automatic Certificate Management Environment(ACME)证书协议自动生成和续订SSL证书（配置后）。更多信息请参见：<br/><a href=" https://docs.openlitespeed.org/config/advanced/acme/ " target="_blank" rel="noopener noreferrer"> 自动SSL证书（ACME） </a><br/><br/>接受的值：<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>开</span> - 启用自动证书生成和续订。虚拟主机可以单独覆盖此设置。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>关</span> - 禁用，但虚拟主机可以单独覆盖此设置。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Disabled</span> - （仅服务器级别）强制在整个服务器范围内关闭ACME；虚拟主机无法覆盖此设置。<br/><br/>默认值：<br/><b>服务器级别：</b>Off<br/><b>虚拟主机级别：</b>继承服务器级别设置', '', '从列表中选择', '');

$_tipsdb['acme_dns_type'] = new DAttrHelp("ACME API dns_type", '（可选）执行通配符证书生成API调用时使用的dns类型。此值取决于所使用的DNS提供商， 生成非通配符证书时不需要。有关可用dns类型API值的更多信息，请参见：<br/><a href=" https://github.com/acmesh-official/acme.sh/wiki/dnsapi " target="_blank" rel="noopener noreferrer"> acme.sh-如何使用DNS API </a><br/><br/>与设置&quot;环境变量&quot;配合使用。', '', '', '对于DNS提供商CloudFlare，dns类型值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>dns_cf</span>。');

$_tipsdb['acme_env'] = new DAttrHelp("环境变量", '（可选）执行通配符证书生成API调用时要包含的环境值。所需的具体值取决于使用的DNS提供商， 生成非通配符证书时不需要。有关所需环境API值的更多信息，请参见：<br/><a href=" https://github.com/acmesh-official/acme.sh/wiki/dnsapi " target="_blank" rel="noopener noreferrer"> acme.sh-如何使用DNS API </a><br/><br/>与设置&quot;ACME API dns_type&quot;配合使用。', '', '<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Key=&quot;VALUE&quot;</span>键值对列表，每行一个，且每个VALUE都用双引号括起。', '对于DNS提供商CloudFlare，唯一必需的环境值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CF_Token=&quot;YOUR_TOKEN&quot;</span>');

$_tipsdb['acme_vhenable'] = new DAttrHelp("已启用", '使用Automatic Certificate Management Environment（ACME）证书协议自动生成和续订SSL证书（配置后）。更多信息请参见：<br/><a href=" https://docs.openlitespeed.org/config/advanced/acme/ " target="_blank" rel="noopener noreferrer"> 自动SSL证书（ACME） </a><br/><br/>接受的值：<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>开</span> - 启用自动证书生成和续订。虚拟主机可以单独覆盖此设置。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>关</span> - 禁用，但虚拟主机可以单独覆盖此设置。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Disabled</span> - （仅服务器级别）强制在整个服务器范围内关闭ACME；虚拟主机无法覆盖此设置。<br/><br/>默认值：<br/><b>服务器级别：</b>关闭<br/><b>虚拟主机级别：</b>继承服务器级别设置', '', '从列表中选择', '');

$_tipsdb['addDefaultCharset'] = new DAttrHelp("添加默认字符集", '指定当内容类型为&quot;text/html&quot;或&quot;text/plain&quot;且没有参数时，是否向&quot;Content-Type&quot;响应报头添加字符集标记。设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>关</span>时，该功能禁用。设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>开</span>时，将添加&quot;自定义默认字符集&quot;中指定的字符集；如果未指定，则添加默认的&quot;iso-8859-1&quot;字符集。', '', '布尔值', '');
...
$_tipsdb['adminEmails'] = new DAttrHelp("管理员邮箱", '指定服务器管理员的电子邮箱地址。 如果设置了电子邮箱地址，管理员将收到重要事件的电子邮件通知（例如， LiteSpeed服务因崩溃而自动重启或者授权即将过期）。', '电子邮件提醒功能只有在服务器有活动的MX服务器（如postfix、exim或sendmail）的情况下才会生效。', '以逗号分隔的电子邮件地址列表。', '');
...
$_tipsdb['allowBrowse'] = new DAttrHelp("可访问", '指定是否可以访问此上下文。设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>以拒绝访问。 您可以使用此功能来防止访问指定目录。 您可以在更新此上下文内容或此目录中有特殊数据时使用它。', '', '布尔值', '');

$_tipsdb['allowQuic'] = new DAttrHelp("开放HTTP3/QUIC（UDP）端口", '允许映射到此侦听器的虚拟主机使用HTTP/3/QUIC网络协议。 要使此设置生效，服务器级别的&quot;启用HTTP3/QUIC&quot;也必须设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。 默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。', '当此设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>时，仍可以通过虚拟主机级别的&quot;启用HTTP3/QUIC&quot;设置禁用HTTP/3/QUIC。', '', '');

$_tipsdb['allowSetUID'] = new DAttrHelp("允许设置UID", '指定CGI脚本是否允许set UID位。如果允许，并且CGI脚本启用了set UID位，那么无论CGI脚本代表哪个用户启动，CGI进程的用户ID都会切换为CGI脚本所有者的用户ID。<br/>默认值为&quot;Off&quot;。', ' 尽可能不要允许CGI脚本设置UID，因为这本身存在安全风险。', '布尔值', '');

$_tipsdb['allowSymbolLink'] = new DAttrHelp("跟随符号链接", '指定在这个虚拟主机内是否要跟随符号链接。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>仅限相同属主</span>选项启用后，只有在链接和目标属主一致时才跟踪符号链接。 此设置将覆盖默认的服务器级设置。', ' 为了更好的安全性，请禁用此功能。为了获得更好的性能，启用它。', '从下拉列表选择', '');
...
$_tipsdb['appType'] = new DAttrHelp("应用程序类型", '此上下文使用的应用程序类型。支持Rack/Rails、WSGI和Node.js。', '', '', '');
...
$_tipsdb['autoFix503'] = new DAttrHelp("自动修复503错误", '指定是否尝试通过平滑重启服务器来修复“503 Service Unavailable”错误。 “503”错误通常由发生故障的外部应用程序引起，重启Web服务器通常可以临时修复该错误。 如果启用，当30秒内出现超过30个“503”错误时，服务器将自动重启。<br/><br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>', '', '从单选框中选择', '');

$_tipsdb['autoIndex'] = new DAttrHelp("自动索引", '在目录中，当&quot;索引文件&quot;中所列的索引文件不可用时，指定运行时是否即时生成目录索引。<br/>此选项可以在虚拟主机级别和Context级别中设置，并可以顺着目录树继承，直到被显式覆盖。 您可以自定义生成的索引页面。请查看在线Wiki操作指南。', ' 建议尽可能关闭自动索引，以防止泄露机密数据。', '布尔值', '');
...
$_tipsdb['autoStart'] = new DAttrHelp("自动启动", '指定是否要由Web服务器自动启动应用程序。只有运行在同一台机器上的FastCGI和LSAPI应用程序才能自动启动。 &quot;地址&quot;中的IP必须是本地IP。通过LiteSpeed CGI守护进程而不是主服务器进程启动，有助于减少系统开销。<br/><br/>默认值：Yes (Through CGI Daemon)', '', '从下拉列表选择', '');

$_tipsdb['backlog'] = new DAttrHelp("待处理队列", '指定监听套接字的待处理队列长度。启用&quot;自动启动&quot;时需要设置。', '', '非负整数', '');

$_tipsdb['banPeriod'] = new DAttrHelp("封禁时长（秒）", '指定在&quot;宽限期（秒）&quot;之后，如果连接数仍然高于 &quot;连接软限制&quot;，来自该IP的新连接将被拒绝多长时间。如果IP 经常被屏蔽，我们建议您延长禁止期以更强硬地惩罚滥用.', '', '非负整数', '');

$_tipsdb['binPath'] = new DAttrHelp("二进制文件路径", 'App Server应用程序二进制文件的位置。', '', '', '');

$_tipsdb['blockBadReq'] = new DAttrHelp("阻止异常请求", '阻止持续发送格式错误的HTTP请求的IP，阻止时长由&quot;封禁时长（秒）&quot;指定。默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。 这有助于阻止反复发送垃圾请求的僵尸网络攻击。', '', '从单选框中选择', '');

$_tipsdb['brStaticCompressLevel'] = new DAttrHelp("Brotli压缩级别（静态文件）", '指定Brotli压缩静态内容的级别。 范围从<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span>(最低)到<span class=&quot;lst-inline-token lst-inline-token--value&quot;>9</span>(最高)。<br/>当设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>时，Brotli压缩将在全局禁用。<br/>默认值：5', ' 压缩可以用来节省网络带宽。 基于文本的响应（例如html，css和javascript文件）效果最好，平均可以将其压缩为原始大小的一半。', '0到11之间的数字。', '');

$_tipsdb['bubbleWrap'] = new DAttrHelp("Bubblewrap容器", '如果希望在Bubblewrap沙箱中启动CGI进程（包括PHP程序），请设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>已启用</span>。有关使用Bubblewrap的详细信息，请参见 <a href="   https://wiki.archlinux.org/title/Bubblewrap " target="_blank" rel="noopener noreferrer">   https://wiki.archlinux.org/title/Bubblewrap </a> 。使用此设置之前，必须先在系统上安装Bubblewrap。<br/>如果服务器级别设置为“Disabled”，则无法在虚拟主机级别启用此设置。<br/>默认值：<br/><b>服务器级别：</b>Disabled<br/><b>虚拟主机级别：</b>继承服务器级别设置', '', '从列表中选择', '');

$_tipsdb['bubbleWrapCmd'] = new DAttrHelp("Bubblewrap命令", 'bubblewraps使用的完整的命令, 包括bubblewrap程序本身。 有关配置此命令的更多信息，请参见： <a href="   https://openlitespeed.org/kb/bubblewrap-in-openlitespeed/ " target="_blank" rel="noopener noreferrer">   https://openlitespeed.org/kb/bubblewrap-in-openlitespeed/ </a>. 如果未指定，将使用下面列出的默认命令。<br/>默认值: <span class=&quot;lst-inline-token lst-inline-token--command&quot;>/bin/bwrap --ro-bind /usr /usr --ro-bind /lib /lib --ro-bind-try /lib64 /lib64 --ro-bind /bin /bin --ro-bind /sbin /sbin --dir /var --dir /tmp --proc /proc --symlink../tmp var/tmp --dev /dev --ro-bind-try /etc/localtime /etc/localtime --ro-bind-try /etc/ld.so.cache /etc/ld.so.cache --ro-bind-try /etc/resolv.conf /etc/resolv.conf --ro-bind-try /etc/ssl /etc/ssl --ro-bind-try /etc/pki /etc/pki --ro-bind-try /etc/man_db.conf /etc/man_db.conf --ro-bind-try /home/$USER /home/$USER --bind-try /var/lib/mysql/mysql.sock /var/lib/mysql/mysql.sock --bind-try /home/mysql/mysql.sock /home/mysql/mysql.sock --bind-try /tmp/mysql.sock /tmp/mysql.sock  --unshare-all --share-net --die-with-parent --dir /run/user/$UID ‘$PASSWD 65534’ ‘$GROUP 65534’</span>', '', '文本', '');

$_tipsdb['certChain'] = new DAttrHelp("证书链", '指定证书是否为链式证书。存储证书链的文件必须为PEM格式， 并且证书必须按链式顺序排列，从最低级别（实际的客户端或服务器证书）到最高级别（根CA）。', '', '布尔值', '');

$_tipsdb['certFile'] = new DAttrHelp("证书文件", 'SSL证书文件的文件名。', ' 证书文件应放在安全目录中，并且只允许服务器运行用户读取。', '文件路径', '');
...
$_tipsdb['cgiResource'] = new DAttrHelp("CGI设置", 'The following settings control CGI processes. Memory and process limits also serve as the default for other external applications if limits have not been set explicitly for those applications.', '', '', '');
...
$_tipsdb['connTimeout'] = new DAttrHelp("连接超时时长（秒）", '指定一个请求允许的最大连接空闲时间。 如果在这段时间内连接一直处于空闲状态(即没有I/O活动），则它将被关闭。', ' 将值设置得尽可能低，在可能的拒绝服务攻击中，这可以帮助释放无效连接所占用的连接数。', '非负整数', '');

$_tipsdb['consoleSessionTimeout'] = new DAttrHelp("会话超时时长（秒）", '自定义WebAdmin控制台会话超时时间。 默认值为60秒。', ' 在生产环境中请设置合适的值，通常应低于300秒。', '整数', '');

$_tipsdb['cpuAffinity'] = new DAttrHelp("CPU亲和性", 'CPU关联将进程绑定到一个或多个CPU（核心）。 始终使用同一CPU对进程来说是有益的，因为这样进程可以利用CPU缓存中剩余的数据。 如果进程移至其他CPU，则不会使用CPU缓存，并且会产生不必要的开销。<br/><br/>CPU Affinity设置控制一个服务器进程将与多少个CPU（核心）相关联。 最小值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>，它将禁用此功能。 最大值是服务器具有的核心数。 通常，<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span>是最佳设置，因为它会最严格地使用CPU亲和力，从而最大程度地利用CPU缓存。<br/><br/>默认值：0', '', '0到64之间的整数。（0将禁用此功能）', '');
...
$_tipsdb['defaultCharsetCustomized'] = new DAttrHelp("自定义默认字符集", '指定当&quot;添加默认字符集&quot;为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>开</span>时使用的字符集。此项可选，默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>iso-8859-1</span>。当&quot;添加默认字符集&quot;为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>关</span>时，本设置不生效。', '', '字符集名称。', 'utf-8');
...
$_tipsdb['disableInitLogRotation'] = new DAttrHelp("禁用初始日志轮转", '指定在启动时是否启用/禁用服务器错误日志文件的轮换。 使用值为“未设置”时，默认启用初始日志轮转。', '', '布尔值', '');
...
$_tipsdb['domainName'] = new DAttrHelp("域名", '指定映射域名。域名不区分大小写，并且会移除开头的&quot;www.&quot;。 允许使用通配符&quot;*&quot;和&quot;?&quot;。&quot;?&quot;只代表一个字符， &quot;*&quot;代表任意数量的字符。 不允许重复域名。', ' 如果侦听器专用于一个虚拟主机， 请始终使用<span class=&quot;lst-inline-token lst-inline-token--value&quot;>*</span>作为域名，以避免不必要的检查。 除catchall域名外，应尽量避免使用带通配符的域名。', '逗号分隔列表。', 'www?.example.com<br/>&quot;*.mydomain.com&quot;将匹配mydomain.com的所有子域。<br/>&quot;*&quot;本身是catchall域名，会匹配任何未匹配的域名。');

$_tipsdb['dynReqPerSec'] = new DAttrHelp("动态请求/秒", '指定每秒可处理的来自单个IP的动态请求的数量（无论与该IP之间建立了多少个连接） 当达到此限制时，所有后来的请求将被延滞到下一秒。<br/>静态内容的请求限制与此限制无关。 可以在服务器或虚拟主机级别设置每个客户端请求的限制。 虚拟主机级别的设置将覆盖服务器级别的设置。', ' 受信任的IP或子网不受影响。', '非负整数', '');

$_tipsdb['enableCoreDump'] = new DAttrHelp("启用核心转储", '指定当服务器由root用户启动时是否启用核心转储。 对于大多数现代Unix系统，出于安全考虑，更改用户ID或组ID的进程不允许生成core文件。不过，core dump文件有助于定位问题根因。 此选项仅适用于Linux内核2.4及更高版本。 Solaris用户应当使用<span class=&quot;lst-inline-token lst-inline-token--command&quot;>coreadm</span>命令来控制这个功能。', ' 仅当你在服务器日志中看到<span class=&quot;lst-inline-token lst-inline-token--value&quot;>no core file created</span>时启用。生成core文件后请立即关闭。创建core dump后请提交bug报告。', '布尔值', '');

$_tipsdb['enableDHE'] = new DAttrHelp("启用DH密钥交换", '允许使用Diffie-Hellman密钥交换进行进一步的SSL加密。', ' DH密钥交换比仅使用RSA密钥更安全。 ECDH和DH密钥安全性相同。<br/><br/> 启用DH密钥交换会增加CPU负载，并且比ECDH密钥交换和RSA都慢。如果可用，建议优先使用ECDH密钥交换。', '布尔值', '');

$_tipsdb['enableDynGzipCompress'] = new DAttrHelp("启用GZIP动态压缩", '控制动态HTTP回应的GZIP压缩。 GZIP compression必须设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>来开启动态GZIP压缩。<br/>默认值：Yes', ' 压缩动态回应将增加CPU和内存的使用，但可以节省网络带宽。', '布尔值', '');

$_tipsdb['enableECDHE'] = new DAttrHelp("启用ECDH密钥交换", '允许使用椭圆曲线Diffie-Hellman密钥交换进行进一步的SSL加密。', ' ECDH密钥交换比仅使用RSA密钥更安全。 ECDH和DH密钥交换安全性相同。<br/><br/> 启用ECDH密钥交换会增加CPU负载，并且比仅使用RSA密钥要慢。', '布尔值', '');
...
$_tipsdb['enableGzipCompress'] = new DAttrHelp("启用压缩", '为静态和动态响应启用GZIP/Brotli压缩。<br/>默认值：Yes', ' 启用后可节省网络带宽。基于文本的响应（如html、css和javascript文件）受益最大，平均可压缩到原始大小的一半。', '从单选框中选择', '');
...
$_tipsdb['enableRecaptcha'] = new DAttrHelp("启用reCAPTCHA", '在当前级别启用CAPTCHA保护功能。必须先在服务器级别将此设置设为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>，才能使用CAPTCHA保护功能。<br/>默认值：<br/><b>服务器级别：</b><span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span><br/><b>虚拟主机级别：</b>继承服务器级别设置', '', '布尔值', '');
...
$_tipsdb['enableSpdy'] = new DAttrHelp("ALPN", '应用层协议协商（ALPN）用于有选择地启用HTTP/3和HTTP/2网络协议。<br/><br/>如果要禁用HTTP/2和HTTP3，请勾选<span class=&quot;lst-inline-token lst-inline-token--value&quot;>无</span>，并取消勾选所有其他选项。<br/><br/>默认值：全部启用', '可在侦听器和虚拟主机级别设置。', '从复选框选择', '');

$_tipsdb['enableStapling'] = new DAttrHelp("启用OCSP Stapling", '确定是否启用OCSP装订，这是一种更有效的验证公钥证书的方式。', '', '布尔值', '');

$_tipsdb['enableh2c'] = new DAttrHelp("在明文传输上启用HTTP/2", '指定是否通过非加密TCP连接启用HTTP/2。 默认设置为禁用。', '', '布尔值', '');

$_tipsdb['env'] = new DAttrHelp("环境变量", '为外部应用程序指定额外的环境变量。', '', 'Key=value。多个变量可以用回车换行分隔。', '');
...
$_tipsdb['extAppAddress'] = new DAttrHelp("地址", '外部应用程序使用的唯一套接字地址。 支持IPv4/IPv6套接字和Unix域套接字（UDS）。 IPv4/IPv6套接字可用于网络通信。 只有当外部应用程序与服务器在同一台机器上时，才能使用UDS。', ' 如果外部应用程序在同一台机器上运行，则首选UDS。如果必须使用IPv4/IPv6， 将IP地址设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>localhost</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>127.0.0.1</span>，这样外部应用就无法从其他机器上访问。  Unix域套接字一般比IPv4套接字拥有更高的性能。', 'IPv4或IPv6地址:端口，或者UDS://path', '127.0.0.1:5434<br/>UDS://tmp/lshttpd/php.sock.');

$_tipsdb['extAppName'] = new DAttrHelp("名称", '此外部应用程序的唯一名称。 在配置的其他部分中使用该名称时，将使用该名称进行引用。', '', '文本', '');

$_tipsdb['extAppPath'] = new DAttrHelp("命令", '指定执行外部应用程序的完整命令行，包括参数。 启用&quot;自动启动&quot;时，此项为必填。如果参数中包含空格或制表符，应使用双引号或单引号括起该参数。', '', '可执行文件的完整路径，可带可选参数。', '');
...
$_tipsdb['extAppType'] = new DAttrHelp("类型", '指定外部应用程序的类型。应用程序类型根据它们提供的服务或与服务器通信所使用的协议区分。可选类型包括： <ul>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>LiteSpeed SAPI应用程序</span>: 使用LSAPI协议与Web服务器通信的应用程序。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>网络服务器</span>: 支持HTTP协议的Web服务器或应用服务器。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>FastCGI应用程序</span>: 具有<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Responder</span>角色的FastCGI应用程序。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>FastCGI认证器</span>: 具有<span class=&quot;lst-inline-token lst-inline-token--value&quot;>授权器</span>角色的FastCGI应用程序。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>SCGI 应用程序</span>: 使用SCGI协议与Web服务器通信的应用程序。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Servlet引擎</span>: 带AJPv13连接器的Servlet引擎，例如Tomcat。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>管道日志程序</span>: 可处理从STDIN流接收的访问日志条目的应用程序。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>负载均衡器</span>: 可在worker应用程序之间进行负载均衡的虚拟应用程序。</li>   <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>uWSGI</span>: 使用uWSGI协议与Web服务器通信的应用程序。</li> </ul>', '大多数应用程序会使用LSAPI或FastCGI协议。LSAPI支持PHP、Ruby和Python。Perl可与FastCGI一起使用。（PHP、Ruby和Python也可以设置为使用FastCGI运行，但使用LSAPI运行更快。）Java使用Servlet引擎。', '从下拉列表选择', '');
...
$_tipsdb['extGroup'] = new DAttrHelp("以此用户组身份运行", '外部应用程序将以此处指定的组名运行。如果未设置，将继承虚拟主机级别的设置。<br/><br/>默认值：未设置', '', '有效的组名。', '');

$_tipsdb['extMaxIdleTime'] = new DAttrHelp("最大空闲时间", '指定服务器停止外部应用之前的最大空闲时间，以释放空闲资源。 当设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>-1</span>时，服务器不会停止外部应用，除非它在ProcessGroup模式下运行； 在ProcessGroup模式下，空闲的外部应用会在30秒后停止。<br/><br/>默认值为: <span class=&quot;lst-inline-token lst-inline-token--value&quot;>-1</span>', ' 此功能在大规模托管环境中特别有用。为了避免一个虚拟主机拥有的文件被另一个虚拟主机的外部应用脚本访问，通常会同时以SetUID模式运行许多不同的应用。将该值设置得较低，可以避免这些外部应用不必要地长时间空闲。', '整数', '');

$_tipsdb['extUmask'] = new DAttrHelp("权限掩码", '设置此外部应用程序进程的默认umask。详情请参阅<span class=&quot;lst-inline-token lst-inline-token--command&quot;> man 2 umask </span>。默认值取自服务器级别的&quot;权限掩码&quot;设置。', '', '有效范围为[000]-[777]的值。', '');

$_tipsdb['extUser'] = new DAttrHelp("以此用户身份运行", '外部应用程序将以此处指定的用户名运行。如果未设置，将继承虚拟主机级别的设置。<br/><br/>默认值：未设置', '', '有效的用户名。', '');

$_tipsdb['extWorkers'] = new DAttrHelp("工作器", '之前在外部负载均衡器中定义的worker组列表。', '', 'ExternalAppType::ExternalAppName格式的逗号分隔列表。', 'fcgi::localPHP, proxy::backend1');
...
$_tipsdb['extraHeaders'] = new DAttrHelp("头部操作", '指定要添加的附加响应/请求头。可添加多个头指令，每行一个。&quot;NONE&quot;可用于禁用父级头继承。如果未提供指令，则假定为&quot;Header&quot;。', ' 语法和用法类似于 <a href="https://httpd.apache.org/docs/2.2/mod/mod_headers.html#header" target="_blank" rel="noopener noreferrer">Apache的mod_headers指令</a><br/><br/> &#039;Header&#039;指令是可选的。从其他位置复制规则时，可以移除或保留它，不会影响使用。', '[Header]|RequestHeader [condition] set|append|merge|add|unset header [value] [early|env=[!]variable]', 'set Cache-control no-cache<br/>append Cache-control no-store<br/>Header set My-header cust_header_val<br/>RequestHeader set My-req-header cust_req_header_val');
...
$_tipsdb['fcgiapp'] = new DAttrHelp("FastCGI应用程序", '指定FastCGI应用程序的名称。 必须在服务器或虚拟主机级别的&quot;外部应用&quot;中定义此应用程序。', '', '从下拉列表选择', '');
...
$_tipsdb['followSymbolLink'] = new DAttrHelp("跟随符号链接", '指定服务静态文件时跟踪符号链接的服务器级别默认设置。<br/>选项有<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>仅限相同属主</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>设置服务器始终跟踪符号链接。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>仅限相同属主</span>设置服务器只有在链接和目标属主一致时才跟踪符号链接。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>表示服务器永远不会跟踪符号链接。 该设置可以在虚拟主机配置中覆盖，但不能通过.htaccess文件覆盖。', ' 要获得最佳安全性，选择<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>仅限相同属主</span>。 要获得最佳性能，选择<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。', '从下拉列表选择', '');

$_tipsdb['forceGID'] = new DAttrHelp("强制GID", '指定一组ID，以用于所有在suEXEC模式下启动的外部应用程序。 当设置为非零值时，所有suEXEC的外部应用程序（CGI、FastCGI、 LSAPI）都将使用该组ID。这可以用来防止外部应用程序访问其他用 户拥有的文件。<br/>例如，在共享主机环境，LiteSpeed以“www-data”用户、“www-data”组 身份运行。每个文件根目录是由用户帐户所有，属组为“www-data”，权限 为0750。如果强制GID被设置为“nogroup”（或“www-data”之外的任何一 个组），所有suEXEC外部应用程序都将以特定用户身份运行，但属组为 “nogroup”。这些外部应用程序的进程依然能够访问属于相应用户的文件（ 因为他们的用户ID），但没有组权限访问其他人的文件。另一方面，服务器 仍然可以服务在任何用户文件根目录下的文件（因为它的组ID）。', ' 设置足够高的值以排除所有系统用户所在的组。', '非负整数', '');

$_tipsdb['forceStrictOwnership'] = new DAttrHelp("强制严格属主检查", '指定是否执行严格的文件所有权检查。 如果启用，Web服务器将检查正在服务的文件的所有者与虚拟主机的所有者是否相同。 如果不同，将返回403拒绝访问错误。 该功能默认是关闭的。', ' 对于共享主机，启用此检查以得到更好的安全性。', '布尔值', '');
...
$_tipsdb['geolocationDB'] = new DAttrHelp("MaxMind地理定位数据库", '多个MaxMind地理定位数据库可以在这里指定。MaxMind有以下数据库类型: 国家，地区，城市，组织，ISP和NETSPEED。如果混合配置“国家”，“地区”，和“城市”类型数据库，则最后一项将会生效。', '', '', '');

$_tipsdb['gracePeriod'] = new DAttrHelp("宽限期（秒）", '指定来自一个IP的连接数超过&quot;连接软限制&quot;之后， 多长时间之内可以继续接受新连接。在此期间，如果总连接数仍然 低于&quot;连接数硬限制&quot;，将继续接受新连接。之后，如果连接数 仍然高于&quot;连接软限制&quot;，相应的IP将被封锁&quot;封禁时长（秒）&quot;里设置的时长。', ' 设置为足够大的数量，以便下载完整网页， 但也要足够低以防范蓄意攻击。', '整数', '');

$_tipsdb['gracefulRestartTimeout'] = new DAttrHelp("平滑重启超时时长（秒）", '平滑重启时，即使新的服务器实例已经启动，旧的实例仍将继续 处理现有的请求。此项超时设置定义了旧实例等待多长时间后中止。 默认值是300秒。 -1表示永远等待。 0表示不等待，立即中止。', '', '整数', '');

$_tipsdb['groupDBCacheTimeout'] = new DAttrHelp("组数据库缓存超时（秒）", '指定多久检查一次后端组数据库变更。 更多详细信息请参阅&quot;用户数据库缓存超时（秒）&quot;。', '', '非负整数', '');

$_tipsdb['groupDBMaxCacheSize'] = new DAttrHelp("组数据库最大缓存大小", '指定组数据库的最大缓存大小。', ' 由于更大的缓存会消耗更多的内存， 更高的值可能会也可能不会提供更好的性能。 请根据你的用户数据库大小和网站使用情况来设置合适的大小。', '非负整数', '');

$_tipsdb['gzipAutoUpdateStatic'] = new DAttrHelp("自动更新静态文件", '指定是否由LiteSpeed自动创建/更新可压缩静态文件的GZIP/Brotli压缩版本。 如果设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>，当请求文件MIME属于&quot;压缩类型&quot;时， LiteSpeed会根据压缩的文件时间戳来创建/更新文件的压缩版本。 压缩的文档会创建在&quot;静态GZIP缓存目录&quot;目录下。 文件名称根据原文件的MD5散列创建。<br/>默认值: Yes', '', '布尔值', '');

$_tipsdb['gzipCacheDir'] = new DAttrHelp("静态GZIP缓存目录", '指定目录路径来存储静态内容的压缩文件。默认是&quot;交换目录&quot;。', '', '目录路径', '');

$_tipsdb['gzipCompressLevel'] = new DAttrHelp("GZIP压缩级别（动态内容）", '指定应用于动态内容的GZIP压缩级别。范围从<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span>（最低）到<span class=&quot;lst-inline-token lst-inline-token--value&quot;>9</span>（最高）。<br/>此设置仅在GZIP压缩和&quot;启用GZIP动态压缩&quot;启用时生效。<br/>默认值：6', ' 更高的压缩级别会消耗更多内存和CPU周期。如果机器有额外性能，可以设置为更高级别。级别6和级别9之间差异不大，但级别9会消耗更多CPU周期。', '1到9之间的数字。', '');

$_tipsdb['gzipMaxFileSize'] = new DAttrHelp("静态文件最大尺寸（字节）", '指定LiteSpeed可以自动创建压缩文件的静态文件最大尺寸。<br/>默认值为 10M', ' 不建议使用LiteSpeed创建/更新较大文件的压缩文件。 压缩操作会占用整个服务器进程并且在压缩结束前新请求都无法被处理。', '不小于1K的字节数。', '');

$_tipsdb['gzipMinFileSize'] = new DAttrHelp("静态文件最小尺寸（字节）", '指定LiteSpeed创建相应压缩文件的静态文件最小尺寸。<br/>默认值：200', '非常小的文件节省的带宽可以忽略，通常无需压缩。', '不小于200的字节数。', '');

$_tipsdb['gzipStaticCompressLevel'] = new DAttrHelp("GZIP压缩级别（静态文件）", '指定GZIP压缩静态内容的级别。 范围从<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span> (最低)到<span class=&quot;lst-inline-token lst-inline-token--value&quot;>9</span> (最高)。。<br/>该选项仅在 GZIP compression和&quot;自动更新静态文件&quot; 启用后才会生效<br/>默认值是<span class=&quot;lst-inline-token lst-inline-token--value&quot;>6</span>', '', '1到9之间的数字。', '');

$_tipsdb['hardLimit'] = new DAttrHelp("连接数硬限制", '指定来自单个IP的并发连接的硬限制。 此限制是永远执行的，客户端将永远无法超过这个限制。 HTTP/1.0客户端通常会尝试建立尽可能多的连接，因为它们需要同时下载嵌入的内容。此限制应设置得足够高，以使HTTP/1.0客户端仍然可以访问相应的网站。 使用&quot;连接软限制&quot;设置期望的连接限制。<br/>建议根据你的网页内容和流量负载，限制在<span class=&quot;lst-inline-token lst-inline-token--value&quot;>20</span>与<span class=&quot;lst-inline-token lst-inline-token--value&quot;>50</span>之间。', ' 一个较低的数字将使得服务器可以服务更多独立的客户。<br/> 受信任的IP或子网不受影响。<br/> 使用大量并发客户端进行基准测试时，设置一个较高的值。', '整数', '');

$_tipsdb['httpdWorkers'] = new DAttrHelp("工作器数量", '指定httpd worker的数量。', ' 设置适当的数量以满足您的需求。增加更多worker不一定意味着性能更好。', '1到16之间的整数。', '');

$_tipsdb['inBandwidth'] = new DAttrHelp("入口带宽（字节/秒）", '指定对单个IP地址允许的最大传入吞吐量（无论与该IP之间建立了多少个连接）。 为提高效率，真正的带宽可能最终会略高于设定值。 带宽是按1KB单位分配。设定值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>可禁用限制。 每个客户端的带宽限制（字节/秒）可以在服务器或虚拟主机级别设置。 虚拟主机级别的设置将覆盖服务器级别的设置。', ' 受信任的IP或子网不受影响。', '非负整数', '');

$_tipsdb['inMemBufSize'] = new DAttrHelp("内存读写缓冲区大小", '指定用于存储请求内容和相应的动态响应的最大缓冲区大小。达到此限制时， 服务器将在&quot;交换目录&quot;中创建临时交换文件。', ' 设置足够大的缓冲区，以容纳所有并发 请求/响应，避免内存和磁盘数据交换。如果交换目录（默认为/tmp/lshttpd/swap/）存在频繁的读写活动，说明缓冲区太小，LiteSpeed正在使用交换文件。', '非负整数', '');

$_tipsdb['indexFiles'] = new DAttrHelp("索引文件", '指定URL映射到目录时按顺序查找的索引文件名称。 您可以在服务器、虚拟主机和上下文级别对其进行自定义。', ' 只设置你需要的索引文件。', '以逗号分隔的索引文件名列表。', '');

$_tipsdb['indexUseServer'] = new DAttrHelp("使用服务器索引文件", '指定是否使用服务器的索引文件。 如果设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>，那么只有服务器的设置将被使用。 如果设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>，那么服务器的设置将不会被使用。 如果设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Addition</span>，那么附加的索引文件可以被添加到此虚拟主机服务器的索引文件列表中。 如果想要禁用此虚拟主机的索引文件，您可以将该值设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>，并将索引文件栏留空。', '', '从下拉列表选择', '');

$_tipsdb['initTimeout'] = new DAttrHelp("初始请求超时时间（秒）", '指定服务器等待外部应用响应新建立连接上的第一个请求的最大时间（秒）。 如果服务器在这个限制时间内没有收到外部应用的任何数据，它将把这个连接标记为坏(Bad)。这有助于识别与外部应用程序的通信问题。 这有助于尽快发现与外部应用的通信问题。如果某些请求的处理时间较长，则增加这个限制以避免503错误信息。', '', '秒数整数', '');
...
$_tipsdb['instances'] = new DAttrHelp("实例数", '指定服务器创建的外部应用程序最大实例数。启用&quot;自动启动&quot;时，此项为必填。 大多数FastCGI/LSAPI应用程序每个进程实例只能处理一个请求。对于这类应用，实例数应与&quot;最大连接数&quot;的值匹配。 有些FastCGI/LSAPI应用程序可以生成多个子进程来同时处理多个请求。对于这类应用，应将实例数设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1</span>，并使用环境变量控制应用可生成的子进程数量。', '', '非负整数', '');
...
$_tipsdb['javaWebAppContext'] = new DAttrHelp("Java Web应用上下文", '许多运行Java应用程序的用户也使用Servlet引擎提供静态内容。 但在这些处理上，Servlet引擎远不如LiteSpeed Web Server高效。 为了提升整体性能，可以将LiteSpeed Web Server配置为网关服务器，由它提供静态内容并将动态Java页面请求转发到Servlet引擎。<br/><br/>LiteSpeed Web Server需要定义特定上下文才能运行Java应用程序。 Java Web应用上下文会根据Java Web应用程序的配置文件（WEB-INF/web.xml）自动创建所有必需的上下文。<br/><br/>设置Java Web应用上下文时需要注意：<br/><ul> <li>设置Java Web应用上下文之前，必须先在&quot;外部应用&quot;中设置<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Servlet引擎</span>外部应用程序。</li> <li>还应为.jsp文件定义&quot;脚本处理程序&quot;。</li> <li>如果Web应用程序打包为.war文件，必须先展开该.war文件。服务器无法访问压缩归档文件。</li> <li>访问同一资源时，无论通过LiteSpeed Web Server还是Servlet引擎内置HTTP服务器，都应使用相同URL。<br/>例如，Tomcat 4.1安装在/opt/tomcat下，&quot;examples&quot; Web应用程序文件位于/opt/tomcat/webapps/examples/。 通过Tomcat内置HTTP服务器访问该应用时使用类似&quot;/examples/***&quot;的URI。 因此对应的Java Web应用上下文应配置为： &quot;URI&quot; = <span class=&quot;lst-inline-token lst-inline-token--value&quot;>/examples/</span>, &quot;位置&quot; = <span class=&quot;lst-inline-token lst-inline-token--value&quot;>/opt/tomcat/webapps/examples/</span>。</li>   </ul>', '', '', '');

$_tipsdb['javaWebApp_location'] = new DAttrHelp("位置", '指定包含此Web应用程序文件的目录。该目录应包含&quot;WEB-INF/web.xml&quot;。<br/><br/>默认值：$DOC_ROOT + &quot;URI&quot;', '', '路径', '');

$_tipsdb['jsonReports'] = new DAttrHelp("输出JSON报告", '将带有.json扩展名的其他JSON格式报告文件输出到/tmp/lshttpd目录。<br/><br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>', ' 这对应用程序开发者很有用，他们可能希望使用大多数编程语言内置的标准JSON 处理工具，将LiteSpeed状态和实时报告集成到自己的应用程序中。', '从单选框中选择', '设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>时，除常规的.status、.rtreport、.rtreport.2等报告文件外， 还会创建.status.json、.rtreport.json、.rtreport.2.json等报告文件。');

$_tipsdb['keepAliveTimeout'] = new DAttrHelp("持续连接超时时长（秒）", '指定持续连接请求的最长闲置时间。 如果在这段时间内没有接收到新的请求，该连接将被关闭。 这个设置只适用于HTTP/1.1连接。HTTP/2连接有较长的闲置时间，不受此设置影响。', ' 我们建议您将值设置得刚好足够处理单个页面 视图的所有请求。没有必要延长持续连接时间。较小的值可以减少闲置 连接，提高能力，以服务更多的用户，并防范拒绝服务攻击。<span class=&quot;lst-inline-token lst-inline-token--value&quot;>2-5</span>秒 对大多数应用是合理范围。LiteSpeed在非持续连接环境是非常高效的。', '非负整数', '');

$_tipsdb['keyFile'] = new DAttrHelp("私钥文件", 'SSL私钥文件的文件名。密钥文件不应加密。', ' 私钥文件应放在安全目录中，并且只允许服务器运行用户读取。', '文件路径', '');
...
$_tipsdb['listenerIP'] = new DAttrHelp("IP地址", '指定此侦听器的IP地址。所有可用IP地址都会列出。IPv6地址会包含在&quot;[ ]&quot;中。<br/><br/>要侦听所有IPv4地址，请选择<span class=&quot;lst-inline-token lst-inline-token--value&quot;>ANY</span>。要侦听所有IPv4和IPv6地址，请选择 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>[ANY]</span>.<br/><br/>为了同时为IPv4和IPv6客户端提供服务，应使用IPv4映射的IPv6地址代替纯IPv4地址。  IPv4映射的IPv6地址应写为[::FFFF:x.x.x.x]。', ' 如果您的计算机在不同的子网中具有多个IP, 您可以选择特定的IP以仅允许来自相应子网的流量。', '从下拉列表选择', '');

$_tipsdb['listenerModules'] = new DAttrHelp("监听器模块", '侦听器模块配置数据默认从服务器模块配置继承。 侦听器模块仅限于TCP/IP第4层Hook。', '', '', '');

$_tipsdb['listenerName'] = new DAttrHelp("监听器名称", '此侦听器的唯一名称。', '', '文本', '');
...
$_tipsdb['listenerSecure'] = new DAttrHelp("加密连接", '指定这是否是安全（SSL）侦听器。 对于安全的侦听器，需要正确设置其他SSL设置。', '', '布尔值', '');
...
$_tipsdb['lname'] = new DAttrHelp("名称 - 侦听器", '用于识别此侦听器的唯一名称。 这是设置侦听器时指定的&quot;监听器名称&quot;。', '', '', '');
...
$_tipsdb['log_debugLevel'] = new DAttrHelp("调试级别", '指定调试日志级别。 要使用此功能，&quot;日志级别&quot;必须设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>。 当“调试级别”设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>时，即使&quot;日志级别&quot; 设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>，调试日志也会被禁用。 &quot;调试日志开关&quot;可用于在运行中的服务器上 控制调试级别而无需重启。', ' 重要！如果你不需要详细的调试日志记录， 应始终将其设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>。启用调试日志记录将严重降低服务性能 ，且可能在很短时间内耗尽磁盘空间。 调试日志记录包括每个请求和响应的详细信息。<br/><br/>我们推荐将日志级别设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>，调试级别设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>。 这些设置意味着调试日志不会塞满磁盘， 但仍可使用&quot;调试日志开关&quot; 控制调试输出。此操作可以实时启用或关闭调试记录， 对于调试繁忙的生产服务器非常有用。', '从下拉列表选择', '');
...
$_tipsdb['log_fileName'] = new DAttrHelp("文件名", '指定日志文件的路径。', ' 将日志文件放置在一个单独的磁盘上。', '文件路径', '');
...
$_tipsdb['log_logLevel'] = new DAttrHelp("日志级别", '指定日志文件中记录的日志级别。 可用级别（由高到低）为: <span class=&quot;lst-inline-token lst-inline-token--value&quot;>ERROR</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>WARNING</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NOTICE</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>INFO</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>。 只有级别与当前设置相同或更高的消息将被记录（级别越低记录越详细）。', ' 使用<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>日志级别本身不会影响性能， 除非&quot;调试级别&quot;设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>以外的级别。建议将日志级别设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>， 并将调试级别设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>。这样不会让调试日志填满磁盘， 但仍可使用&quot;调试日志开关&quot;控制调试输出。此操作可以实时启用或关闭调试记录， 对于调试繁忙的生产服务器非常有用。', '从下拉列表选择', '');
...
$_tipsdb['lstatus'] = new DAttrHelp("状态 - 侦听器", '此侦听器的当前状态。状态为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>运行中</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>错误</span>。', '如果侦听器处于<span class=&quot;lst-inline-token lst-inline-token--value&quot;>错误</span>状态，可以查看服务器日志了解原因。', '', '');

$_tipsdb['mappedListeners'] = new DAttrHelp("映射的监听器", '指定此模板映射到的所有侦听器名称。 此模板成员虚拟主机的侦听器到虚拟主机映射会添加到此字段指定的侦听器。 该映射会根据成员虚拟主机各自配置中的域名和别名，将侦听器映射到虚拟主机。', '', '逗号分隔列表', '');

$_tipsdb['maxCGIInstances'] = new DAttrHelp("最大CGI实例数量", '指定服务器可以启动的CGI进程最大并发数量。 对于每个对CGI脚本的请求，服务器需要启动一个独立的CGI进程。 在Unix系统中，并发进程的数量是有限的。过多的并发进程会降 低整个系统的性能，也是一种进行拒绝服务攻击的方法。 LiteSpeed服务器将对CGI脚本的请求放入管道队列，限制并发 CGI进程数量，以确保最优性能和可靠性。 硬限制为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>2000</span>。', ' 更高的数量并不一定转化为更快的性能。 在大多数情况下，更低的数量提供更好的性能和安全性。更高的数量 只在CGI处理过程中读写延迟过高时有帮助。', '非负整数', '');

$_tipsdb['maxCachedFileSize'] = new DAttrHelp("最大缓存小文件大小（字节）", '指定预分配内存缓冲区中缓存的静态文件最大尺寸。静态文件 可以用四种不同的方式服务：内存缓存、内存映射缓存、直接读写和 sendfile()。 尺寸小于&quot;最大缓存小文件大小（字节）&quot;的文件将使用内存缓存服务。尺寸大于该限制、但小于 &quot;最大MMAP文件大小（字节）&quot;的文件，将使用内存映射缓存服务。 尺寸大于&quot;最大MMAP文件大小（字节）&quot;的文件将通过直接读写或sendfile() 服务。使用内存缓存服务小于4K的文件是最佳做法。', '', '非负整数', '');
...
$_tipsdb['maxConns'] = new DAttrHelp("最大连接数", '指定服务器与外部应用之间可建立的最大并发连接数。该设置控制外部应用程序可同时处理多少个请求。 然而，真正的限制还取决于外部应用本身。如果外部的速度不够快，或者无法处理大量的并发请求，那么将这个值设置得更高也无济于事。', ' 设置一个高值并不能直接变成高性能。设置为一个不会使外部应用程序过载的值，将提供最佳的性能/吞吐量。', '非负整数', '');

$_tipsdb['maxDynRespHeaderSize'] = new DAttrHelp("动态回应报头最大大小(bytes)", '指定动态回应的最大报头大小。硬限制为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>131072</span>字节或128K.<br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32768</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32K</span>', ' 设置一个合理的低值以帮助识别外部应用程序产生的 坏的动态回应。', '非负整数', '');

$_tipsdb['maxDynRespSize'] = new DAttrHelp("最大动态响应主体大小(bytes)", '指定动态回应的最大主内容尺寸。硬限制是<span class=&quot;lst-inline-token lst-inline-token--value&quot;>2047MB</span>。', ' 设置一个合理的低值以帮助识别坏的响应。恶意脚本经常包含 无限循环而导致大尺寸回应。', '非负整数', '');

$_tipsdb['maxKeepAliveReq'] = new DAttrHelp("最大持续连接请求数", '指定通过持续连接（持久）会话处理的请求的最大数量。一旦达 到此限制，连接将被关闭。您也可以为每个虚拟主机单独设置限制。', ' 设置为合理的较高的值。值为“1”或“0”时将禁用持续连接。', '非负整数', '');

$_tipsdb['maxMMapFileSize'] = new DAttrHelp("最大MMAP文件大小（字节）", '指定使用内存映射（MMAP）的最大静态文件大小。 静态文件可以用四种不同的方式服务：内存缓存、内存映射缓存、直接读写和 sendfile()。 尺寸小于&quot;最大缓存小文件大小（字节）&quot;的文件将使用内存缓存服务。尺寸大于该限制、但小于 &quot;最大MMAP文件大小（字节）&quot;的文件，将使用内存映射缓存服务。 尺寸大于&quot;最大MMAP文件大小（字节）&quot;的文件将通过直接读写或sendfile() 服务。 由于服务器有一个32位的地址空间（2GB），不建议使用内存映射非常大的文件。', '', '非负整数', '');
...
$_tipsdb['maxReqBodySize'] = new DAttrHelp("最大请求主内容大小（字节）", '指定HTTP请求主内容最大尺寸。对于32位操作系统， 硬限制为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>2GB</span>。对于64位操作系统，几乎是无限的。', ' 为了防止拒绝服务攻击，尽量将限制值设定到实际需求的大小。 交换空间的剩余空间必须比这个限制值大。', '非负整数', '');

$_tipsdb['maxReqHeaderSize'] = new DAttrHelp("最大请求头大小（字节）", '指定请求URL中包含的HTTP请求头最大值。 硬限制为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>131072</span> 字节或128K.<br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32768</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>32K</span>', ' 设置合理的低值来减少内存的使用并帮助识别虚假请求和拒绝服务攻击。<br/>对于大多数网站来说4000-8000已经足够大。', '非负整数', '');

$_tipsdb['maxReqURLLen'] = new DAttrHelp("最大请求URL长度（字节）", '指定请求URL的最大大小。 URL是一个纯文本的地址，包含查询字符串来请求服务器上的资源。 硬限制为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>65530</span>个字节。 大于此值的值，例如<span class=&quot;lst-inline-token lst-inline-token--value&quot;>64K</span>（大6个字节），将被视为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>65530</span>。<br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>8192</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>8K</span>。', ' 将其设置合理的低值来以减少内存使用 并帮助识别虚假请求和拒绝服务攻击。<br/>对大多数网站2000-3000已经足够大，除非使用HTTP GET而不是POST来提交大型的查询字符串。', '非负整数', '');
...
$_tipsdb['memHardLimit'] = new DAttrHelp("内存硬限制（字节）", '与&quot;内存软限制（字节）&quot;非常相同，但是在一个用户进程中，软限制 可以被放宽到硬限制的数值。硬限制可以在服务器级别或独立的外部应用程序级别设 置。如果未在独立的外部应用程序级别设定限制，将使用服务器级别的限制。<br/>如果在两个级别都没有设置该限制，或者限制值设为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>，将使用操 作系统的默认设置。', ' 不要过度调整这个限制。如果您的应用程序需要更多的内存， 这可能会导致503错误。', '非负整数', '');

$_tipsdb['memSoftLimit'] = new DAttrHelp("内存软限制（字节）", '以字节为单位指定服务器启动的外部应用进程或程序的内存占用限制。<br/>此限制的目的主要是为了防范软件缺陷或蓄意攻击造成的过度内存使用， 而不是限制正常使用。确保留有足够的内存，否则您的应用程序可能故障并 返回503错误。限制可以在服务器级别或独立的外部应用程序级别设置。如 果未在独立的外部应用程序级别设定限制，将使用服务器级别的限制。<br/>如果在两个级别都没有设置该限制，或者限制值设为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>，将使用操 作系统的默认设置。', ' 不要过度调整这个限制。如果您的应用程序需要更多的内存， 这可能会导致503错误。', '非负整数', '');
...
$_tipsdb['minGID'] = new DAttrHelp("最小的GID", '指定外部应用程序的最小组ID。 如果组ID比这里指定的值更小，其外部脚本的执行将被拒绝。 如果的LiteSpeed Web服务器是由“Root”用户启动，它可以在“suEXEC” 模式运行外部应用程序，类似Apache（可以切换到与Web服务器不同的用户/组ID）。', ' 设置足够高的值以排除所有系统用户所属的组。', '非负整数', '');

$_tipsdb['minUID'] = new DAttrHelp("最小的UID", '指定外部应用程序的最小用户ID。 如果用户ID比这里指定的值更低。其外部脚本的执行将被拒绝。 如果的LiteSpeed Web服务器由“Root”用户启动，它可以在“suEXEC” 模式运行外部应用程序，类似Apache（可以切换到与Web服务器不同的用户/组ID）。', ' 设置足够高的值以排除所有系统/特权用户。', '非负整数', '');
...
$_tipsdb['moduleEnabled'] = new DAttrHelp("启用模块", '全局启用模块Hook。此设置可在侦听器和虚拟主机级别覆盖。<br/><br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>', '', '布尔值', '');
...
$_tipsdb['nodeBin'] = new DAttrHelp("Node路径", 'Node.js可执行文件的路径。', '', '绝对路径', '');
...
$_tipsdb['opsAuditRetainFiles'] = new DAttrHelp("活动日志保留文件数", '指定WebAdmin控制台要保留的操作审计文件最大数量。活动日志文件达到5 MB或每天会轮转。未设置时默认值：30个文件。', '如果需要更长的审计轨迹，可以增大此值。此保留策略按文件数计算，因此活动量较高时覆盖的天数可能较少；更多文件会占用更多磁盘空间。', '整数', '');

$_tipsdb['outBandwidth'] = new DAttrHelp("出口带宽（字节/秒）", '指定对单个IP地址允许的最大传出吞吐量（无论与该IP之间建立了多少个连接）。 为提高效率，真正的带宽可能最终会略高于设定值。 带宽按4KB为单位分配。设定值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>可禁用限制。 每个客户端的带宽限制（字节/秒）可以在服务器或虚拟主机级别设置。 虚拟主机级别的设置将覆盖服务器级别的设置。', ' 按8KB单位设置带宽可获得更好的性能。<br/> 受信任的IP或子网不受影响。', '非负整数', '');

$_tipsdb['pcKeepAliveTimeout'] = new DAttrHelp("连接持续连接超时时长", '指定空闲持久连接保持打开的最大时间（秒）。<br/><br/>设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>-1</span>时，连接永不超时。设置为0或更大时，连接会在经过该秒数后关闭。', '', '秒数整数', '');

$_tipsdb['perClientConnLimit'] = new DAttrHelp("按客户端限速", '这些是基于客户端IP的连接控制设置。 这些设置有助于缓解DoS（拒绝服务）和DDoS（分布式拒绝服务）攻击。', '', '', '');
...
$_tipsdb['quicBasePLPMTU'] = new DAttrHelp("PLPMTU基础值", 'QUIC默认使用的PLPMTU (无报头的最大数据包大小,以字节为单位)的默认值. 设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>将会允许QUIC设置大小.<br/>这个设置必须低于 &quot;PLPMTU最大值&quot; 的值.<br/>默认值：0', '', '0或1200至65527之间的整数', '');

$_tipsdb['quicCfcw'] = new DAttrHelp("连接流量控制窗口", '为QUIC连接分配的缓冲区的初始大小。 默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;> 1.5M </span>。', '更大的窗口大小将使用更多的内存。', '64K到512M之间的数字', '');

$_tipsdb['quicCongestionCtrl'] = new DAttrHelp("拥塞控制", '使用的拥塞控制算法。 可以手动设置，也可以选择“默认”选项，将其保留到的QUIC库中。<br/>默认值：Default', '', '从下拉列表选择', '');

$_tipsdb['quicEnable'] = new DAttrHelp("启用HTTP3/QUIC", '在整个服务器范围内启用HTTP3/QUIC网络协议。 默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。', '当此设置设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>时，仍然可以通过&quot;开放HTTP3/QUIC（UDP）端口&quot;设置在侦听器级别 或通过&quot;启用HTTP3/QUIC&quot;设置在虚拟主机级别禁用HTTP3/QUIC', '布尔值', '');
...
$_tipsdb['quicMaxCfcw'] = new DAttrHelp("最大连接流量控制窗口", '指定连接流控制窗口缓冲区通过自动调优可以达到的最大大小。<br/>默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;> 0 </span>，这意味着将使用&quot;连接流量控制窗口&quot;的值，并且不会自动调整。', '更大的窗口大小将使用更多的内存。', '0或64K到512M之间的数字', '');

$_tipsdb['quicMaxPLPMTU'] = new DAttrHelp("PLPMTU最大值", 'PLPMTU（不含报头的最大数据包大小，以字节为单位）的探测上限。此设置用于限制 DPLPMTUD搜索范围中的“最大数据包大小”。设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>将允许QUIC自行选择大小。 (默认情况下LSQUIC暂定MTU为1,500 字节 (以太网)).<br/>这个设置应该比&quot;PLPMTU基础值&quot;的值高.<br/>默认值：0', '', '0或1200至65527之间的整数', '');

$_tipsdb['quicMaxSfcw'] = new DAttrHelp("最大流控制窗口", '指定由于自动调整而允许流控制窗口达到的最大大小。<br/>默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>，这意味着将使用&quot;流控制窗口&quot;的值，并且不会自动调整', '更大的窗口大小将使用更多的内存。', '0或64K到128M之间的数字', '');

$_tipsdb['quicMaxStreams'] = new DAttrHelp("每个连接的最大并发流数", '每个QUIC连接的最大并发数。 默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>100</span>。', '', '10到1000之间的整数', '');

$_tipsdb['quicSfcw'] = new DAttrHelp("流控制窗口", 'QUIC愿意为每个流接收的初始数据量。 默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>1M</span>。', '更大的窗口大小将使用更多的内存。', '64K到128M之间的数字', '');
...
$_tipsdb['rcvBufSize'] = new DAttrHelp("接收缓冲区大小（字节）", '每个TCP套接字的接收缓冲区大小。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>512K</span>是允许的最大缓冲区大小。', ' 建议将此值保留为“未设置”或设置为0以使用操作系统的默认缓冲区大小。<br/> 处理大载荷入站请求，如文件上传时，大的接收缓冲区会提高性能。<br/> 将此值设置为较低的值将减少吞吐量和每个套接字的内存使用率，从而在内存成为瓶颈时允许服务器拥有更多并发套接字。', '非负整数', '');

$_tipsdb['realm'] = new DAttrHelp("领域", '指定此上下文的授权域。 指定后，必须提供有效的用户名和密码才能访问此上下文。 &quot;授权Realm&quot;需要在&quot;虚拟主机安全&quot;部分进行设置。 此设置使用每个授权域的&quot;领域名称&quot;。', '', '从下拉列表选择', '');

$_tipsdb['realmName'] = new DAttrHelp("领域名称", '指定授权Realm的唯一名称。', '', '文本', '');
...
$_tipsdb['realtimerpt'] = new DAttrHelp("实时统计", '实时统计链接会打开实时服务器状态报告页面。 这是监控系统的便捷工具。<br/><br/>该报告显示服务器统计信息的快照。此快照的刷新速率由右上角的刷新间隔下拉列表控制。<br/><br/>报告包含以下部分： <ul><li>服务器健康状态显示基本服务器统计、运行时间、负载以及被anti-DDoS阻止的IP。</li>   <li>服务器列出当前流量吞吐量、连接和请求统计。</li>  <li>虚拟主机显示每个虚拟主机的请求处理状态和外部应用程序状态。</li>  <li>外部应用程序列出当前运行的外部应用程序及其使用统计。  CGI守护进程lscgid始终作为外部应用程序运行。</li> </ul><br/><br/>实时统计中的许多行带有图表图标。 点击此图标会打开该行统计数据的实时更新图表。<br/><br/>在“服务器”部分的“请求”旁边，有一个标记为（详细信息）的链接。 该链接会进入请求快照页面，你可以查看哪些客户端正在发出特定类型的请求， 或站点的哪些方面成为瓶颈。蓝色区域中的字段可用于筛选快照， 以隔离服务器的特定部分，或查找正在执行特定操作的客户端。', '', '', '');

$_tipsdb['recaptchaAllowedRobotHits'] = new DAttrHelp("允许的机器人请求次数", '设置每10秒允许“好机器人”通过的点击次数。 当服务器处于高负载状态时，僵尸程序仍会受到限制。<br/>默认值是<span class=&quot;lst-inline-token lst-inline-token--value&quot;>3</span>.', '', '非负整数', '');

$_tipsdb['recaptchaBotWhiteList'] = new DAttrHelp("机器人白名单", '自定义允许访问的用户代理列表。 将受到“好机器人”的限制，包括allowedRobotHits。', '', '用户代理列表，每行一个。 支持正则表达式。', '');
...
$_tipsdb['recaptchaSiteKey'] = new DAttrHelp("站点密钥", 'reCAPTCHA或hCaptcha服务分配的公钥。<br/>使用reCAPTCHA时，如果未设置站点密钥，将使用默认Site Key（不推荐）。', '', '文本', '');
...
$_tipsdb['recaptchaType'] = new DAttrHelp("reCAPTCHA类型", '指定与密钥对一起使用的CAPTCHA类型。<br/>如果未提供密钥对，且此设置设为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>未设置</span>，则会使用<span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Invisible</span>类型的默认CAPTCHA密钥对。<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Checkbox</span>会向访问者显示复选框CAPTCHA以进行验证。 （需要&quot;站点密钥&quot;、&quot;密钥&quot;）<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Invisible</span>会尝试自动验证CAPTCHA，验证成功后会重定向到目标页面。 （需要&quot;站点密钥&quot;、&quot;密钥&quot;）<br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>hCaptcha</span>可用于支持CAPTCHA提供商 <a href="https://www.hcaptcha.com" target="_blank" rel="noopener noreferrer">hCaptcha</a>。 （需要&quot;站点密钥&quot;、&quot;密钥&quot;）<br/>默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>reCAPTCHA Invisible</span>。', '', '从列表中选择', '');
...
$_tipsdb['renegProtection'] = new DAttrHelp("SSL重新协商保护", '指定是否启用SSL密钥重新协商保护，以 防御基于SSL握手的攻击。默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。', '可以在侦听器和虚拟主机级别启用此设置。', '布尔值', '');

$_tipsdb['required'] = new DAttrHelp("要求（已认证的用户/组）", '指定哪些用户/用户组可以访问此上下文。 这允许你在多个上下文中使用同一个用户/组数据库（由&quot;领域&quot;指定）， 但只允许该数据库中的特定用户/组访问此上下文。', '', '语法与Apache的Require指令兼容。例如： <ul> <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>user username [username ...]</span><br/>只有列出的用户可以访问此上下文。</li> <li> <span class=&quot;lst-inline-token lst-inline-token--value&quot;>group groupid [groupid ...]</span><br/>只有属于列出组的用户可以访问此上下文。</li> </ul> 如果未指定此设置，所有有效用户都可以访问此资源。', '');
...
$_tipsdb['restart'] = new DAttrHelp("应用更改/平滑重启", '点击<span class=&quot;lst-inline-token lst-inline-token--value&quot;>平滑重启</span>后，将启动新的服务器进程。 对于<span class=&quot;lst-inline-token lst-inline-token--value&quot;>平滑重启</span>，旧服务器进程只会在所有请求处理完成后 （或达到&quot;平滑重启超时时长（秒）&quot;限制后）退出。<br/><br/>配置更改会在下一次重启时应用。 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>平滑重启</span>会在不中断服务器服务的情况下应用这些更改。', '平滑重启生成新服务器进程通常少于2秒。', '', '');

$_tipsdb['restrained'] = new DAttrHelp("访问管制", '指定是否可以通过此网站访问虚拟主机根目录（$VH_ROOT）之外的文件。 如果设置是<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>，只可以访问$VH_ROOT下的文件， 访问指向$VH_ROOT以外文件或目录的符号链接或Context都将被阻止。 尽管如此，这里不会限制CGI脚本的访问。 这个选项在共享主机下非常有用。 &quot;跟随符号链接&quot;可以设置成<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>来允许用户使用在$VH_ROOT下的符号链接， $VH_ROOT以外的则不可以。', ' 在共享主机环境下打开该功能。', '布尔值', '');
...
$_tipsdb['retryTimeout'] = new DAttrHelp("重试超时时间（秒）", '指定服务器在重试之前，等待出现通信问题的外部应用程序的时间。', '', '秒数整数', '');
...
$_tipsdb['runOnStartUp'] = new DAttrHelp("开机自启", '指定是否在服务器启动时启动外部应用程序。 只适用于可以管理自己子进程的外部应用程序，并且&quot;实例数&quot;值设置为&quot;1&quot;。<br/><br/>如果启用，将在服务器启动时而不是运行时创建外部进程。<br/><br/>选择“Yes (Detached mode)”时，可以通过分别触碰$SERVER_ROOT/admin/tmp/或$VH_ROOT/目录下的 &#039;.lsphp_restart.txt&#039;文件，在服务器级别或虚拟主机级别重新启动所有分离模式进程。<br/><br/>默认值：Yes (Detached mode)', ' 如果配置的外部进程有很大的启动开销（例如Rails），应启用此选项以减少首页响应时间。', '布尔值', '');

$_tipsdb['runningAs'] = new DAttrHelp("以此身份运行", '指定服务器进程运行所使用的用户/组。该值在安装前运行configure命令时，通过“--with-user”和“--with-group”参数设置。要重置这些值，必须重新运行configure命令并重新安装。', ' 服务器不应以特权用户（如“root”）运行。 请务必将服务器配置为以没有登录/shell访问权限的非特权用户/组组合运行。通常，用户/组使用<span class=&quot;lst-inline-token lst-inline-token--value&quot;>nobody</span>是不错的选择。', '', '');

$_tipsdb['scgiContext'] = new DAttrHelp("SCGI上下文", 'SCGI应用程序不能直接使用。必须将SCGI应用程序配置为脚本处理程序，或通过SCGI上下文映射到URL。 SCGI上下文会将URI关联到SCGI应用程序。', '', '', '');

$_tipsdb['scgiapp'] = new DAttrHelp("SCGI应用程序", '指定SCGI应用程序的名称。此应用程序必须在服务器或虚拟主机级别的&quot;外部应用&quot;部分中定义。', '', '从下拉列表选择', '');

$_tipsdb['servAction'] = new DAttrHelp("操作", '此菜单提供六个操作：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>平滑重启</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>调试日志开关</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>服务器日志查看器</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>实时统计</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>版本管理器</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>编译LSPHP</span>。 <ul><li>&quot;应用更改/平滑重启&quot;会平滑重启服务器进程，不中断正在处理的请求。</li> 	<li>&quot;调试日志开关&quot;可开启或关闭调试日志。</li> 	<li>&quot;服务器日志查看器&quot;可通过日志查看器查看服务器日志。</li> 	<li>&quot;实时统计&quot;可查看实时服务器状态。</li> 	<li>&quot;版本管理&quot;可下载LSWS的新版本，并在不同版本之间切换。</li> 	<li>编译LSPHP可为LiteSpeed Web Server编译PHP。</li> </ul>', '也可以使用shell工具<span class=&quot;lst-inline-token lst-inline-token--command&quot;>$SERVER_ROOT/bin/lswsctrl</span>控制服务器进程， 但这需要登录shell。', '', '');
...
$_tipsdb['serverPriority'] = new DAttrHelp("优先级", '指定服务进程的优先级。数值范围从 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>-20</span> 到 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>20</span>。数值越小，优先级越高。', ' 通常，较高的优先级会导致繁忙的服务器上的Web性能稍有提高。 不要将优先级设置为高于数据库进程的优先级。', '整数', '');
...
$_tipsdb['setUidMode'] = new DAttrHelp("外部应用程序设置UID模式", '指定如何为外部应用程序进程设置用户ID。可以选择下面三种方式： <ul><li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Server UID</span>: 为外部应用程序设置与服务器用户/组ID相同的用户/组ID。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI File UID</span>: 为外部应用CGI程序设置基于可执行文件的用户/组ID。该选项仅适用于CGI，不适用于FastCGI或LSPHP。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>Doc Root UID</span>: 为外部应用程序设置基于当前虚拟主机文档根目录的用户/组ID。</li> </ul><br/><br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Server UID</span>', ' 在共享主机环境中，建议使用<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI File UID</span> 或 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>Doc Root UID</span>模式来防止一个虚拟主机下的文件被另一个虚拟主机的外部应用程序访问。', '从下拉列表选择', '');
...
$_tipsdb['shType'] = new DAttrHelp("处理器类型", '指定处理这些脚本文件的外部应用程序类型。可用类型包括： <span class=&quot;lst-inline-token lst-inline-token--value&quot;>LSAPI app</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>网络服务器</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Fast CGI</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>SCGI</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Load balancer</span>、 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>Servlet引擎</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>uWSGI</span>或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>模块处理器</span>。除<span class=&quot;lst-inline-token lst-inline-token--value&quot;>CGI</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>模块处理器</span> 处理器类型外，&quot;处理器名称&quot;还必须设置为&quot;外部应用&quot;部分中已预先定义的外部应用程序。', '', '从列表中选择', '');

$_tipsdb['shmDefaultDir'] = new DAttrHelp("默认SHM目录", '将共享内存的默认目录更改为指定的路径。 如果该目录不存在，则将创建该目录。除非另有说明，否则所有SHM数据都将存储在此目录中。', '', 'path', '');

$_tipsdb['showVersionNumber'] = new DAttrHelp("服务器签名", '指定是否在响应头的Server参数中显示服务器签名和版本号。 有三个选项：当设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>隐藏版本</span>时，只显示<span class=&quot;lst-inline-token lst-inline-token--value&quot;>LiteSpeed</span>。当设置为 <span class=&quot;lst-inline-token lst-inline-token--value&quot;>显示版本</span>，显示LiteSpeed和版本号。  设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>隐藏全部头</span>时，整个Server头都不会显示在响应报头中。', ' 如果你不想暴露服务器的版本号，设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>隐藏版本</span>。', '从下拉列表选择', '');

$_tipsdb['sname'] = new DAttrHelp("名称 - 服务器", '用于识别此服务器的唯一名称。 这是在常规配置中指定的&quot;服务器名称&quot;。', '', '', '');

$_tipsdb['sndBufSize'] = new DAttrHelp("发送缓冲区大小（字节）", '每个TCP套接字的发送缓冲区的大小。设定值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>0</span>使用 操作系统默认的缓冲区大小。<span class=&quot;lst-inline-token lst-inline-token--value&quot;>65535</span>是允许的最大缓冲区大小。', ' 建议将此值保留为“未设置”或设置为0以使用操作系统的默认缓冲区大小。<br/> 如果您的网站服务大量的静态文件，增加发送缓冲区 大小来提高性能。<br/> 将此值设置为较低的值将减少吞吐量和每个套接字的内存使用率，从而在内存成为瓶颈时允许服务器拥有更多并发套接字。', '非负整数', '');

$_tipsdb['softLimit'] = new DAttrHelp("连接软限制", '指定来自单个IP的并发连接的软限制。 并发连接数低于&quot;连接数硬限制&quot;时，此软限制可以在&quot;宽限期（秒）&quot;期间临时超过， 但Keep-Alive连接将被尽快断开，直到连接数低于软限制。 如果&quot;宽限期（秒）&quot;之后，连接数仍然超过软限制，相应的IP将被封锁 &quot;封禁时长（秒）&quot;所设置的时长。<br/>例如，如果页面包含许多小图像，浏览器可能会尝试同时建立许多连接，尤其是HTTP/1.0客户端。你应当在短时间内允许这些连接。<br/>HTTP/1.1客户端还可能建立多个连接，以加快下载，另外SSL需要为非SSL连接建立单独的连接。确保限制设置正确， 以免影响正常服务。建议限制在<span class=&quot;lst-inline-token lst-inline-token--value&quot;>5</span>与<span class=&quot;lst-inline-token lst-inline-token--value&quot;>10</span>之间。', ' 较低的数值可让服务器服务更多不同的客户端。<br/> 受信任的IP或子网不受影响。<br/> 使用大量并发客户端进行性能评测时，请设置一个较高的值。', '非负整数', '');
...
$_tipsdb['sslOCSP'] = new DAttrHelp("OCSP Stapling", '在线证书状态协议（OCSP）是更加有效的检查数字证书是否有效的方式。 它通过与另一台服务器（OCSP响应服务器）通信，以获取证书有效的验证，而不是通过证书吊销列表（CRL）进行检查。<br/><br/>OCSP装订是对该协议的进一步改进，允许服务器以固定的时间间隔而不是每次请求证书时与OCSP响应程序进行检查。 有关更多详细信息，请参见<a href="   https://zh.wikipedia.org/wiki/%E5%9C%A8%E7%BA%BF%E8%AF%81%E4%B9%A6%E7%8A%B6%E6%80%81%E5%8D%8F%E8%AE%AE " target="_blank" rel="noopener noreferrer">   OCSP Wikipedia页面 </a>。', '', '', '');
...
$_tipsdb['sslSessionCacheSize'] = new DAttrHelp("会话缓存大小（字节）", '设置要存储在缓存中的SSL会话ID的最大数量。 默认值为1,000,000。', '', '整数', '');

$_tipsdb['sslSessionCacheTimeout'] = new DAttrHelp("会话缓存超时（秒）", '此值确定需要重新握手之前，会话ID在缓存中有效的时间。 默认值为 3,600.', '', '整数', '');

$_tipsdb['sslSessionTicketKeyFile'] = new DAttrHelp("SSL会话票据密钥文件", '允许管理员创建和维护SSL票证密钥。该文件长度必须为48字节。 如果此选项留空，负载均衡器将生成并轮换自己的密钥集。<br/>重要：为保持前向保密，强烈建议每隔<b>SSL会话票证生存时间</b>秒更换密钥。 如果无法做到，建议将此字段留空。', '', '路径', '');

$_tipsdb['sslSessionTicketLifetime'] = new DAttrHelp("SSL会话票据有效期（秒）", '此值确定需要重新握手之前会话记录单有效的时间。 默认值为3600。', '', '整数', '');

$_tipsdb['sslSessionTickets'] = new DAttrHelp("启用会话票据", '使用OpenSSL的默认会话票证设置启用会话记录单。 服务器级别设置必须设置为“是”才能使虚拟主机设置生效。<br/>默认值:<br/><b>服务器级别:</b> Yes<br/><b>虚拟主机级别:</b> Yes', '', '布尔值', '');

$_tipsdb['sslStrictSni'] = new DAttrHelp("严格SNI证书", '指定是否严格要求专用的虚拟主机证书配置。启用后，没有专用证书配置的虚拟主机SSL连接将失败， 而不会使用默认的兜底证书。<br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>', '', '从单选框中选择', '');

$_tipsdb['sslStrongDhKey'] = new DAttrHelp("强SSL DH密钥", '指定是使用2048位还是1024位DH密钥进行SSL握手。 如果设置为“是”，则2048位DH密钥将用于2048位SSL密钥和证书。 在其他情况下，仍将使用1024位DH密钥。 默认值为“是”。 Java的早期版本不支持大于1024位的DH密钥大小。 如果需要Java客户端兼容性，则应将其设置为“否”。', '', '从单选按钮选择', '');
...
$_tipsdb['staticReqPerSec'] = new DAttrHelp("静态请求数/秒", '指定每秒可处理的来自单个IP的静态内容请求数量（无论与该IP之间建立了多少个连接）。<br/>当达到此限制时，所有后来的请求将被延滞到下一秒。 对于动态内容请求的限制与本限制无关。 每个客户端的请求限制可以在服务器或虚拟主机级别设置。 虚拟主机级别的设置将覆盖服务器级别的设置。', ' 受信任的IP或子网不受影响。', '非负整数', '');

$_tipsdb['statuscode'] = new DAttrHelp("状态码", '指定外部重定向响应状态码。 如果状态码在300和399之间，可以指定&quot;目标URI&quot;。', '', '从下拉列表选择', '');

$_tipsdb['suexecGroup'] = new DAttrHelp("suEXEC用户组", '在当前Context级别，以该组身份运行。 必须设置虚拟主机级别的<b>suEXEC用户</b>或外部应用程序级别的<b>以用户身份运行</b>，<b>suEXEC组</b>才会生效。<br/><br/>可以使用<b>运行方式组</b>设置在外部应用程序级别覆盖此配置。<br/>默认值：<b>suEXEC用户</b>设置值', '', '有效组名或GID', '');
...
$_tipsdb['swappingDir'] = new DAttrHelp("交换目录", '指定交换文件的存放目录。 服务器在chroot模式启动时，该路径相对于新的根目录， 否则，它相对于真正的根目录。<br/><br/>LiteSpeed使用自己的虚拟内存 以降低系统的内存使用量。虚拟内存和磁盘交换会用来存储大的请求内容和 动态响应。交换目录应设置在有足够剩余空间的磁盘上。<br/><br/>默认值: /tmp/lshttpd/swap', ' 将交换目录设置在一个单独的磁盘上，或者增加最大读写缓冲区大小以避免交换。', '路径', '');
...
$_tipsdb['throttleEnabled'] = new DAttrHelp("启用登录限制", '为WebAdmin控制台启用登录限制。启用后，重复的失败登录尝试会被跟踪并临时屏蔽， 以减少暴力破解密码攻击。如果只启用此选项，而其他登录限制设置保持未设置，则会使用内置默认值。', ' 除非正在排查登录问题，否则请在生产环境中保持启用。 启用后如果未设置自定义值，默认值为失败5次、初始屏蔽900秒、最大屏蔽14400秒。', '从单选框中选择', '');
...
$_tipsdb['throttleMaxFailures'] = new DAttrHelp("最大登录失败次数", '指定客户端在被屏蔽前允许连续登录失败的次数。未设置时默认值：5。', ' 较低的值会增强保护，但也可能更快屏蔽合法用户。', '整数', '');

$_tipsdb['toggleDebugLog'] = new DAttrHelp("调试日志开关", '切换调试日志会在<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>HIGH</span>之间切换 &quot;调试级别&quot;的值。 由于调试日志会影响性能并可能快速填满硬盘， 生产服务器上的&quot;调试级别&quot;通常应设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>NONE</span>。 此功能可用于快速开启或关闭调试日志，以便在生产服务器上排查问题。 通过此方式开启或关闭调试日志不会改变服务器配置中显示的任何内容。', '只有当&quot;日志级别&quot;设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>DEBUG</span>时， &quot;调试日志开关&quot;才会生效。<br/> 重要！调试日志包含每个请求和响应的详细信息。启用调试日志会严重降低服务性能， 并可能在很短时间内耗尽磁盘空间。此功能只应在诊断服务器问题时短时间使用。', '', '');

$_tipsdb['totalInMemCacheSize'] = new DAttrHelp("小文件缓存总大小（字节）", '指定分配用于缓存/服务小静态文件的总内存。', '', '非负整数', '');

$_tipsdb['totalMMapCacheSize'] = new DAttrHelp("总MMAP缓存大小（字节）", '指定分配用于缓存/服务中等大小静态文件的总内存。', '', '非负整数', '');
...
$_tipsdb['umask'] = new DAttrHelp("权限掩码", '设置CGI进程默认的umask。 通过<span class=&quot;lst-inline-token lst-inline-token--command&quot;> man 2 umask</span>命令了解详细信息。这也可作为外部应用程序&quot;权限掩码&quot;的默认值。', '', '数值有效范围为[000] - [777]', '');

$_tipsdb['uploadPassByPath'] = new DAttrHelp("通过文件路径传递上传数据", '指定是否按文件路径传递上传数据。 如果启用，则上传时文件路径以及其他一些信息将发送到后端处理程序，而不是文件本身。 这样可以节省CPU资源和文件传输时间，但需要一些更新才能实现。 如果禁用，则文件内容将传输到后端处理程序，请求主体仍解析为文件。', ' 如果向下兼容不是问题，启用此功能可加快文件上传处理速度。', '布尔值', '');
...
$_tipsdb['useAIO'] = new DAttrHelp("使用异步 I/O (AIO)", '指定是否使用异步I/O服务静态文件，以及要使用的AIO实现。选项<span class=&quot;lst-inline-token lst-inline-token--value&quot;>LINUX AIO</span>和<span class=&quot;lst-inline-token lst-inline-token--value&quot;>io_uring</span>仅在Linux机器上可用。<br/>默认值：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>', ' 对于I/O等待较高的服务器，AIO可帮助提升性能。<br/> 选择<span class=&quot;lst-inline-token lst-inline-token--value&quot;>io_uring</span>但当前机器不支持时，将改用<span class=&quot;lst-inline-token lst-inline-token--value&quot;>Linux AIO</span>。', '从列表中选择', '');

$_tipsdb['useIpInProxyHeader'] = new DAttrHelp("使用报头中的客户端IP", '指定是否将在HTTP请求报头中的X-Forwarded-For参数列出的IP地址，用于 所有的IP地址相关的功能，包括 连接/带宽限制、访问控制和IP地理定位。<br/><br/>如果你的Web服务器放置在负载均衡器或代理服务器之后，此功能非常有用。 如果您选择了“仅限受信任的IP”，只有在请求来自受信任IP时，X-Forwarded-For 中的IP才会被使用。受信任IP可在服务器级别的&quot;允许列表&quot;中定义。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>保留来自受信任IP的头部</span>与<span class=&quot;lst-inline-token lst-inline-token--value&quot;>仅限受信任的IP</span>相同，但是用于后端的X-Forwarded-For头不会被修改为包含连接对端地址。<br/><br/><span class=&quot;lst-inline-token lst-inline-token--value&quot;>使用最后一个IP（对于AWS ELB）</span>将使用“X-Forwarded-For”列表中的最后一个IP地址。如果您正在使用AWS Elastic Load Balancer，或期望真实IP被追加到“X-Forwarded-For”列表末尾，请选择此选项。', '', '从下拉列表选择', '');

$_tipsdb['useSendfile'] = new DAttrHelp("使用sendfile()", '指定是否使用sendfile()系统调用来服务静态文件。静态文件 可以用四种不同的方式服务：内存缓存、内存映射缓存、直接读写和 sendfile()。 尺寸小于&quot;最大缓存小文件大小（字节）&quot;的文件将使用内存缓存服务。尺寸大于该限制、但小于 &quot;最大MMAP文件大小（字节）&quot;的文件，将使用内存映射缓存服务。 尺寸大于&quot;最大MMAP文件大小（字节）&quot;的文件将通过直接读写或sendfile() 服务。Sendfile()是一个“零拷贝”系统调用，可在服务非常大的 文件时大大减少CPU的使用率。Sendfile()需要一个优化的网卡内核驱动， 因此可能不适合某些小厂商的网络适配器。', '', '布尔值', '');

$_tipsdb['userDBCacheTimeout'] = new DAttrHelp("用户数据库缓存超时（秒）", '指定多久检查一次后端用户数据库变更。 在缓存中每个条目都有一个时间戳。 当缓存日期超过指定的超时时间时，将检查后端数据库是否有变化。 如果没有，时间戳将被重置为当前时间，否则会将新的数据载入。 服务器重载和平滑重启会立即清除缓存。', ' 如果后端数据库不经常发生变更，设置较长的缓存时间来获得更好的性能。', '非负整数', '');
...
$_tipsdb['userDBMaxCacheSize'] = new DAttrHelp("用户数据库最大缓存大小", '指定用户数据库的最大缓存大小。 最近访问的用户认证信息会被缓存在内存中以提供最佳性能。', ' 由于更大的缓存会消耗更多的内存，更高的值可能会也可能不会提供更好的性能。 请根据您的用户数据库大小和网站使用情况来设定一个合适的大小。', '非负整数', '');
...
$_tipsdb['uwsgiapp'] = new DAttrHelp("uWSGI应用程序", '指定uWSGI应用程序的名称。此应用程序必须在服务器或虚拟主机级别的&quot;外部应用&quot;部分中定义。', '', '从下拉列表选择', '');

$_tipsdb['vaction'] = new DAttrHelp("操作 - 虚拟主机", '此字段显示用于禁用、启用或重启虚拟主机的按钮。 对一个虚拟主机执行的操作不会影响Web服务器的其他部分。', '更新虚拟主机内容时，临时禁用该虚拟主机是个好做法。', '', '');

$_tipsdb['vdisable'] = new DAttrHelp("禁用", '<span class=&quot;lst-inline-token lst-inline-token--value&quot;>禁用</span>操作会停止正在运行的虚拟主机。 新的请求将不再被接受，但正在处理的请求会照常完成。', '', '', '');

$_tipsdb['venable'] = new DAttrHelp("启用", '<span class=&quot;lst-inline-token lst-inline-token--value&quot;>启用</span>操作会启动已停止的虚拟主机。 这将允许其接受新请求。', '', '', '');
...
$_tipsdb['vhEnableBr'] = new DAttrHelp("启用Brotli压缩", '指定是否为此虚拟主机启用Brotli压缩。 这个设置仅在服务器级别的&quot;Brotli压缩级别（静态文件）&quot;设置为一个非零值时有效。', '', '布尔值', '');

$_tipsdb['vhEnableGzip'] = new DAttrHelp("启用压缩", '指定是否为此虚拟主机启用GZIP压缩。 这个设置仅在服务器级别的GZIP compression设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>时有效。', '', '布尔值', '');

$_tipsdb['vhEnableQuic'] = new DAttrHelp("启用HTTP3/QUIC", '为此虚拟主机启用HTTP3/QUIC网络协议。 要使此设置生效，服务器级别的&quot;启用HTTP3/QUIC&quot;和监听器级别的&quot;开放HTTP3/QUIC（UDP）端口&quot;也必须分别设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。 默认值为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>是</span>。', '当此设置为<span class=&quot;lst-inline-token lst-inline-token--value&quot;>否</span>时，将不再发送HTTP3/QUIC公告。如果浏览器仍缓存HTTP3/QUIC信息，并且服务器级别和监听器级别仍启用HTTP3/QUIC，则在这些信息不再缓存或遇到HTTP3/QUIC协议错误之前，仍会继续使用HTTP3/QUIC连接。', '从单选框中选择', '');

$_tipsdb['vhMaxKeepAliveReq'] = new DAttrHelp("最大持续连接请求数", '指定通过持续连接（持久）会话处理的请求的最大数量。当该限制值达到时连接将被断开。你可以为不同虚拟主机设置不同的数值。这个数值不能超过服务器级别的&quot;最大持续连接请求数&quot;限制值。', ' 设置为一个合理的较高数值。设置为1或更小的值将禁用keep-alive连接。', '整数', '');
...
$_tipsdb['vhlog_fileName'] = new DAttrHelp("文件名", '指定日志文件的路径。', ' 将日志文件放在单独的磁盘上。', '文件名可以是绝对路径，也可以是相对于$SERVER_ROOT或$VH_ROOT的相对路径。', '');
...
$_tipsdb['vstatus'] = new DAttrHelp("状态 - 虚拟主机", '虚拟主机的当前状态。 状态可以是：<span class=&quot;lst-inline-token lst-inline-token--value&quot;>运行中</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>已停止</span>、<span class=&quot;lst-inline-token lst-inline-token--value&quot;>需要重启</span> 或<span class=&quot;lst-inline-token lst-inline-token--value&quot;>运行中 - 已从配置中移除</span>。  <ul>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>运行中</span>表示虚拟主机已加载并正在提供服务。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>已停止</span>表示虚拟主机已加载但未提供服务（已禁用）。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>需要重启</span>表示这是新添加的虚拟主机，服务器尚未加载其配置。</li>     <li><span class=&quot;lst-inline-token lst-inline-token--value&quot;>运行中 - 已从配置中移除</span>表示虚拟主机已从服务器配置中删除，但仍在提供服务。</li> </ul>', '', '', '');
...
$_tipsdb['EDTP:gzipCompressLevel'] = array('GZIP压缩级别范围为1（最小）到9（最大）。');
