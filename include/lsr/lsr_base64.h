 ...
 */
int                 lsr_base64_decode( const char *encoded, int encodeLen, char *decoded );
 ...
 */
int                 lsr_base64_encode( const char *decoded, int decodedLen, char *encoded );

/** @lsr_base64_encode_length
 * @brief Given a length, gets the expected length
 * of an encoded buffer.
 * 
 * @param[in] len - The length of the buffer to encode.
 * @return The expected length of an encoded buffer.
 */
static inline int   lsr_base64_encode_length( int len ) 
    { return (((len + 2) / 3) * 4); }
