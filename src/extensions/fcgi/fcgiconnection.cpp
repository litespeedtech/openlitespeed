...
#define FCGI_INPUT_BUFSIZE G_BUF_SIZE
int FcgiConnection::processFcgiData()
{
    ...
    do
    {
        len = read( HttpGlobals::g_achBuf, FCGI_INPUT_BUFSIZE );
        if ( D_ENABLED( DL_MORE ) )
        {
            LOG_D(( getLogger(),
        ...
        if ( len == -1 )
            return len;
        int used;
        char * pCur = HttpGlobals::g_achBuf;
        int left = len;
        while( left > 0 )
        {
            switch( m_iRecStatus )
...
{
    LOG_INFO(( getLogger(), "[%s] FcgiConnection watching event: %d, wantWrite: %d, "
