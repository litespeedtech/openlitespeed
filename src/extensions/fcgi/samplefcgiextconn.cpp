    ...
    : HttpExtConnector()
{
    ...
    m_env.add( "ACCEPT", "image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, image/png, */*" );
    m_env.add( "GATEWAY_INTERFACE", "CGI/1.1" );
    m_env.add( "REQUEST_METHOD", "POST" );
    m_env.add( "HTTP_USER_AGENT", "Mozilla/4.78 [en] (X11; U; Linux 2.4.18-1smp i686)" );
    m_env.add( "REQUEST_URI", "/fcgi/echo-cpp" );
    char achBuf[ 30];
    safe_snprintf( achBuf, 30, "%d", m_iBodySize );
