...
int LsapiReq::appendEnv( LsapiEnv * pEnv, HttpSession *pSession )
{
    ...
    if ( psTemp )
    {
        ...
        ((lsapi_req_header *)m_bufReq.begin())->m_scriptFileOff = pEnv->bufSize() - psTemp->len() - 1;
        ++count;
    }
...
int LsapiReq::buildReq( HttpSession *pSession, int * totalLen )
{
    ...
    *totalLen = m_bufReq.size() + sizeof( lsapi_http_header_index )
    ...
    buildPacketHeader( &((lsapi_req_header *)m_bufReq.begin())->m_pktHeader, LSAPI_BEGIN_REQUEST, 
                       *totalLen );
    m_pIovec->append( m_bufReq.begin(), m_bufReq.size() );

    ret = appendHttpHeaderIndex( pReq, ((lsapi_req_header *)m_bufReq.begin())->m_cntUnknownHeaders );
    if ( ret )
        return ret;
