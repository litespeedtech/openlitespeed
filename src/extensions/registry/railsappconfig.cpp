        ...
        int runOnStart, const char *pRubyPath)
{
    ...
    {
        ConfigCtx::getCurConfigCtx()->log_errorPath("Rack application",
                achFileName);
        return NULL;
    }
    if (!pRailsRunner)
                                      ...
                                      const char *pRubyBin)
{
    ...
    {
        ConfigCtx::getCurConfigCtx()->log_error("Ruby path is not vaild: %s",
                                                pRubyBin);
        pRubyBin = NULL;
    }
...
int RailsAppConfig::loadRailsDefault(const XmlNode *pNode)
{
    ...
    if (ConfigCtx::getCurConfigCtx()->getLongValue(pNode, "enableRailsHosting",
            0, 1, 0) == 1)
    {
        HttpGlobals::s_railsAppLimit = ConfigCtx::getCurConfigCtx()->getLongValue(
                                           pNode, "railsAppLimit", 0, 20, 1);
        HttpGlobals::s_rubyProcLimit = ConfigCtx::getCurConfigCtx()->getLongValue(
