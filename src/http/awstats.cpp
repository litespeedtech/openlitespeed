...
int Awstats::prepareAwstatsEnv(const HttpVHost *pVHost)
{
    ...
    safe_snprintf(achBuf, 1024, "%s/conf/awstats.model.conf",
                  m_sWorkingDir.c_str());
    if (nio_stat(achBuf, &st) == -1)
    {
        char achTmp[1024];
...
int Awstats::updateIfNeed(long curTime, const HttpVHost *pVHost)
{
    ...
    if (!m_iLastUpdate)
    {
        ...
        struct stat st;
        safe_snprintf(achConfFile, 1024, "%s/conf/awstats.%s.conf",
                      m_sWorkingDir.c_str(),
                      pVHost->getName());
                     ...
                     char *iconURI, const char *vhDomain, int vhAliasesLen)
{
    ...
    if (needConvert)
    {
        ...
        pValue = achBuf;
        ConfigCtx::getCurConfigCtx()->log_info("SiteAliases is set to '%s'",
                                               achBuf);
    }
