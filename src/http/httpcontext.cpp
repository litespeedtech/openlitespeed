    ...
    if (pList)
    {
        ...
        for (iter = pList->begin(); iter != pList->end(); ++iter)
        {
            ...
            if (setCustomErrUrls(pCode, pUrl) != 0)
                ConfigCtx::getCurConfigCtx()->log_error("failed to set up custom error page %s - %s!",
                                                        pCode, pUrl);
            else
                ++add ;
        }
                        ...
                        int type)
{
    ...
    if (pNode)
    {
        ...
        {
            if (*pValue != '/')
                ConfigCtx::getCurConfigCtx()->log_error("Invalid rewrite base: '%s'",
                                                        pValue);
            else
