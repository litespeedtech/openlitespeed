...
HttpReq::HttpReq()
    ...
    , m_fdReqFile(-1)
{
    m_urls = (key_value_pair *)g_pool.allocate(sizeof(key_value_pair) *
             (MAX_REDIRECTS + 1));
...
key_value_pair *HttpReq::getValueByKey(const AutoBuf &buf, int idxOff,
                                       const char *pName,
                                       int namelen) const
{
    if (!idxOff)
...
void HttpReq::removeKeyValueByKey(const AutoBuf &buf, int idxOff,
                                  const char *pKey, int keyLen)
{
    ...
    if (pEntry)
    {
        ...
        int *used = (((int *)p) + 1);
        key_value_pair *pLast = (key_value_pair *)(p + sizeof(int) * 2) +
                                (*used - 1);
        if (pLast != pEntry)
...
int HttpReq::processContextPath()
{
    ...
    if (m_iMatchedLen)
        return processMatchedURI(pURI, iURILen, HttpGlobals::g_achBuf,
                                 m_iMatchedLen);
                         ...
                         char *pBegin, char *pEnd,  int &cacheable)
{
    ...
    if (p != pEnd)
    {
        ...
        {
            if (strncmp(getOrgURI() + orgURILen - m_iPathInfoLen, p,
                        m_iPathInfoLen) == 0)
...
void HttpReq::tranEncodeToContentLen()
{
    ...
    pBegin -= 17;
    len += 17;
    int n = safe_snprintf(pBegin, len, "Content-length: %ld",
                          m_lEntityFinished);
...
const char *HttpReq::findEnvAllias(const char *pKey, int keyLen,
                                   int &alliasKeyLen)
{
...
key_value_pair *HttpReq::addEnv(const char *pOrgKey, int orgKeyLen,
...
void HttpReq::restorePathInfo()
{
    ...
    m_pSSIRuntime->restorePathInfo(m_iPathInfoOff, m_iPathInfoLen,
                                   m_iRedirects);
                               ...
                               const char *&pURL, int &iURLLen)
{
    if ((pCtx->getLocationLen() <= m_iMatchedLen) &&
        (strncmp(HttpGlobals::g_achBuf, pCtx->getLocation(),
