...
private:
    ...
    key_value_pair *getValueByKey(const AutoBuf &buf, int idxOff,
                                  const char *pName,
                                  int namelen) const;
    void removeKeyValueByKey(const AutoBuf &buf, int idxOff, const char *pKey,
                             int keyLen);
...
public:
    ...
    {
        key_value_pair *pIdx = getValueByKey(m_headerBuf, m_headerIdxOff, pName,
                                             namelen);
        if (pIdx)
        {
            valLen = pIdx->valLen;
    ...
    key_value_pair *addEnv(const char *pKey, int keyLen, const char *pValue,
                           int valLen);
    const char *getEnv(const char *pKey, int keyLen, int &valLen);
    const char *getEnvByIndex(int idx, int &keyLen, const char *&pValue,
                              int &valLen);
    int  getEnvCount()
...
//     const HttpContext * getHTAContext() const   {   return m_pHTAContext;   }
    ...
    {
        return m_reqBuf.getPointer((m_iRedirects) ? m_urls[0].keyOff :
                                   m_curURL.keyOff);
    }
    int  getOrgURILen()
    {
    ...
    int   getOrgReqURILen()
    {
        if ((m_iRedirects) ? m_urls[0].valOff : m_curURL.valOff)
            return m_reqURLLen - 1 - ((m_iRedirects) ? m_urls[0].valLen :
                                      m_curURL.valLen);
        else
            return m_reqURLLen;
    }
