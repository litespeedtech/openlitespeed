...
int HttpSession::reqBodyDone()
{
    ...
    {
        ret = m_sessionHooks.runCallbackNoParam(LSI_HKPT_RCVD_REQ_BODY,
                                                (LsiSession *)this);
        if (ret <= -1)
...
int HttpSession::restartHandlerProcess()
{
    ...
    m_sessionHooks.restartSessionHooks(sessionLevels,
                                       sizeof(sessionLevels) / sizeof(int), pContext->getSessionHooks());
    ...
    m_iFlag &= ~(HSF_RESP_HEADER_DONE | HSF_RESP_WAIT_FULL_BODY |
                 HSF_RESP_FLUSHED | HSF_HANDLER_DONE);
...
int HttpSession::processWebSocketUpgrade(const HttpVHost *pVHost)
{
    ...
    LOG_D((getLogger(), "[%s] VH: web socket, name: [%s] URI: [%s]",
           getLogId(), pVHost->getName(), m_request.getURI()));
...
int HttpSession::processNewReq()
{
    ...
    if ((HttpGlobals::s_useProxyHeader == 1) ||
        ((HttpGlobals::s_useProxyHeader == 2)
    ...
    if (!pVHost)
    {
        if (D_ENABLED(DL_LESS))
        {
            register char *pHostEnd = (char *)m_request.getHostStr() +
    ...
    {
        m_sessionHooks.triggered();
        ret = m_sessionHooks.runCallbackNoParam(LSI_HKPT_HTTP_BEGIN,
                                                (LsiSession *)this);
        if (ret <= -1)
    ...
    {
        m_sessionHooks.triggered();
        if (m_sessionHooks.runCallbackNoParam(LSI_HKPT_RECV_REQ_HEADER,
                                              (LsiSession *)this) < 0)
...
int HttpSession::processURI(int resume)
{
    ...
    if (this->getReq()->getHttpHandler() != NULL &&
        this->getReq()->getHttpHandler()->getHandlerType() ==
        HandlerType::HT_MODULE)
        return handlerProcess(m_request.getHttpHandler());
    ...
    while (proceed)
    {
        ...
        if (!resume)
        {
            ...
            m_pModuleConfig = ((HttpContext *)pContext)->getModuleConfig();
            if (!m_sessionHooks.hasOwnCopy())
                m_sessionHooks.inherit(((HttpContext *)
                                        pContext)->getSessionHooks());//FIXME

            if (m_sessionHooks.isEnabled(LSI_HKPT_URI_MAP))
            {
                m_sessionHooks.triggered();
                if (m_sessionHooks.runCallbackNoParam(LSI_HKPT_URI_MAP,
                                                      (LsiSession *)this) < 0)
            ...
            if (m_request.getHttpHandler()
                && m_request.getHttpHandler()->getHandlerType() == HandlerType::HT_MODULE)
                goto DO_AUTH;
...
int HttpSession::doWrite()
{
    ...
    if (m_pHandler
        && !(m_iFlag & (HSF_HANDLER_DONE | HSF_MODULE_WRITE_SUSPENDED)))
    {
...
static int writeRespBodyTermination(HttpSession *session, const char *pBuf,
                                    int len)
{
...
int HttpSession::writeRespBody(const char *pBuf, int len)
{
    ...
    return runFilter(LSI_HKPT_SEND_RESP_BODY, (void *)writeRespBodyTermination,
                     pBuf, len, 0);
}
...
int HttpSession::setupGzipFilter()
{
    ...
    if (gz & (UPSTREAM_GZIP | UPSTREAM_DEFLATE))
    {
        if (recvhkptNogzip || hkptNogzip || !(gz & REQ_GZIP_ACCEPT))
        {
            ...
            if (addModgzipFilter((LsiSession *)this, 0, 0,
                                 -3000) == -1)     //addModgzipFilter((LsiSession *)this, isSend, 0, -3000 );
                return -1;
            m_response.getRespHeaders().del(
                HttpRespHeaders::H_CONTENT_ENCODING);    //FIXME: check if removed in modgzip
    ...
    if (gz == GZIP_REQUIRED)
    {
        ...
        else //turn on compression at SEND_RESP_BODY filter
        {
            ...
            if (addModgzipFilter((LsiSession *)this, 1,
                                 HttpServerConfig::getInstance().getCompressLevel(), 3000) == -1)
                return -1;
...
int HttpSession::runFilter(int hookLevel, void *pfTermination,
...
int HttpSession::appendDynBody(const char *pBuf, int len)
{
    ...
    return runFilter(LSI_HKPT_RECV_RESP_BODY, (void *)appendDynBodyTermination,
                     pBuf, len, 0);
}
...
int HttpSession::respHeaderDone(int iRespState)
{
    ...
    {
        m_sessionHooks.triggered();
        ret = m_sessionHooks.runCallbackNoParam(LSI_HKPT_RECV_RESP_HEADER,
                                                (LsiSession *)this);
    }
...
int HttpSession::endResponseInternal(int success)
{
    ...
    {
        m_sessionHooks.triggered();
        ret = m_sessionHooks.runCallback(LSI_HKPT_RCVD_RESP_BODY,
                                         (LsiSession *)this,
...
//             param._flag_in = LSI_CB_FLAG_IN_EOF;
        ...
        LOG_D(("[HttpSession::sendStaticFile] sent: %d, buffered: %d", len,
               buffered));
