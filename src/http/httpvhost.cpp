...
void HttpVHost::onTimer()
{
    ...
    else
    {
        ...
        if (m_pBytesLog)
            LogRotate::testAndRoll(m_pBytesLog, HttpGlobals::s_uid,
                                   HttpGlobals::s_gid);
        if (m_pLogger)
        {
...
int HttpVHost::configBasics(const XmlNode *pVhConfNode, int iChrootLen)
{
    ...
    {
        ConfigCtx::getCurConfigCtx()->log_error("failed to set document root - %s!",
                                                pPath);
        return -1;
    }
...
int HttpVHost::configRealm(const XmlNode *pRealmNode)
{
    ...
    if (p2 == NULL)
    {
        ConfigCtx::getCurConfigCtx()->log_error("missing <%s> in <%s>", "userDB",
                                                "realm");
        return -1;
    }
    ...
    if (strcasecmp(pType, "file") == 0)
    {
        ...
        ConfigCtx::getCurConfigCtx()->log_error("unsupported DB type in realm %s!",
                                                pName);
                                      ...
                                      const char *pHandler, int allowBrowse)
{
    ...
    if (type < HandlerType::HT_FASTCGI && type != HandlerType::HT_MODULE)
    {
        if (allowBrowse)
        {
            ...
            if (ret)
            {
                ConfigCtx::getCurConfigCtx()->log_errorPath("context location",
                        pLocation);
                return NULL;
            }
            ...
            if (!match)
            {
                ...
                {
                    ConfigCtx::getCurConfigCtx()->log_error("path is not accessible: %s",
                                                            achRealPath);
                    return NULL;
                }
                ...
                if (PathLen > 512)
                {
                    ConfigCtx::getCurConfigCtx()->log_error("path is too long: %s",
                                                            achRealPath);
                    return NULL;
                }
                             ...
                             const XmlNode *pNode)
{
                 ...
                 getVhRoot()->len())))
    {
        currentCtx.log_error("AWStats working directory: %s is not inside virtual host root: "
                             ...
                             ((iChrootLen == 0) ? ("") : (HttpGlobals::s_psChroot->c_str())),
                             getVhRoot()->c_str());
        return -1;
    }
                                     ...
                                     const char *pWorkerName, int allowBrowse)
{
    ...
    if (access(achFileName, F_OK) != 0)
    {
        ConfigCtx::getCurConfigCtx()->log_errorPath("Web-App configuration",
                achFileName);
        return NULL;
    }
                           ...
                           const char *pLocation)
{
    ...
    else
    {
        ...
        if (orgCode)
        {
            ...
            if (code == -1)
            {
                ConfigCtx::getCurConfigCtx()->log_warn("Invalid status code %d, use default: 302!",
                                                       orgCode);
                code = SC_302;
            }
        }
...
int HttpVHost::configContext(const XmlNode *pContextNode)
{
    ...
    if (pHandler)
    {
        if (ConfigCtx::getCurConfigCtx()->expandVariable(pHandler, achHandler,
                256) < 0)
        {
            currentCtx.log_notice("add String is too long for scripthandler, value: %s",
                                  pHandler);
            return -1;
        }
    ...
    if ((*pUri != '/') && (!match))
    {
        currentCtx.log_error("URI must start with '/' or 'exp:', invalid URI - %s",
                             pUri);
        return -1;
    }
        ...
        const HttpContext *pContext)
{
    ...
    if (config->own_data_flag == 1)
    {
        ...
        config->config = pModule->_config_parser->_parse_config(
                             config->sparam->c_str(), init_config, LSI_CONTEXT_LEVEL,
                             pContext->getURI());
...
int HttpVHost::config(const XmlNode *pVhConfNode)
{
    ...
    {
        HttpSessionHooks *parentHooks = ((HttpContext *)
                                         pRootContext->getParent())->getSessionHooks();
        pRootContext->setInternalSessionHooks(parentHooks);
    }
    else
    {
        pRootContext->initExternalSessionHooks();
        ModuleManager::getInstance().inheritHttpApiHooks(
    ...
    while (1)
    {
        ...
        if (pConfFile != NULL)
        {
            if (ConfigCtx::getCurConfigCtx()->getValidFile(achVhConf, pConfFile,
                    "vhost config") == 0)
            {
                pVhConfNode = ConfigCtx::getCurConfigCtx()->parseFile(achVhConf,
                              "virtualHostConfig");

                if (pVhConfNode == NULL)
                    ConfigCtx::getCurConfigCtx()->log_error("cannot load configure file - %s !",
                                                            achVhConf);
                else
                    gotConfigFile = true;
            }
...
int HttpVHost::checkDeniedSubDirs(const char *pUri, const char *pLocation)
{
    ...
    DeniedDir::iterator iter = HttpGlobals::getDeniedDir()->lower_bound(
                                   pLocation);
