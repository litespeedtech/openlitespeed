...
int ModUserdir::buildPath( char * pURL, int urlLen, char * pPath, int &len )
{
    ...
    if ( p )
        *p = 0;
    int ret = buildPath( pURL, urlLen, (p)?(p+1):(pURL + urlLen), pPath, len );
    if ( p )
        *p = '/';
    if ( ret == SC_403 )
        return ret;
...
int ModUserdir::buildPath( char * pURL, int urlLen, char *p, char * pPath, int &len )
{
    pURL += 2;
    ...
    if ( m_mode == MUD_UNIX )
    {
        ...
        if ( pw )
        {
            len = safe_snprintf( pPath, len, "%s/%s/%s", pw->pw_dir, m_userdir.c_str(), p );
        }
    }
    else
    {
        ...
        if ( m_wildcard >= 0 )
        {
            ...
            pos += m_wildcard;
            len = safe_snprintf( pos, (pPath + len) - pos, "%s%s/%s",
