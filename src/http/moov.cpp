...
static int boxheader_read(unsigned char * buf, boxheader_t * header, unsigned char* boundry);
...
static unsigned char * bytes_write(unsigned char * buf, int count, uint64_t ui);
static unsigned char * boxheader_write(unsigned char * buf, boxheader_t * header);
static unsigned char * stsc_write(unsigned char * dst, void * box);
static unsigned char * stts_write(unsigned char * dst, void * box);
static unsigned char * stsz_write(unsigned char * dst, void * box, int no_size_table);
...
static uint32_t stsc_adjust(stsc_t * stsc, uint32_t c1,uint32_t c2, unsigned char * stsc_entries);
static uint32_t stts_adjust(stts_t * stts, uint32_t s1, uint32_t s2, unsigned char * stts_entries);
static uint32_t stsz_adjust(stsz_t * stsz,uint32_t s1,uint32_t s2);
static uint32_t stco_co64_adjust(stco_co64_t * stco_co64, uint32_t c1, uint32_t c2);
...
static int calc_new_moov_diff(moov_t * moov_box, float start_time, float end_time);
static uint32_t duration_to_sample(uint64_t duration, unsigned char * stts_entries, uint32_t count);
static uint32_t sample_to_chunk(uint32_t sample, unsigned char * stsc_entries, uint32_t count, uint32_t max_chunk, uint32_t * chunkFirstSample, uint32_t * chunkLastSample);
static uint32_t sample_to_duration(uint32_t s1, uint32_t s2, unsigned char * stts_entries, uint32_t count);
static uint64_t trak_max_offset_new( stbl_t * stbl, uint32_t chunk, moov_t * moov);
                ...
                uint32_t * mini_moov_size)    //in-out
{
    ...
    while(pos < file_size)
    {
        ...
        {
            case FourCC('m', 'o', 'o', 'v'):
                if(moov_box.end !=0)    //An F4V file must contain one and only one moov box
                {
#ifdef UNIT_TEST
                ...
                break;
            case FourCC('m', 'd', 'a', 't'):
                if(mdat_box.end !=0)    //An F4V file must contain one and only one mdat box
                {
#ifdef UNIT_TEST
    ...
    mmap_size = (size_t)moov_box.header.ExtendedSize + moov_box.start % moov_getpagesize();
    ...
    if ( !mmap_data )
        return -1;
    moov_data = ( unsigned char *)mmap_data + moov_box.start % moov_getpagesize();
    /*
...
int get_moov(    int fd,                        //in - video file descriptor
                ...
                uint32_t mini_moov_size)    //in
{
    ...
    do{
        ...
        t = (int64_t)(moov.header.ExtendedSize + moov.header.deltaSize     - moov.mdat_start_offset + moov.mdat_header_length);
...
int get_mdat(    int fd,                        //in - video file descriptor
                float start_time,            //in - seconds
                float end_time,                //in - seconds. 0 means end of moovie
    ...
    while (sub_buf < boundry)
    {
        ...
        switch(dummy.BoxType){
            ...
            case FourCC('c','h','p','l'):
                ...
                moov_box->header.deltaSize += -1 * dummy.ExtendedSize; //possibly multiple "chpl" boxes
                break;
            default:
                moov_box->header.deltaSize += -1 * dummy.ExtendedSize; //optimization: delete all other boxes
                break;
        }
    ...
    if(sub_buf == boundry){
        ...
        for(i=0;i<moov_box->track_num;i++)
        {
            ...
            stbl->stco_co64.offset_in_file = moov_box->offset_in_file + 
                        (stbl->stco_co64.header.start - moov_box->header.start) + stbl->stco_co64.header.length + 1+3+4;
            stbl->stsz.offset_in_file = moov_box->offset_in_file + 
                        (stbl->stsz.header.start - moov_box->header.start) + stbl->stsz.header.length + 1+3+4+4;
        }
        return(1);
    }else{
#ifdef UNIT_TEST
    ...
    while (sub_buf < boundry)
    {
        ...
        switch(dummy.BoxType){
            ...
            case FourCC('m','d','i','a'):
                ...
                break;
            default:
                trak_box->header.deltaSize += -1 * dummy.ExtendedSize; //optimization: delete all other boxes
                break;
        }
        sub_buf += dummy.ExtendedSize;
    }
    ...
    while (sub_buf < boundry)
    {
        ...
        switch(dummy.BoxType){
            ...
            case FourCC('m','i','n','f'):
                ...
                break;
            default:
                mdia_box->header.deltaSize += -1 * dummy.ExtendedSize; //optimization: delete all other boxes
                break;
        }
        sub_buf += dummy.ExtendedSize;
    }
    ...
    while (sub_buf < boundry)
    {
        ...
        switch(dummy.BoxType){
            case FourCC('s','t','b','l'):
                ...
                break;
            default:
                minf_box->header.deltaSize += -1 * dummy.ExtendedSize; //optimization: delete all other boxes
                break;
        }
        sub_buf += dummy.ExtendedSize;
    }
    ...
    while (sub_buf < boundry)
    {
        ...
        switch(dummy.BoxType){
            ...
            case FourCC('s','t','s','d'):
                ...
                break;
            default:
                stbl_box->header.deltaSize += -1 * dummy.ExtendedSize; //optimization: delete all other boxes
                break;
        }
        sub_buf += dummy.ExtendedSize;
    }
...
static int stsz_read(unsigned char * buf, void * box)
{
    ...
    stsz_box->ConstantSize = v;    //If all samples have the same size, this field is set with that constant size;otherwise it is 0
    sub_buf += 4;
...
static int calc_new_moov_diff(moov_t * moov_box, float start_time, float end_time)
{
    ...
    for(i=0;i < (unsigned int)moov_box->track_num;i++)
    {
        ...
        s1 = duration_to_sample(t1, (unsigned char*)stbl->stts.Entries, stbl->stts.Count);
        c1 = sample_to_chunk(s1, (unsigned char*)stbl->stsc.Entries, stbl->stsc.Count, max_chunk, & s1, & s1_2);

        s2 = duration_to_sample(t2, (unsigned char*)stbl->stts.Entries, stbl->stts.Count);
        c2 = sample_to_chunk(s2, (unsigned char*)stbl->stsc.Entries, stbl->stsc.Count, max_chunk, & s2_1, & s2);
        ...
        moov_box->trak[i].mdia.header.deltaSize += moov_box->trak[i].mdia.minf.header.deltaSize;
        moov_box->trak[i].header.deltaSize += moov_box->trak[i].mdia.header.deltaSize;
        deltaSize += moov_box->trak[i].header.deltaSize;

        moov_box->trak[i].mdia.mdhd.newDuration = sample_to_duration(s1, s2, (unsigned char*)stbl->stts.Entries, stbl->stts.Count);
        moov_box->trak[i].tkhd.newDuration = (uint64_t)moov_box->mvhd.TimeScale * moov_box->trak[i].mdia.mdhd.newDuration / moov_box->trak[i].mdia.mdhd.TimeScale;
...
static uint32_t sample_to_chunk(uint32_t sample, unsigned char * stsc_entries, uint32_t count, uint32_t max_chunk, uint32_t * chunkFirstSample, uint32_t * chunkLastSample)
{
    uint32_t i,chunk_p, sample_p, chunk_c, sample_acc=0,t,chunk_delta, chunk;
...
static uint32_t stco_co64_adjust(stco_co64_t * stco_co64, uint32_t c1, uint32_t c2)
{
    ...
    stco_co64->header.deltaSize = -1*(stco_co64->OffsetCount - (c2-c1+1))*t;
...
static int mini_moov_calc_size(moov_t * moov_box)
{
    ...
    for(i=0;i < moov_box->track_num;i++)
    {
        ...
        trak->mdia.minf.header.ExtendedSize = trak->mdia.minf.header.length + stbl->header.ExtendedSize;
        trak->mdia.header.ExtendedSize = trak->mdia.header.length + trak->mdia.mdhd.header.ExtendedSize +
                                                    trak->mdia.minf.header.ExtendedSize;
        trak->header.ExtendedSize = trak->header.length + trak->tkhd.header.ExtendedSize +
                                                    trak->mdia.header.ExtendedSize;

        newS += trak->header.ExtendedSize;
    }
    moov_box->header.ExtendedSize = moov_box->header.length + moov_box->mvhd.header.ExtendedSize + newS +4+8*3;
...
static int mini_moov_write(unsigned char * buf, moov_t * box)
{
    ...
    memcpy(dst,moov_box->mvhd.header.start,moov_box->mvhd.header.ExtendedSize);
    ...
    for(i=0;i < moov_box->track_num; i++)
    {
        ...
        memcpy(dst, trak->mdia.mdhd.header.start, trak->mdia.mdhd.header.ExtendedSize);//mdhd
        ...
        memcpy(dst, stbl->stco_co64.header.start, stbl->stco_co64.header.ExtendedSize);//stco_co64
        ...
        t = stbl->stsz.header.ExtendedSize;
        stbl->stsz.header.ExtendedSize = stbl->stsz.header.length + 1+3+4*2+8+4;
        dst = boxheader_write(dst,&stbl->stsz.header);
        memcpy(dst, stbl->stsz.header.start+stbl->stsz.header.length, 1+3+4*2);
        dst += 1+3+4*2;
        dst = bytes_write(dst,8,moov_box->offset_in_file + (stbl->stsz.header.start - moov_box->header.start));
    ...
    while (sub_buf < boundry)
    {
        ...
        switch(dummy.BoxType){
            ...
            case FourCC('s','t','s','d'):
                ...
                r=stsd_read(sub_buf,(void*)&moov_box->trak[trak_index].mdia.minf.stbl.stsd);
                if(r==0)
                    return(0);
                sub_buf += dummy.ExtendedSize;
                break;
            case FourCC('s','t','s','c'):
                ...
                r=stsc_read(sub_buf,(void*)&moov_box->trak[trak_index].mdia.minf.stbl.stsc);
                if(r==0)
                    return(0);
                sub_buf += dummy.ExtendedSize;
                break;
            case FourCC('s','t','t','s'):
                ...
                r=stts_read(sub_buf,(void*)&moov_box->trak[trak_index].mdia.minf.stbl.stts);
                if(r==0)
                    return(0);
                sub_buf += dummy.ExtendedSize;
                break;
            case FourCC('s','t','s','z'):
                ...
                moov_box->trak[trak_index].mdia.minf.stbl.stsz.ConstantSize = bytes_read(sub_buf + dummy.length+1+3, 4);
                moov_box->trak[trak_index].mdia.minf.stbl.stsz.SizeCount = bytes_read(sub_buf + dummy.length+1+3+4, 4);
                moov_box->trak[trak_index].mdia.minf.stbl.stsz.offset_in_file = bytes_read(sub_buf + dummy.length+1+3+8, 8);
                moov_box->trak[trak_index].mdia.minf.stbl.stsz.original_box_size = bytes_read(sub_buf + dummy.length+1+3+16, 4);
                sub_buf += dummy.length + 1+3+4*2+8+4;
                break;
            case FourCC('s','t','c','o'):
            case FourCC('c','o','6','4'):
                moov_box->trak[trak_index].mdia.minf.stbl.stco_co64.header=dummy;
                r=stco_co64_read(sub_buf,(void*)&moov_box->trak[trak_index].mdia.minf.stbl.stco_co64);
                if(r==0)
                    return(0);
                sub_buf += dummy.ExtendedSize;
                break;
            case FourCC('s','t','s','s'):
                ...
                r=stss_read(sub_buf,(void*)&moov_box->trak[trak_index].mdia.minf.stbl.stss);
                if(r==0)
                    return(0);
                sub_buf += dummy.ExtendedSize;
                break;
            default:
#ifdef UNIT_TEST
                printf("boundry=%u,here=%u\n",(uint32_t)(boundry - buf),(uint32_t)(sub_buf - buf));
#endif
                return(0);
                break;
        }
...
static int moov_write_chunk(moov_data_t    * moov_data, moov_t * moov_box)
{
    ...
    {
        moov_data->remaining_bytes = moov_box->header.ExtendedSize + moov_box->header.deltaSize;
        already_return = 0;
    }else
        already_return = moov_box->header.ExtendedSize + moov_box->header.deltaSize - moov_data->remaining_bytes;
    ...
    for(i=0;i < (unsigned int)moov_box->track_num; i++)
    {
        ...
        acc += stbl->stco_co64.header.ExtendedSize + stbl->stco_co64.header.deltaSize;
    ...
    if(in_memory==0)    //send stsz in file
    {
        ...
        moov_data->file.start_offset = stbl->stsz.offset_in_file + stbl->stsz.header.length + 1+3+4*2 + stbl->stsz.newFirstEntry * 4;
    ...
    if(trak_to_send == 0){
        ...
        memcpy(dst,moov_box->mvhd.header.start,moov_box->mvhd.header.ExtendedSize);//mvhd
        if(moov_box->mvhd.Version==0){
                bytes_write(dst + moov_box->mvhd.header.length + 1+3+4*3,4,(uint64_t)moov_box->mvhd.newDuration);
        }else{
                bytes_write(dst + moov_box->mvhd.header.length + 1+3+8*2+4,8,(uint64_t)moov_box->mvhd.newDuration);
        }
        dst += moov_box->mvhd.header.ExtendedSize;
    }
    ...
    if(trak->tkhd.Version==0){
            bytes_write(dst + trak->tkhd.header.length + 1+3+4*4,4,(uint64_t)trak->tkhd.newDuration);
    }else{
            bytes_write(dst + trak->tkhd.header.length + 1+3+8*2+4*2,8,(uint64_t)trak->tkhd.newDuration);
    }
    dst += trak->tkhd.header.ExtendedSize;
    ...
    if(trak->mdia.mdhd.Version==0){
            bytes_write(dst + trak->mdia.mdhd.header.length + 1+3+4*3,4,(uint64_t)trak->mdia.mdhd.newDuration);
    }else{
            bytes_write(dst + trak->mdia.mdhd.header.length + 1+3+8*2+4,8,(uint64_t)trak->mdia.mdhd.newDuration);
    }
    dst += trak->mdia.mdhd.header.ExtendedSize;
