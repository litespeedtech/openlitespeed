...
int SubstItem::parseServerVar( const char * pCurLine, const char * &pFormatStr, const char * pEnd, int isSSI )
{
...
int RequestVars::parseHttpHeader( const char * pName, int len, const char * &pHeaderName, int &headerLen )
{
    int ret = -1;
    if ( len <= 0 )
        return ret;
...
const char * RequestVars::getCookieValue( HttpReq * pReq, const char * pCookieName,
...
{
    if (( iIndex >= 0 )&&( iIndex <= (int)(sizeof(s_pHeaders)/sizeof( char *)) ))
