...
KeyData * RewriteMapFile::parseLine( const char * pKey, int keyLen, char * pLine, char *pLineEnd )
{
...
int RewriteMap::reloadFromStore()
{
    ...
    if ( m_pStore->open() )
    {
        LOG_ERR(( "Failed to open rewrite map file: %s", m_pStore->getDataStoreURI() ));
        return -1;
    }
    KeyData * pData;
...
int RewriteMap::lookup( const char * pKey, int keyLen, char * pValue, int valLen )
{
    if ( m_type <= TYPE_RND )
    {
    ...
    else
    {
        ...
        {
        case TYPE_INT_UPPER:
            if ( StringTool::strnupper( pKey, pValue, valLen ) == NULL )
                valLen = 0;
            break;
        case TYPE_INT_LOWER:
        ...
        case TYPE_INT_ESCAPE:
            ...
            break;
        case TYPE_INT_UNESC:
            valLen = HttpUtil::unescape_n( pKey, keyLen, pValue, valLen );
            break;
        case TYPE_DBM:
        case TYPE_PRG:
        default:
