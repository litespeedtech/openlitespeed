                    ...
                    const RewriteMapList * pMaps )
{
    ...
    if (( ret )||( argEnd == argBegin ))
    {
        if ( pError )
            HttpLog::parse_error( s_pCurLine,  pError );
        HttpLog::parse_error( s_pCurLine,  "Rewrite rule missing substitution string." );
        return -1;
    }
...
int RewriteRule::parseRuleFlag( const char * &pRuleStr, const char *pEnd )
{
    ...
    if ( *pRuleStr != '[' )
    {
        HttpLog::parse_error( s_pCurLine,  "invalid third parameter, '[' is expected" );
        return -1;
    }
    ++pRuleStr;
    while( pRuleStr < pEnd )
...
int RewriteRule::parseRule( char * pRule, const char * pEnd, const RewriteMapList * pMaps )
{
    const char * argBegin = NULL;
    const char * argEnd = NULL;
