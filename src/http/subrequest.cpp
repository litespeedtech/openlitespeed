...
//put subrequest related code in this file. 
#include <http/httpsession.h>

typedef int (*SubSessionCb)( HttpSession * pSubSession, void * param, int flag );
...
HttpSession * HttpSession::newSubSession( SubSessInfo_t * pSubSessInfo )
{
    ...
    pSession->setFlag( HSF_SEC_CLEARED | HSF_SEC_RESP_CLEARED | HSF_SUB_SESSION );
    ...
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] Create SUB SESSION: %d, flag: %d, "
                "method: %s, URI: %s, len: %d, QS: %s, len: %d\n",
                getLogId(), m_iSubReqSeq, pSubSessInfo->m_flag, 
...
int HttpSession::execSubSession( HttpSession * pSubSess, int iHttpError )
{
    ...
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] Execute SUB SESSION: %d\n",
               getLogId(), ((HioChainStream* )pSubSess->getStream())->getSequence() ));
...
int HttpSession::attachSubSession( HttpSession * pSubSess )
{
    ...
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] attach SUB SESSION: %d\n",
               getLogId(), pStream->getSequence() ));
...
int HttpSession::detachSubSession( HttpSession * pSubSess )
{
    HioChainStream * pStream = (HioChainStream* )pSubSess->getStream();
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] detach SUB SESSION: %d\n",
               getLogId(), pStream->getSequence() ));
    pStream->setParentSession( NULL );
    //set Stream to black hole mode, 
    pStream->setFlag( HIO_FLAG_BLACK_HOLE, 1 );
...
int HttpSession::includeSubSession()
{
    ...
    {
        if ( D_ENABLED( DL_MEDIUM ) )
            LOG_D(( getLogger(), "[%s] include response from SUB SESSION: %d\n",
                    getLogId(), ((HioChainStream* )m_pCurSubSession->getStream())->getSequence() ));
...
{
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] onSubSessionEndResp() SUB SESSION: %d\n",
                getLogId(), ((HioChainStream* )pSubSess->getStream())->getSequence() ));
...
{
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] onSubSessionRespIncluded() SUB SESSION: %d\n",
                getLogId(), ((HioChainStream* )pSubSess->getStream())->getSequence() ));
...
{
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] close SUB SESSION: %d\n",
                getLogId(), ((HioChainStream* )pSubSess->getStream())->getSequence() ));
...
{
    if ( D_ENABLED( DL_MEDIUM ) )
        LOG_D(( getLogger(), "[%s] cancel SUB SESSION: %d\n",
                getLogId(), ((HioChainStream* )pSubSess->getStream())->getSequence() ));
...
void HttpReq::setNewOrgUrl( const char* pUrl, int len, const char* pQS, int qsLen )
{
    int total = len; 
    if ( qsLen > 0 )
        total += qsLen + 1;
...
int HttpReq::clone(HttpReq* pProto, SubSessInfo_t * pSubSessInfo )
{
    ...
    if ( pProto->m_commonHeaderOffset[ HttpHeader::H_COOKIE ] > m_iHttpHeaderEnd )
    {
...
void HttpReq::updateReqHeader(int index, const char* pNewValue, int newValueLen )
{
...
int HttpReq::copyCookieHeaderToBufEnd(int oldOff, const char * pCookie, int cookieLen )
{
    int offset, diff;
    char * pBuf;
    ...
    if ( !pCookie )
        pCookie = m_headerBuf.getPointer( m_commonHeaderOffset[ HttpHeader::H_COOKIE ] );
