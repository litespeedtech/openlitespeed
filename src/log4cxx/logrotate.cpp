                ...
                int compress, uid_t uid, gid_t gid )
{
    ...
    n = safe_snprintf( achBuf, max, "%s.%04d_%02d_%02d",
    ...
    while( 1 )
    {
        ...
        l = safe_snprintf( pMoreSuffix, max, ".%d", suffix++ );
    }
    if ( pSuffix && *pSuffix )
    {
        safe_snprintf( achName, 1024, "%s%s", pFileName, pSuffix );
        pFileName = achName;
    }
    ...
    if ( compress )
    {
        ...
        safe_snprintf( achName, 1024, "%s.gz", achBuf );
        GzipBuf gzBuf;
...
int LogRotate::roll(Appender * pAppender, uid_t uid, gid_t gid, off_t rollingSize )
{
    int ret = 0;
...
int LogRotate::testRolling( Appender * pAppender, off_t rollingSize, uid_t uid, gid_t gid )
{
    int ret = 0;
    if ( rollingSize <= 0 )
        return ret;
