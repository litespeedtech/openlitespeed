...
void AllGlobalDataHashTInit()
{
    for (int i = 0; i < LSI_CONTAINER_COUNT; ++i)
        LsiapiBridge::gLsiGDataContHashT[i] = new __LsiGDataContHashT(30,
...
__LsiGDataItemHashT::iterator get_gdata_iterator(__LsiGDataItemHashT
        *container, const char *key, int key_len)
{
...
static void renew_gdata_TTL(__LsiGDataItemHashT::iterator iter, int TTL,
...
void erase_gdata_element(lsi_gdata_cont_val_t *containerInfo,
                         __LsiGDataItemHashT::iterator iter)
{
    gdata_item_val_t *pItem = iter.second();
...
//-1 error
static int recover_file_gdata(lsi_gdata_cont_val_t *containerInfo,
                              const char *key, int key_len, const char *file_path,
                              __LsiGDataItemHashT::iterator &iter, lsi_release_callback_pf release_cb,
...
LSIAPI void *get_gdata(lsi_gdata_cont_val_t *containerInfo,
                       const char *key, int key_len, lsi_release_callback_pf release_cb,
                       int renew_TTL, lsi_deserialize_pf deserialize_cb)
{
    __LsiGDataItemHashT *pCont = containerInfo->container;
    __LsiGDataItemHashT::iterator iter = get_gdata_iterator(pCont, key,
                                         key_len);
    gdata_item_val_t *pItem = NULL;
    int TTL = 0;
...
LSIAPI int delete_gdata(lsi_gdata_cont_val_t *containerInfo,
                        const char *key, int key_len)
{
    __LsiGDataItemHashT::iterator iter = get_gdata_iterator(
            containerInfo->container, key, key_len);
    __LsiGDataItemHashT *pCont = containerInfo->container;
    if (iter != pCont->end())
        erase_gdata_element(containerInfo, iter);
...
LSIAPI int set_gdata(lsi_gdata_cont_val_t *containerInfo, const char *key,
...
LSIAPI lsi_gdata_cont_val_t *get_gdata_container(int type, const char *key,
        int key_len)
{
    ...
    __LsiGDataContHashT *pLsiGDataContHashT =
        LsiapiBridge::gLsiGDataContHashT[type];
    lsi_gdata_cont_val_t *containerInfo = NULL;
    char file_path[LSI_MAX_FILE_PATH_LEN] = {0};
    ...
    else
    {
        ...
        __LsiGDataItemHashT *pContainer = new __LsiGDataItemHashT(30,
                lsi_global_data_hash_fn, lsi_global_data_cmp);
        memcpy(p, key, key_len);

        time_t tm = DateTime::s_curTime;
        containerInfo = (lsi_gdata_cont_val_t *)malloc(sizeof(
...
{
    __LsiGDataItemHashT *pCont = containerInfo->container;
    for (__LsiGDataItemHashT::iterator iter = pCont->begin();
         iter != pCont->end(); iter = pCont->next(iter))
        erase_gdata_element(containerInfo, iter);
    return 0;
}
