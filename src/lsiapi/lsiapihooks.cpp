...
void LsiApiHooks::initGlobalHooks()
{
    ...
    m_pIolinkHooks->get(LSI_HKPT_L4_BEGINSESSION)->setFlag(
        LSI_HOOK_FLAG_NO_INTERRUPT);
    m_pIolinkHooks->get(LSI_HKPT_L4_ENDSESSION)->setFlag(
        LSI_HOOK_FLAG_NO_INTERRUPT);

    m_pHttpHooks->get(LSI_HKPT_HTTP_BEGIN)->setFlag(
        LSI_HOOK_FLAG_NO_INTERRUPT);
    m_pHttpHooks->get(LSI_HKPT_HTTP_END)->setFlag(LSI_HOOK_FLAG_NO_INTERRUPT);
    m_pHttpHooks->get(LSI_HKPT_HANDLER_RESTART)->setFlag(
        LSI_HOOK_FLAG_NO_INTERRUPT);
}
                       ...
                       short priority, short flag)
{
    ...
    for (pHook = begin(); pHook < end(); ++pHook)
    {
        if (pHook->_priority > priority)
            break;
        else if (pHook->_priority == priority && pHook->_module == pModule
                 && pHook->_cb == cb)
...
LsiApiHook *LsiApiHooks::find(const lsi_module_t *pModule,
                              lsi_callback_pf cb)
{
    LsiApiHook *pHook;
    for (pHook = begin(); pHook < end(); ++pHook)
...
int LsiApiHooks::runCallback(int level, lsi_cb_param_t *param) const
{
    ...
    LsiApiHook *hookEnd =
        end();  //FIXME: I found end() will change since m_iEnd will change
    while (hook != hookEnd)
    {
        rec1 = *param;
