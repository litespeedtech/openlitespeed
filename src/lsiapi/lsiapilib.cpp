...
static int lsiapi_add_session_hook(lsi_session_t *session, int index,
...
static int lsiapi_remove_session_hook(lsi_session_t *session, int index,
                                      const lsi_module_t *pModule)
{
    LsiApiHooks *pHooks;
    switch (index)
...
static void *get_cb_module_data(const lsi_cb_param_t *param, int level)
{
    ...
    return pData->get(MODULE_DATA_ID(((LsiApiHook *)
                                      param->_cur_hook)->_module)[level]);
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
typedef int (* appendDynBodyTermination_fun)(HttpSession *session,
        const char *pBuf, int len);
static int stream_write_next(lsi_cb_param_t *pParam, const char *buf,
                             int len)
{
    if ((((LsiApiHook *)pParam->_cur_hook) + 1) ==
        pParam->_hook_info->_hooks->end())
    {
        return (*(appendDynBodyTermination_fun)(
                    pParam->_hook_info->_termination_fp))(
...
static int lsiapi_stream_read_next(lsi_cb_param_t *pParam, char *pBuf,
                                   int size)
{
    if (((LsiApiHook *)pParam->_cur_hook) ==
        pParam->_hook_info->_hooks->begin())
    {
...
static int set_resp_header(lsi_session_t *session,
                           unsigned int header_index, const char *name, int nameLen, const char *val,
                           int valLen, int add_method)
{
    HttpSession *pSession = (HttpSession *)((LsiSession *)session);
    if (pSession == NULL)
        return -1;
...
static int get_resp_header(lsi_session_t *session,
                           unsigned int header_index, const char *name, int nameLen,
...
static int get_resp_headers(lsi_session_t *session, struct iovec *iov,
                            int maxIovCount)
{
    HttpSession *pSession = (HttpSession *)((LsiSession *)session);
    if (pSession == NULL)
...
static int lsi_set_timer(unsigned int timeout_ms,
                         lsi_timer_callback_pf timer_cb, void *timer_cb_param)
{
    ...
    pModuleTimer->m_tmExpireUs = DateTime::s_curTimeUs + 1000 *
                                 (timeout_ms % 1000);
    ...
    while (curPos)
    {
        ...
        if (pNext && ((pNext->m_tmExpire < pModuleTimer->m_tmExpire) ||
                      (pNext->m_tmExpire == pModuleTimer->m_tmExpire
                       && pNext->m_tmExpireUs <= pModuleTimer->m_tmExpireUs)))
            curPos = pNext;
        else
        {
                             ...
                             int maxValLen)
{
    ...
    if (type <= LSI_REQ_VAR_HTTPS)
        ret = RequestVars::getReqVar(pSession,
                                     type - LSI_REQ_VAR_REMOTE_ADDR + REF_REMOTE_ADDR,  p, maxValLen);
...
static int is_resp_buffer_gzippped(lsi_session_t *session)
{
    ...
    if (pSession->getReq()->gzipAcceptable() & (UPSTREAM_GZIP |
            UPSTREAM_DEFLATE))
        return 1;
    else
        return 0;
                                ...
                                const lsi_module_t *pModule, const char *path, int pathLen)
{
    ...
    int fd = dup(
                 pSession->getSendFileInfo()->getFileData()->getFileData()->getfd());
    return fd;
}
                                  ...
                                  char *buf, int maxLen)
{
    ...
    case LSI_VHOST_LEVEL:
        ret = ConfigCtx::getCurConfigCtx()->expandVariable(pVarible, buf, maxLen,
                1);
        break;
