...
int ModuleManager::storeModulePointer(int index, lsi_module_t *pModule)
{
    ...
    {
        module_count += 5;
        m_gModuleArray = (ModulePointer *)realloc(m_gModuleArray,
        ...
        const char *pType, lsi_module_t *pModule)
{
    ...
    if (D_ENABLED(DL_MORE))
        LOG_D(("%s module [%s] built with API v%hd.%hd has been registered successfully.",
...
void ModuleManager::disableModule(lsi_module_t *pModule)
{
    ...
    for (i = 0; i < size; ++i)
        ((LsiApiHooks *)(LsiApiHooks::getIolinkHooks()->get(i + base)))->remove(
            pModule);

    base = LsiApiHooks::getHttpHooks()->getBase();
    size = LsiApiHooks::getHttpHooks()->getSize();
    for (i = 0; i < size; ++i)
        ((LsiApiHooks *)(LsiApiHooks::getHttpHooks()->get(i + base)))->remove(
            pModule);
}
        ...
        if (pModule->_init)
        {
            ...
            else
            {
                ...
                LOG_INFO(("[Module: %s %s] has been initialized successfully",
                          MODULE_NAME(pModule), ((pModule->_info) ? pModule->_info : "")));
            }
        }
    }
    return 0;
...
{
    m_gModuleArray = (ModulePointer *)malloc(module_count_init_num * sizeof(
                         ModulePointer));
...
void ModuleManager::OnTimer100msec()
{
    ...
    while (1)
    {
        ...
        if (pNext && ((pNext->m_tmExpire < DateTime::s_curTime) ||
                      ((pNext->m_tmExpire == DateTime::s_curTime)
                       && (pNext->m_tmExpireUs <= DateTime::s_curTimeUs))))
        {
...
void ModuleManager::inheritIolinkApiHooks(IolinkSessionHooks
        *apiIolinkHooks, ModuleConfig *moduleConfig)
{
    ...
    for (short module_id = 0; module_id < count; ++module_id)
    {
        ...
        {
            for (int level = apiIolinkHooks->getBase();
                 level < apiIolinkHooks->getBase() + apiIolinkHooks->getSize(); ++level)
 ...
 */
void ModuleManager::inheritHttpApiHooks(HttpSessionHooks *apiHttpHooks,
                                        ModuleConfig *moduleConfig)
{
    apiHttpHooks->inherit(
        LsiApiHooks::getHttpHooks());  //inherit from global level
    int count = moduleConfig->getCount();
    for (short module_id = 0; module_id < count; ++module_id)
    {
        if (!moduleConfig->getFilterEnable(module_id))
        {
            for (int level = apiHttpHooks->getBase();
                 level < apiHttpHooks->getBase() + apiHttpHooks->getSize(); ++level)
                apiHttpHooks->getCopy(level)->remove(m_gModuleArray[module_id]);
        }
    }
}

void ModuleManager::updateHttpApiHook(HttpSessionHooks *apiHttpHooks,
...
ModuleConfig::~ModuleConfig()
{
    ...
    {
        config = get(i);
        if (config->own_data_flag == 2 && config->config
...
void ModuleConfig::inherit(const ModuleConfig *parentConfig)
{
    for (int i = 0; i < m_count; ++i)
    {
        ...
        config->own_data_flag =
            0;  //may inherit the config but this flag should not be set
        config->sparam = NULL;
    }
}
...
int ModuleConfig::parsePriority(const XmlNode *pModuleNode, int *priority)
{
    ...
    {
        if (pModuleNode
            && (pValue = pModuleNode->getChildValue(LsiApiHooks::s_pHkptName[i])))
                                  ...
                                  ModuleConfig *pModuleConfig, int level, const char *name)
{
         ...
         iter != moduleConfigNodeList->end(); ++iter)
    {
        ...
        parseConfig(pNode, moduleIter.second()->getModule(), pModuleConfig, level,
                    name);
    }
    return ret;
}
