...
public:
    ...
    void inheritIolinkApiHooks(IolinkSessionHooks *apiIolinkHooks,
                               ModuleConfig *moduleConfig);
    void inheritHttpApiHooks(HttpSessionHooks *apiHttpHooks,
                             ModuleConfig *moduleConfig);
    void updateHttpApiHook(HttpSessionHooks *apiHttpHooks,
...
private:
    ModulePointer
    *m_gModuleArray;  //It is pointers stored base on the _module_id order
...
typedef struct lsi_module_config_t
{
    lsi_module_t *module;
    int16_t
    own_data_flag;  //0: no own data, may use the inherited config, no sparam, 1: own, not parsed, 2: own, parsed
    int16_t
    filters_enable; //Comment: use bits, now bits count is LSI_HKPT_TOTAL_COUNT
    void
    *config;  //Should be a struct of all parameters, if config_own_flag set, this need to be released
    AutoStr2   *sparam;
} lsi_module_config_t;
