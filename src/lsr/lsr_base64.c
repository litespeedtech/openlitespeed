...
static const unsigned char s_lsr_decodeTable[128] =
{
...
static const unsigned char s_lsr_encodeTable[] =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
            
int lsr_base64_decode( const char *encoded, int size, char *decoded )
{
...
int lsr_base64_encode( const char *decoded, int size, char *encoded )
{
    ...
    while ( pEnd > pDecoded ) 
    {
        *pEncoded++ = s_lsr_encodeTable[( ( ch = *pDecoded++ ) >> 2 ) & 0x3f];
        *pEncoded++ = s_lsr_encodeTable[( ( ch & 0x03 ) << 4 ) | ( *pDecoded >> 4 )];
        ch = *pDecoded++;
        *pEncoded++ = s_lsr_encodeTable[( ( ch & 0x0f ) << 2 ) | ( *pDecoded >> 6 )];
        *pEncoded++ = s_lsr_encodeTable[*pDecoded++ & 0x3f];
    }
    ...
    if ( size > 0 )
    {
        *pEncoded++ = s_lsr_encodeTable[( (ch = *pDecoded++) >> 2 ) & 0x3f];
        ...
        if ( size == 1 )
        {
            *pEncoded++ = s_lsr_encodeTable[( ch & 0x03 ) << 4];
            *pEncoded++ = '=';
        ...
        {
            *pEncoded++ = s_lsr_encodeTable[(( ch & 0x03 ) << 4) | ( *pDecoded >> 4 )];
            *pEncoded++ = s_lsr_encodeTable[( *pDecoded & 0x0f ) << 2];
        }
        *pEncoded++ = '=';
    }
