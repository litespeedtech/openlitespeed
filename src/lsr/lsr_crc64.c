...
const uint64_t lsr_crc64_table[4][256] = {
    {
...
extern uint64_t
lsr_crc64( uint64_t crc, const uint8_t *buf, size_t size )
{
    crc = ~crc;
    ...
    if (size > 4) {
        while ((uintptr_t) (buf) & 3) {
            crc = lsr_crc64_table[0][*buf++ ^ A1(crc)] ^ S8(crc);
            --size;
        }
        ...
        while (buf < limit) {
            ...
            crc = lsr_crc64_table[3][A(tmp)]
                ^ lsr_crc64_table[2][B(tmp)]
                ^ S32(crc)
                ^ lsr_crc64_table[1][C(tmp)]
                ^ lsr_crc64_table[0][D(tmp)];
        }
    }
