...
static const void *lsr_md5_transform( lsr_md5_ctx_t *ctx, const void *p, unsigned long size );
...
int lsr_md5_update( lsr_md5_ctx_t *ctx, const void *p, size_t len )
{
    ...
    if ( used ) 
    {
        ...
        len -= available;
        lsr_md5_transform( ctx, ctx->shiftBuf, 64 );
    }
    if ( len >= 64 ) 
    {
        p = lsr_md5_transform( ctx, p, len & ~(unsigned long)0x3f );
        len &= 0x3f;
    }
...
unsigned char *lsr_md5( const unsigned char *p, size_t len, unsigned char *ret )
{
    ...
    if ( !lsr_md5_init( &ctx ) )
        return NULL;
    lsr_md5_update( &ctx, p, len );
    lsr_md5_final( ret, &ctx );
    memset( &ctx, 0, sizeof( lsr_md5_ctx_t ) );
    return ret;
}
