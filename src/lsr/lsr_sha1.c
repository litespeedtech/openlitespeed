...
*****************************************************************************/
#include <lsr/lsr_sha1.h>

#if defined( USE_OPENSSL_SHA1 )

int lsr_sha1_init( lsr_sha1_ctx_t *ctx )
{
    return SHA1_Init( (SHA_CTX *)ctx );
}

void lsr_sha1_free( lsr_sha1_ctx_t *ctx )
{
    /* OpenSSL does not have a free. */
}

int lsr_sha1_update( lsr_sha1_ctx_t *ctx, const unsigned char *input, size_t ilen )
{
    return SHA1_Update( (SHA_CTX *)ctx, input, ilen );
}

int lsr_sha1_finish( lsr_sha1_ctx_t *ctx, unsigned char *output )
{
    return SHA1_Final( output, (SHA_CTX *)ctx );
}

unsigned char *lsr_sha1( const unsigned char *input, size_t ilen, unsigned char *output )
{
...
int lsr_sha1_init( lsr_sha1_ctx_t *ctx )
{
    if ( !memset( ctx, 0, sizeof( lsr_sha1_ctx_t ) ) )
        return 0;
...
void lsr_sha1_free( lsr_sha1_ctx_t *ctx )
{
    ...
    memset( ctx, 0, sizeof( lsr_sha1_ctx_t ) );
}

static void lsr_sha1_process( lsr_sha1_ctx_t *ctx, const unsigned char data[64] )
{
...
int lsr_sha1_update( lsr_sha1_ctx_t *ctx, const unsigned char *input, size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        lsr_sha1_process( ctx, ctx->m_buf );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
...
static const unsigned char lsr_sha1_padding[64] =
{
...
int lsr_sha1_finish( lsr_sha1_ctx_t *ctx, unsigned char *output )
{
    ...
    lsr_sha1_update( ctx, lsr_sha1_padding, padn );
    lsr_sha1_update( ctx, msglen, 8 );
...
unsigned char *lsr_sha1( const unsigned char *input, size_t ilen, unsigned char *output )
{
    ...
    if ( !lsr_sha1_init( &ctx ) )
        return NULL;
    lsr_sha1_update( &ctx, input, ilen );
    lsr_sha1_finish( &ctx, output );
    lsr_sha1_free( &ctx );
    return output;
}
#endif