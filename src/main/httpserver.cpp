...
private:
    ...
    int initAdns()
    {
        ...
        if (HttpGlobals::s_adns.init() == -1)
            return -1;
        HttpGlobals::getMultiplexer()->add(&HttpGlobals::s_adns,
...
int generateConnReport(int fd)
{
    ...
    if (pCtrl->getMaxConns() - pCtrl->availConn() - HttpGlobals::s_iIdleConns <
        0)
    ...
    int n = safe_snprintf(achBuf, 4096,
                          ...
                          HttpGlobals::s_lSSLBytesRead / 1024,
                          HttpGlobals::s_lSSLBytesWritten / 1024,
...
int HttpServerImpl::generateProcessReport(int fd)
{
    ...
    if (days)
        p += safe_snprintf(p, &achBuf[4096] - p, " %ld day%s", days,
                           (days > 1) ? "s" : "");
...
int HttpServerImpl::generateRTReport()
{
    ...
    ret = HttpGlobals::getClientCache()->generateBlockedIPReport(
              pAppender->getfd());
...
void HttpServerImpl::offsetChroot()
{
    ...
    HttpLog::offsetChroot(HttpGlobals::s_psChroot->c_str(),
    ...
    HttpGlobals::getServerInfo()->m_pChroot =
        HttpGlobals::getServerInfo()->dupStr(HttpGlobals::s_psChroot->c_str(),
                HttpGlobals::s_psChroot->len());
    m_vhosts.offsetChroot(HttpGlobals::s_psChroot->c_str(),
                          HttpGlobals::s_psChroot->len());
}
...
int HttpServerImpl::reinitMultiplexer()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        if (!(m_listeners[i]->getBinding() & (1L << (HttpGlobals::s_iProcNo - 1))))
            ...
            HttpGlobals::getMultiplexer()->add(m_listeners[i],
                                               POLLIN | POLLHUP | POLLERR);
    }
    initAdns();
    return 0;
        ...
        const char *pVHostName)
{
    ...
    if (pDomains == NULL)
    {
        ConfigCtx::getCurConfigCtx()->log_error("missing <domain> in <vhostMap> - vhost = %s listener = %s",
                                                pVHost, pListener);
        return -1;
    }
        ...
        for (iter = pList->begin(); iter != pList->end(); ++iter)
        {
            ...
            if (pListener)
            {
                ...
                if ((configVirtualHostMappings(pListener,
                                               ((confType == 0) ? (pListenerNode->getChild("vhostMapList")) :
                                                (pListenerNode)),
        ...
        if (!isAdmin)
        {
            ...
            if (pModuleList)
                ModuleConfig::parseConfigList(pModuleList, &pListener->m_moduleConfig,
                                              LSI_SERVER_LEVEL, pName);

            ModuleManager::getInstance().inheritIolinkApiHooks(
                &pListener->m_iolinkSessionHooks, &pListener->m_moduleConfig);
        }
        ...
        char *pchPHPBin)
{
    ...
    chown(&HttpGlobals::s_pAdminSock[5], pVHostAdmin->getUid(),
          pVHostAdmin->getGid());
                                       ...
                                       const char *pSwapDir)
{
    while (1)
    {
        ...
        if (pSwapDir)
            setSwapDir(pSwapDir);
        safe_snprintf(achBuf, 4096, "%s/tmp/ocspcache/",
        ...
        HttpGlobals::s_useProxyHeader = ConfigCtx::getCurConfigCtx()->getLongValue(
                                            pRoot, "useIpInProxyHeader", 0, 2, 0);
    ...
    XmlNode *pVhConfNode, XmlNode *pTmpConfNode, const char *pTemplateName)
{
    ...
    for (iter = pList->begin(); iter != pList->end(); ++iter)
    {
        ...
        if (getVHost(pName))
        {
            ...
            currentCtx.log_info("Virtual host %s already exists, skip template configuration.",
                                pName);
            continue;
        }
...
int HttpServerImpl::configVHTemplate(const XmlNode *pNode)
{
                                ...
                                "name", 1);
    {
        ...
        pTmpConfNode = ConfigCtx::getCurConfigCtx()->parseFile(achTmpConf,
                       "virtualHostTemplate");
...
int HttpServerImpl::configServerBasics(int reconfig, const XmlNode *pRoot)
{
    ...
    while (1)
    {
        ...
        HttpGlobals::s_priority = ConfigCtx::getCurConfigCtx()->getLongValue(pRoot,
                                  "priority", -20, 20, 0);

        HttpGlobals::s_children = ConfigCtx::getCurConfigCtx()->getLongValue(pRoot,
        ...
        HttpGlobals::s_503AutoFix = ConfigCtx::getCurConfigCtx()->getLongValue(
                                        pRoot, "AutoFix503", 0, 1, 1);
        HttpGlobals::s_enableH2c = ConfigCtx::getCurConfigCtx()->getLongValue(
                                       pRoot, "enableh2c", 0, 1, 0);
...
int HttpServerImpl::loadAdminConfig(XmlNode *pRoot)
{
    ...
    {
        currentCtx.log_error("missing configuration file for admin server: %s!",
                             achConfFile);
        return -1;
    }

    XmlNode *pAdminConfNode = ConfigCtx::getCurConfigCtx()->parseFile(
                                  achConfFile, "adminConfig");

    if (pAdminConfNode == NULL)
    {
        currentCtx.log_error("cannot load configure file for admin server - %s !",
                             achConfFile);
        return -1;
    }
...
int HttpServerImpl::configChroot(const XmlNode *pRoot)
{
                ...
                0)))
    {
        ...
        if (pValue)
        {
            ...
            {
                ConfigCtx::getCurConfigCtx()->log_error("chroot must be valid absolute path: %s",
                                                        pChroot);
                strcpy(pChroot, "/");
                len = 1;
            }
...
int HttpServerImpl::configServer(int reconfig, XmlNode *pRoot)
{
    ...
    if (!reconfig)
    {
        ...
        ConfigCtx::getCurConfigCtx()->log_info("old priority: %d, new priority: %d",
                                               pri, new_pri);
    }
...
void HttpServerImpl::enableCoreDump()
{
...

    {
        ...
        if (dumpable == -1)
            ...
            ConfigCtx::getCurConfigCtx()->log_notice("Child: %d] Core dump is %s.",
                    getpid(),
                    (dumpable) ? "enabled" : "disabled");
    }
#endif
