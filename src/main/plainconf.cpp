...
static HashStringMap<plainconfKeywords *> allKeyword(29,
        GHash::i_hash_string, GHash::i_comp_string);
...
void saveUnknownItems(const char *fileName, int lineNumber,
...
void addModuleWithParam(XmlNode *pCurNode, const char *moduleName,
                        const char *param)
{
...
XmlNode *parseFile(const char *configFilePath)
{
    ...
    if (gModuleList.size() != 1)
        logToMem(LOG_LEVEL_ERR,
                 "parseFile find '{' and '}' do not match in the end of file %s.",
                 configFilePath);
    gModuleList.clear();
