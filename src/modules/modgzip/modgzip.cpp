...
{
    ModgzipMData *myData = (ModgzipMData *) g_api->get_module_data(
...
static int initZstream(lsi_cb_param_t *rec, lsi_module_t *pModule,
                       z_stream *pStream, uint8_t compressLevel)
{
    int ret = 0;
    pStream->avail_in = 0;
    ...
    {
        if (deflateInit2(pStream, compressLevel, Z_DEFLATED, 16 + MAX_WBITS, 8,
...
int flushLoopBuf(lsi_cb_param_t *rec, int iZState, LoopBuff *pBuff)
{
    ...
    {
        sz = g_api->stream_write_next(rec, _loopbuff_getdataref(pBuff),
                                      _loopbuff_blockSize(pBuff));
        if (sz < 0)
            return -1;
        else if (sz > 0)
        {
...
static int clearData(lsi_cb_param_t *rec)
{
    g_api->free_module_data(rec->_session, g_api->get_module(rec),
                            LSI_MODULE_DATA_HTTP, releaseData);
    return 0;
}
                       ...
                       int isSend)
{
    ...
    ZBufInfo *pZBufInfo = (isSend ? & (myData->sendZBufInfo) : &
                           (myData->recvZBufInfo));
    const char *pCompressFlag = ((pZBufInfo->compressLevel == 0) ?
                                 DECOMPRESS_FLAG : COMPRESS_FLAG);
    ...
    if (pZBufInfo->iZState == Z_UNINITED)
    {
        ...
        {
            //assert(isSend == 0);
            g_api->remove_session_hook(rec->_session, LSI_HKPT_RECV_RESP_BODY,
                                       pModule);
    ...
    do
    {
        ...
        if (ret >= Z_OK)
        {
            ...
            else if (sz < 0)
            {
                g_api->log(rec->_session, LSI_LOG_ERROR,
                           "[%s%s] compressbuf in %d, return %d (written %d, flag in %d)\n",
        ...
        {
            g_api->log(rec->_session, LSI_LOG_ERROR,
                       "[%s%s] compressbuf in %d, compress function return %d\n",
    ...
    g_api->log(rec->_session, LSI_LOG_DEBUG,
               "[%s%s] compressbuf [%s] in %d, consumed: %d, written %d, flag in %d, buffer has %d.\n",
               pModuleName, pCompressFlag, pSendingFlag, rec->_param_len, consumed,
               written, rec->_flag_in, _loopbuff_getdatasize(pBuff));
    return consumed;
}
...
static void set_Module_data(lsi_session_t *session, lsi_module_t *pModule,
                            ModgzipMData *myData)
{
    g_api->set_module_data(session, pModule, LSI_MODULE_DATA_HTTP,
                           (void *)myData);
    g_api->add_session_hook(session, LSI_HKPT_HTTP_END, pModule, clearData,
                            LSI_HOOK_NORMAL, 0);
    g_api->add_session_hook(session, LSI_HKPT_HANDLER_RESTART, pModule,
                            clearData, LSI_HOOK_NORMAL, 0);
}

static int addHooks(lsi_session_t *session, lsi_module_t *pModule,
                    int isSend, int priority, uint8_t compressLevel)
{
    ModgzipMData *myData = (ModgzipMData *) g_api->get_module_data(session,
    ...
    if (!myData)
    {
        ...
        if (!myData)
        {
            g_api->log(session, LSI_LOG_ERROR,
                       "[%s] AddHooks failed: no enough memory [Error 1].\n",
                       g_api->get_module_name(pModule));
            return -1;
        }
        else
    ...
    if (isSend)
    {
        ...
        {
            g_api->add_session_hook(session, LSI_HKPT_SEND_RESP_BODY, pModule,
                                    sendingHook, priority, LSI_HOOK_FLAG_PROCESS_STATIC |
    ...
    else
    {
        ...
        {
            g_api->add_session_hook(session, LSI_HKPT_RECV_RESP_BODY, pModule,
                                    recvingHook, priority, 1);
            g_api->add_session_hook(session, LSI_HKPT_RCVD_RESP_BODY, pModule,
                                    clearDataPartR, priority, 0);
    ...
    else
    {
        ...
        if (myData->recvZBufInfo.inited == 0 && myData->sendZBufInfo.inited == 0)
            delete myData;
        g_api->log(session, LSI_LOG_ERROR,
                   "[%s] AddHooks failed: no enough memory [Error 2].\n",
                   g_api->get_module_name(pModule));
        return -1;
    }
}
...
int addModgzipFilter(lsi_session_t *session, int isSend,
                     uint8_t compressLevel, int priority)
{
    if (compressLevel == 0)
