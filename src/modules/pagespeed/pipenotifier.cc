...
int PipeNotifier::handleEvents( short int event )
{
    ...
    if( event & POLLIN )
    {
        ...
        g_api->log( NULL, LSI_LOG_DEBUG, "[PipeNotifier] handleEvents called, fd = %d, read %d byte(s), session=%ld\n ",
...
int PipeNotifier::initNotifier( Multiplexer* pMultiplexer, void* session )
{
    ...
    g_api->log( NULL, LSI_LOG_DEBUG, "[PipeNotifier] initNotifier called, fd = %d ret = %d.\n ", getfd(), ret );
