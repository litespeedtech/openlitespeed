...
bool HostInfo::getHostByName(const char *name)
{
    ...
    do
    {
        pHosts = ::gethostbyname_r( name, (char *)this, m_pBuf, H_BUF_SIZE, &lh_errno );
        iRetry ++;
    }
...
bool HostInfo::getHostByAddr(const void *addr, int len, int type)
{
    ...
    do
    {
        pHosts = ::gethostbyaddr_r( (const char*)addr, len, type, (char *)this, m_pBuf, H_BUF_SIZE, &lh_errno );
        iRetry ++;
    }
    while (( pHosts == NULL )&&( lh_errno == TRY_AGAIN )&& (iRetry < 3 ));
