...
static const char s_h2sUpgradeResponse[] = "HTTP/1.1 101 Switching Protocols\r\nConnection: Upgrade\r\nUpgrade: h2c\r\n\r\n";
...
int H2Connection::init()
{
    ...
    m_pCurH2Header = (H2FrameHeader*)m_H2HeaderMem;
    return 0;
}
...
H2Connection& H2Connection::operator=( const H2Connection & other )
{
    return *this;
}
            ...
            if ( m_iCurrentFrameRemain > m_iPeerMaxFrameSize )
            {
                if ( D_ENABLED( DL_LESS ) )
                    LOG_D(( getLogger(), "[%s] Frame size is too large, "
                            ...
                            getLogId(), m_iPeerMaxFrameSize, m_iCurrentFrameRemain ));
                return -1;
            }
        }
...
int H2Connection::processSettingFrame( H2FrameHeader * pHeader )
{
    ...
    {
        LOG_INFO(( getLogger(), "[%s] bad SETTINGS frame, frame size does not match.",
                        getLogId() ));          
        return -1;
    }
    ...
    if ( pHeader->getFlags() & H2_FLAG_ACK )
    {
        if ( iEntries != 0 )
        {
            LOG_INFO(( getLogger(), "[%s] bad SETTINGS frame with ACK flag, has %d entries.",
                        getLogId(), iEntries ));          
            return -1;
        }
        ...
        {
            LOG_INFO(( getLogger(), "[%s] received SETTINGS frame with ACK flag before sending our SETTINGS frame.",
                        getLogId() ));          
            return -1;
        }
    }
    ...
    for( int i = 0; i < iEntries; i++)
    {
        ...
        if ( D_ENABLED( DL_LESS ) )
        {
            LOG_D(( getLogger(), "[%s] %s(%d) value: %d", getLogId(), 
                    (iEntryID < 7)?cpEntryNames[iEntryID]:"INVALID", iEntryID, iEntryValue ));
        }
        switch( iEntryID )
        {
        case H2_SETTINGS_HEADER_TABLE_SIZE:
            m_hp.getReqDynamicTable().updateMaxCapacity(iEntryValue);
            break;
...
int H2Connection::processWindowUpdateFrame( H2FrameHeader * pHeader )
{
    ...
    if ( delta == 0 || tmpVal > 2147483647)
    {
        ...
        if ( D_ENABLED( DL_LESS ) )
        {
            LOG_D(( getLogger(), "[%s] session WINDOW_UPDATE ERROR: %d, window size: %d, total %d (m_iDataInWindow: %d) ",
    ...
    else
    {
        ...
        if ( D_ENABLED( DL_LESS ) )
        {
            LOG_D(( getLogger(), "[%s] session WINDOW_UPDATE: %d, window size: %d, streamID: %d ",
...
int H2Connection::processRstFrame( H2FrameHeader* pHeader )
{
    ...
    LOG_D(( getLogger(), "[%s] streamID:%d processRstFrame, error code: %d", getLogId(), 
            streamID, errorCode ));
...
int H2Connection::processContinuationFrame(H2FrameHeader* pHeader )
{
    ...
    int iDatalen = ( m_bufInput.size() < m_iCurrentFrameRemain)? ( m_bufInput.size()):(m_iCurrentFrameRemain);
    if (iDatalen <= 0)
        return 0;
    ...
    if( m_bufInput.blockSize() == m_bufInput.size() || iDatalen < m_bufInput.blockSize())
    ...
    int rc = extractCompressedData(pSrc, bufEnd);
...
int H2Connection::processHeadersFrame(H2FrameHeader* pHeader )
{
    ...
    if (iHeaderFlag & H2_FLAG_PRIORITY)
    {
        ...
        priority.m_dependStreamId = beReadUint32((const unsigned char *)pSrc) & 0x7FFFFFFFu;
        priority.m_weight = *(pSrc + 4) + 1; //Add one to the value to obtain a weight between 1 and 256.
        pSrc += 5;
    }
    else
...
int H2Connection::extractCompressedData( unsigned char *pSrc, unsigned char *bufEnd )
{
    int rc, n = 0;
    m_bufInflate.clear();
    ...
    while((rc = m_hp.decHeader(pSrc, bufEnd, name, name_len, val, val_len)) > 0)
    {
        name[name_len] = 0x00;
        val[val_len] = 0x00;
        //printf("name[%s] val[%s]\n", name, val);
        ...
        if (name[0] == ':')
        {
            ...
            {
                ; //must be http or https, but needn't it
...
H2Stream* H2Connection::getNewStream( uint32_t uiStreamID, uint8_t ubH2_Flags, Priority_st& priority)
{
    H2Stream* pStream;
...
void H2Connection::createInitH2cUpgradedStream( HioStreamHandler *pSession )
{
...
int H2Connection::sendFrame8Bytes( H2FrameType type, uint32_t uiStreamId, uint32_t uiVal1, uint32_t uiVal2 )
{
    getBuf()->guarantee( 17 );
...
int H2Connection::sendFrame4Bytes( H2FrameType type, uint32_t uiStreamId, uint32_t uiVal1 )
{
    H2FrameHeader header( 4, type, 0, uiStreamId );
    getBuf()->guarantee( 13 );
    ...
    if ( D_ENABLED( DL_MORE ) )
    {
        LOG_D(( getLogger(), "[%s] send %s frame, value: %d"
                , getLogId(), getH2FrameName( type ), uiVal1 ) );
    }
    return 0;
}
...
int H2Connection::append400BadReqReply( uint32_t uiStreamID )
{
    return 0;
}
...
int H2Connection::doGoAway(H2ErrorCode status)
{
    if ( D_ENABLED( DL_LESS ) )
    {
        LOG_D(( getLogger(), "[%s] H2Connection::doGoAway(), status = %d ", getLogId(), status ) );
    }
...
void H2Connection::logDeflateInflateError( int n, int iDeflate )
{
...
int H2Connection::compressHeaders( HttpRespHeaders *pRespHeaders, unsigned char *buf, int maxSize )
{
    ...
    char * p = (char *)HttpStatusCode::getCodeString( pRespHeaders->getHttpCode() ) + 1;
    pCur = m_hp.encHeader(pCur, pBufEnd, (char *)":status", 7, p, 3, 0 );
    ...
    for (int pos=pRespHeaders->HeaderBeginPos(); pos != pRespHeaders->HeaderEndPos(); pos = pRespHeaders->nextHeaderPos(pos))
    {
        ...
        {
            pCur = m_hp.encHeader(pCur, pBufEnd, key, keyLen, (char *)pIov->iov_base, pIov->iov_len, 0);
        }
    }
...
int H2Connection::sendRespHeaders( HttpRespHeaders *pRespHeaders, uint32_t uiStreamID )
{
    if ( D_ENABLED( DL_MORE ) )
    {
    ...
    int rc = compressHeaders( pRespHeaders, achHdrBuf, H2_TMP_HDR_BUFF_SIZE );
    if (rc < 0)
        return -1;
    
    getBuf()->guarantee( rc + 9 );
    appendCtrlFrameHeader( H2_FRAME_HEADERS, rc,   H2_FLAG_END_HEADERS, uiStreamID );  //H2_FLAG_END_STREAM
...
void H2Connection::recycle()
{
    if ( D_ENABLED( DL_MORE ) )
    {
        LOG_D(( getLogger(), "[%s] H2Connection::recycle()",
                getLogId() ));
    }
    delete this;
}

void H2Connection::resetStream( H2Stream * pStream, H2ErrorCode code )
{
    sendRstFrame( pStream->getStreamID(), code );
    recycleStream( pStream->getStreamID() );
}

void H2Connection::resetStream( StreamMap::iterator it, H2ErrorCode code )
{
