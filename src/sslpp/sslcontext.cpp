...
SSLContext *SSLContext::config(const XmlNode *pNode)
{
    ...
    pSSL = setKeyCertCipher(achCert, achKey, pCAFile, pCAPath, pCiphers,
                            ...
                            (cv != 0),
                            ConfigCtx::getCurConfigCtx()->getLongValue(pNode, "regenProtection", 0, 1,
                                    1));

    if (pSSL == NULL)
    {
        ConfigCtx::getCurConfigCtx()->log_error("failed to create SSL Context with key: %s, Cert: %s!",
                                                achKey, achCert);
        return NULL;
    }
    ...
    if (enableSpdy)
    {
        if (-1 == pSSL->enableSpdy(enableSpdy))
            ConfigCtx::getCurConfigCtx()->log_error("SPDY/HTTP2 can't be enabled [try to set to %d].",
                                                    enableSpdy);
    }
