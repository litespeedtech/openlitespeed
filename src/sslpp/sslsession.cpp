...
static char * obj2String( TSSLSession * pObj)
{
    ...
    if ( ( pObj->x_cache.x_magic == SSLSession::shmMagic() ) )
        *cp++ = 'M';
    else
        *cp++ = ' ';
        
    int dt = (DateTime::s_curTime - pObj->x_cache.x_expireTime) * 1000 + 
                        (DateTime::s_curTimeUs / 1000) - pObj->x_cache.x_expireTimeMs;
    *cp = ' ';
                        
    if (pObj->x_cache.x_expired || (dt > 0))
        *cp++ = 'X'; 
    else
        *cp++ = ' '; 
    *cp++ = pObj->x_cache.x_inited  ? 'I' : ' ';
    *cp++ = pObj->x_cache.x_tracked ? 'T' : ' ';
    *cp++ = pObj->x_cache.x_expired ? 'E' : ' ';
...
{
    HttpLog::notice("SSLSesion::unget <%p> %d %d context %p" , this, m_ready, m_ref, m_context);
    --m_ref;
    if (m_ref == 0)
    {
        remove();
...
{
    HttpLog::notice("~SSLSesion <%p> %d %d context %p" , this, m_ready, m_ref, m_context);
    if (m_ready)
    {
        remove();
        // m_pShmHash->unget();
                    ...
                    , SSL_CTX* pCtx)
{
    ...
    if (!pSSLSession)
    {
        ConfigCtx::getCurConfigCtx()->log_notice(
    ...
    ConfigCtx::getCurConfigCtx()->log_notice("SET OPENSSL CTX %p <%p> MAP %s CACHE %d [%d] TIMEOUT %d [%d] Options %X [%X] Mode %X [%X]",
                                             pSSLContext, pCtx, name
...
static inline int id2ascii(unsigned char * frombuf, int size, char * tobuf, int maxsize)
{
    register int oldsize = maxsize;
...
SSL_SESSION* SSLSession::sess_get_cb( SSL* pSSL, unsigned char* id, int len, int* ref )
{
    SSL_CTX * pCtx = SSL_get_SSL_CTX(pSSL);
    SSLSession * pSSLSession = (SSLSession*)SSL_CTX_get_ex_data(pCtx, s_idxContext);
...
SSL_SESSION* SSLSession::sess_get_func( SSL* pSSL, unsigned char* id, int len, int* ref )
{
...
again:    
    ...
    if (pObj)
    {
        if (pObj->x_cache.x_magic == s_shmMagic)
        {
            if ( (!pObj->x_cache.x_expired ) )
            {
...
int SSLSession::sess_new_func( SSL* pSSL, SSL_SESSION* pSess )
{
    ...
    if (pObj)
    {
        ...
        pObj->x_valueLen = session_len;
        unsigned char * cp = (unsigned char *)pObj->x_sessionData;
        i2d_SSL_SESSION(pSess, &cp);
        
        pObj->x_cache.x_expireTime += m_expireSec ;
        pObj->x_cache.x_inited = 1;
        if (!pObj->x_cache.x_tracked)
...
void SSLSession::sess_remove_func( SSL_SESSION* pSess )
{
    ...
    // lock
    m_base->lock();
    TSSLSession * pObj = (TSSLSession*)m_base->findObj(pSess->session_id, pSess->session_id_length);
    if (pObj)
    {
        if (pObj->x_cache.x_magic == s_shmMagic)
        {
            pObj->x_cache.x_expired = 1;
        }
    }
...
{
    register int        num = 0;
    register SSLSession * pSSLSession;
...
#ifdef DEBUG_RUN
    TSSLSession * pObj = (TSSLSession*)p;
    LsHash_stat_t * pHashStat = (LsHash_stat_t *)pUParam;
