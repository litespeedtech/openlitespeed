...
void testParseSubst()
{
    ...
    char achTest1[] = "$1%0\\\\\\$1\\%1${map1:${map1:$2|\"\"}|%{DOCUMENT_ROOT}}%{HTTP_USER_AGENT}"
    ...
    CHECK( ((RewriteSubstItem *)pMapRef->getDefaultFormat()->head()->next())->getType()
                     == REF_DOC_ROOT );
    CHECK( pMapRef->getKeyFormat() );
    RewriteSubstItem * pItem1 = (RewriteSubstItem *)(pMapRef->getKeyFormat()->head()->next());
