 ...
 * 
 * lsr_conf_parse_line/lsr_conf_parse_line_kv:
 * - Increment numItems
 ...
 * - Create a matching pOut*KV for the expected outcome for parse_line_kv
 * - Add the array to the end of pOutputKV
 * The test should now be added.
 * 
 * lsr_conf_parse_multi/lsr_get_conf_line:
...
TEST( lsr_confparser_test)
{
    ...
    {
        lsr_objarray_t *pList = lsr_conf_parse_line( &confParser, pInput[i], pInput[i] + pInputLen[i] );
        CHECK( iOutArraySize[i] == lsr_objarray_getsize( pList ));
        for( int j = 0; j < iOutArraySize[i]; ++j )
        {
            lsr_str_t *pObj = (lsr_str_t *)lsr_objarray_getobj( pList, j );
            CHECK( strncmp( pOutput[i][j], 
                            lsr_str_c_str( pObj ), 
                            lsr_str_len( pObj )
                          ) == 0 );
        }
    ...
    // Parse Multi Test
    lsr_objarray_t *pMulti = lsr_conf_parse_multi( &confParser, pBufBegin, pBufEnd );
    CHECK( lsr_objarray_getsize( pMulti ) == iMultiLineOutLen );
    for( int i = 0; i < iMultiLineOutLen; ++i )
    {
        lsr_str_t *pObj = (lsr_str_t *)lsr_objarray_getobj( pMulti, i );
        CHECK( strncmp( pMultiLineOutput[i], 
                            lsr_str_c_str( pObj ), 
                            lsr_str_len( pObj )
                          ) == 0 );
    }
    ...
    int n = 0;
    while( (pLineBegin = lsr_get_conf_line( &pBufBegin, pBufEnd, &pLineEnd )) != NULL )
    {
        CHECK( pBufBegin <= pBufEnd + 1 );
        CHECK( pLineBegin < pLineEnd );
        CHECK( n < iNumGetLineOut );
        lsr_objarray_t *pList = lsr_conf_parse_line( &confParser, pLineBegin, pLineEnd );
        CHECK( iGetLineOutSize[n] == lsr_objarray_getsize( pList ));
        for( int j = 0; j < iGetLineOutSize[n]; ++j )
        {
            lsr_str_t *pObj = (lsr_str_t *)lsr_objarray_getobj( pList, j );
            CHECK( strncmp( pGetLineOut[n][j], 
                            lsr_str_c_str( pObj ), 
                            lsr_str_len( pObj )
                          ) == 0 );
        }
    ...
    {
        pBufBegin = pInput[i];
        pLineBegin = lsr_get_conf_line( &pBufBegin, pBufBegin + pInputLen[i], &pLineEnd );
        CHECK( pLineBegin != NULL );
        lsr_objarray_t *pList = lsr_conf_parse_line_kv( &confParser, pLineBegin, pLineEnd );
        CHECK( lsr_objarray_getsize( pList ) == 2 );
        for( int j = 0; j < 2; ++j )
        {
            lsr_str_t *pObj = (lsr_str_t *)lsr_objarray_getobj( pList, j );
            CHECK( !pOutputKV[i][j] == !lsr_str_c_str( pObj ) );
            CHECK( strncmp( pOutputKV[i][j], 
                            lsr_str_c_str( pObj ), 
                            lsr_str_len( pObj )
                          ) == 0 );
        }
