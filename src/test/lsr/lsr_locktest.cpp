...
static inline int lsr_xxx_spin_unlock( lsr_spinlock_t * p )
{
    __sync_lock_release( p );
    return 0;
}

static inline int lsr_yyy_spin_unlock( lsr_spinlock_t * p )
{
    barrier();
    *p = lock_Avail;
    return 0;
...
static inline int lsr_xxx_spin_lock( lsr_spinlock_t * p )
{
    while (1)
    {
        if ( ( *p == lock_Avail )
...
static inline int lsr_yyy_spin_lock( lsr_spinlock_t * p )
{
    while (1)
    {
...
static inline int lsr_zzz_spin_lock( lsr_spinlock_t * p )
{
    while (1)
    {
        barrier();
...
#define LSI_SETUP1   lsr_atomic_spin_setup
#define LSI_TRYLOCK1 lsr_atomic_spin_trylock
#define LSI_LOCK1    lsr_atomic_spin_lock
#define LSI_UNLOCK1  lsr_atomic_spin_unlock
typedef int          LOCK_TYPE1;

#define LSI_SETUP2   lsr_futex_setup
#define LSI_TRYLOCK2 lsr_futex_trylock
#define LSI_LOCK2    lsr_futex_lock
#define LSI_UNLOCK2  lsr_futex_unlock
typedef int          LOCK_TYPE2;

#define LSI_SETUP3   lsr_pspinlock_setup
#define LSI_TRYLOCK3 lsr_pspinlock_trylock
#define LSI_LOCK3    lsr_pspinlock_lock
#define LSI_UNLOCK3  lsr_pspinlock_unlock
typedef lsr_pspinlock_t  LOCK_TYPE3;

#define LSI_SETUP4   lsr_pthread_mutex_setup
...
#define LSI_SETUP5   lsr_lock_setup
#define LSI_TRYLOCK5 lsr_lock_trylock
#define LSI_LOCK5    lsr_lock_lock
#define LSI_UNLOCK5  lsr_lock_unlock
typedef lsr_lock_t   LOCK_TYPE5;
...
int testlock( int numthreads, int index )
{
    ...
    for (i = 0; i < numthreads; i++, p++)
    {
        ...
        if (pthread_create(&p->m_thread, NULL, testinfo[index].testFunc, (void *)p))
        {
            numthreads = i;
            break;
        }
