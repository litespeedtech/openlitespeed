...
TEST( lsr_PoolTest_test )
{
    ...
    {
        ptr0[i] = (char *)lsr_palloc( 97 );
        memset( ptr0[i], fill, 97 );
        CHECK( memcmp( ptr0[i], cmp, 97 ) == 0 );
        ptr1[i] = (char *)lsr_palloc( 97 );
    ...
    ptrs[0] = ptr0[i];
    ptrs[1] = ptr1[i];
    ptrs[2] = (char *)lsr_prealloc( ptrs[0], 22 );
    CHECK( memcmp( ptrs[2], cmp, 22 ) == 0 );
    ptrs[0] = (char *)lsr_prealloc( ptrs[2], 144 );
    CHECK( memcmp( ptrs[0], cmp, 22 ) == 0 );
    lsr_pfree( (void *)ptrs[0] );
    
    ptrs[3] = (char *)lsr_prealloc( ptrs[1], 22 );
    CHECK( ptrs[3] == ptrs[1] );
    ptrs[1] = (char *)lsr_prealloc( ptrs[3], 144 );
    CHECK( memcmp( ptrs[1], cmp, 22 ) == 0 );
    ptrs[3] = (char *)lsr_prealloc( ptrs[1], LSR_POOL_LGFREELIST_MAXBYTES );
    CHECK( memcmp( ptrs[3], cmp, 22 ) == 0 );
    lsr_pfree( (void *)ptrs[3] );
    
    int *psizes = sizes;;
    for( i = 0; i < (int)(sizeof(sizes)/sizeof(sizes[0])); i++ )
    {
        ptrs[i] = (char *)lsr_palloc( *psizes );
        memset( ptrs[i], fill, *psizes++ );
    }
    
    while ( --i >= 0 )
    {
        lsr_pfree( (void *)ptrs[i] );
    }
    ...
    ptrs[0] = (char *)lsr_palloc( LSR_POOL_SMFREELIST_MAXBYTES+16 );
    lsr_pfree( (void *)ptrs[0] );
    ptrs[0] = (char *)lsr_palloc( LSR_POOL_SMFREELIST_MAXBYTES+16 );
    lsr_pfree( (void *)ptrs[0] );
    ...
    {
        lsr_pfree( (void *)ptr0[i] );
        lsr_pfree( (void *)ptr1[i] );
    }
}
