...
static int  plus_int(GHash::iterator iter, void *pData)
{
    iter->setData((void *)((long)iter->getData() + (long)pData));
    return 0;
}
...
TEST(GHashTest_test)
{
    ...
    int n = hash.for_each2(hash.begin(), hash.end(), plus_int, (void *)10);
    CHECK(n == 4);
