...
TEST(LoopBufXTest_test)
{
    lsr_xpool_t pool;
    lsr_xpool_init( &pool );
    LoopBuf *buf = new (lsr_xpool_alloc( &pool, sizeof( LoopBuf )))LoopBuf( &pool );
    CHECK( 0 == buf->size() );
    CHECK( 0 < buf->capacity() );
    ...
    LoopBuf *lbuf = new (lsr_xpool_alloc( &pool, sizeof( LoopBuf )))LoopBuf( &pool, 0 );
    buf->swap( *lbuf );
...
TEST(XLoopBufTest_test)
{
    lsr_xpool_t pool;
    lsr_xpool_init( &pool );
    XLoopBuf *buf = new (lsr_xpool_alloc( &pool, sizeof( XLoopBuf )))XLoopBuf( &pool, 0 );
    CHECK( 0 == buf->size() );
    CHECK( 0 < buf->capacity() );
    CHECK( 0 == buf->reserve(0) );
    CHECK( 0 == buf->capacity() );
    ...
    XLoopBuf *lbuf = new (lsr_xpool_alloc( &pool, sizeof( XLoopBuf )))XLoopBuf( &pool, 0 );
    buf->swap( *lbuf );
    buf->reserve(200);
    //printf( "n=%d\n", n );
    CHECK(200 <= buf->capacity());
