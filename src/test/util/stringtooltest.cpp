...
SUITE( StringtoolTest )
{
    TEST( mempbrkTest )
    {
        ...
        pAccept = "f";
        iAcceptLength = 1;
        CHECK( StringTool::mempbrk( pInput, iInputLength, pAccept, iAcceptLength ) == pInput + 5 );

        pAccept = "jklofd";
        iAcceptLength = 4;
        CHECK( StringTool::mempbrk( pInput, iInputLength, pAccept, iAcceptLength ) == NULL );

        iAcceptLength = 5;
        CHECK( StringTool::mempbrk( pInput, iInputLength, pAccept, iAcceptLength ) == pInput + 5 );

        iAcceptLength = 6;
        CHECK( StringTool::mempbrk( pInput, iInputLength, pAccept, iAcceptLength ) == pInput + 3 );
    ...
    TEST( memmemTest )
    {
        const char *pAccept, *pOutput, *pInput = "abcdefghijklmnopqrstuvwxyzghijkm";
        printf( "STRINGTOOL: Memmem Test\n" );
        pAccept = "abc";
        CHECK( (pOutput = (const char *)StringTool::memmem(pInput, strlen(pInput), pAccept, strlen(pAccept))) == pInput );
        
        pAccept = "ghijk";
        CHECK( (pOutput = (const char *)StringTool::memmem(pInput, strlen(pInput), pAccept, strlen(pAccept))) == memchr(pInput, 'g', strlen(pInput)) );
        
        pAccept = "ghijkm";
        CHECK( (pOutput = (const char *)StringTool::memmem(pInput, strlen(pInput), pAccept, strlen(pAccept))) == memrchr(pInput, 'g', strlen(pInput)) );
        
        pAccept = "sabc";
        CHECK( StringTool::memmem(pInput, strlen(pInput), pAccept, strlen(pAccept)) == NULL );
    ...
    TEST( Memspn )
    {
        ...
        for( int i = 0; i < iNumTests; ++i )
        {
            iOutput = StringTool::memspn( pMemspnInput, aMemspnInputSize[i], pMemspnAccept, aMemspnAcceptSize[i] );
    ...
    TEST( MemCspn )
    {
        ...
        for( int i = 0; i < iNumTests; ++i )
        {
            iOutput = StringTool::memcspn( pMemspnInput, aMemspnInputSize[i], pMemspnAccept, aMemspnAcceptSize[i] );
