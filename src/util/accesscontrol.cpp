...
public:
    SubNetNode(in_addr_t id, in_addr_t mask, int allowed)
        : m_id(id), m_mask(mask), m_allowed(allowed)
        {}
    ~SubNetNode();
...
SubNetNode* SubNetNode::insertChild(in_addr_t ip, in_addr_t mask, int allowed)
{
...
public:
    ...
    SubNet6Node* insertChild( const in6_addr &ip, const in6_addr &mask, int allowed);
...
SubNet6Node* SubNet6Node::insertChild( const in6_addr &ip, const in6_addr &mask, int allowed)
{
    Addr6 id;
...
int AccessControl::addSubNetControl(const char* ip, const char* netMask, int allowed)
{
...
int AccessControl::addIPControl( const in6_addr &ip, int allowed)
{
    ...
    Addr6 * p = (Addr6 *)&ip;
    if (( !p->m_addrs[0] )&&( !p->m_addrs[1] )&&( !p->m_addrs[2] )&&( !p->m_addrs[3] ))
    {
        m_pRoot6->setAccess(allowed);
        return 0;
    }
                                     ...
                                     int allowed)
{
    ...
    if ( IN6_IS_ADDR_V4MAPPED( &subNet )) 
    {
        if ( p->m_addrs[3] )
        {
            return addSubNetControl( ((Addr6 *)&subNet)->m_addrs[3], p->m_addrs[3], allowed );
        }
        return -1;
    }
